...
description_en: "Automate Bugbug tasks via Rube MCP (Composio). Always search tools first for current schemas."
description_tr: "Bugbug görevlerini Rube MCP (Composio) aracılığıyla otomatikleştirin. Her zaman mevcut şemalar için öncelikle araçları arayın."
category: "Development"
repo: "ComposioHQ/awesome-claude-skills"
stars: 65377
url: "https://github.com/ComposioHQ/awesome-claude-skills/blob/HEAD/composio-skills/bugbug-automation/SKILL.md"
...
related_files: []
body_tr: |-
  # Bugbug Otomasyonu Rube MCP Aracılığıyla
  
  Composio'nun Bugbug toolkit'i üzerinden Rube MCP aracılığıyla Bugbug işlemlerini otomatikleştirin.
  
  **Toolkit dokümantasyonu**: [composio.dev/toolkits/bugbug](https://composio.dev/toolkits/bugbug)
  
  ## Ön Koşullar
  
  - Rube MCP bağlı olmalı (RUBE_SEARCH_TOOLS kullanılabilir)
  - `RUBE_MANAGE_CONNECTIONS` aracılığıyla aktif Bugbug bağlantısı (toolkit `bugbug`)
  - Mevcut tool schema'larını almak için her zaman `RUBE_SEARCH_TOOLS` çağırın
  
  ## Kurulum
  
  **Rube MCP'yi Edinin**: İstemci konfigürasyonunuzda MCP sunucusu olarak `https://rube.app/mcp` ekleyin. API anahtarı gerekmez — sadece endpoint'i ekleyin ve çalışır.
  
  1. `RUBE_SEARCH_TOOLS`'un yanıt verdiğini doğrulayarak Rube MCP'nin kullanılabilir olduğunu kontrol edin
  2. Toolkit `bugbug` ile `RUBE_MANAGE_CONNECTIONS` çağırın
  ...
  - **Her zaman önce arayın**: Tool schema'ları değişebilir. `RUBE_SEARCH_TOOLS` çağırmadan asla tool slug'larını veya argument'leri hardcode etmeyin
  ...
  | İşlem | Yaklaşım |
  |-----------|----------|
  | Tool'ları bul | `RUBE_SEARCH_TOOLS` ile Bugbug'a özel use case |
  | Bağlan | `RUBE_MANAGE_CONNECTIONS` ile toolkit `bugbug` |
