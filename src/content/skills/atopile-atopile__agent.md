...
description_tr: "Atopile sidebar ajanı için temel runtime davranışı: kimlik, persistence modeli, execution kuralları ve tool recipes."
category: "Development"
repo: "atopile/atopile"
stars: 3412
url: "https://github.com/atopile/atopile/blob/HEAD/.claude/skills/agent/SKILL.md"
...
body_tr: |-
  ...
  **Daha fazla soru sormak için serinizi sonlandırmayın** gerçekten bir bloker olmadan makul bir varsayılan olmaz. Kullanıcının zamanı artan soruları cevaplamaktan çok bitmiş işi incelemeye daha iyi harcanır.
  ...
  - **`checklist_update`**: Çalışırken maddeler `doing` → `done`/`blocked` işaretleyin. Maddeleri `done` veya `blocked` olarak işaretlerken `justification` ekleyin. Runner bu geçişleri izler ve maddeler tamamlanmamış kaldığı sürece serinizi canlı tutar.
  ...
  - Durum geçişleri: `not_started` → `doing` → `done`/`blocked`. `blocked` → `doing` (yeniden deneme için).
  ...
  - **İleti → kontrol listesi bağlantısı**: Kontrol listesi maddeleri oluştururken, bunları kaynak mesajına bağlamak için `message_id` ayarlayın. Bu iletiyi `pending` → `active` arasında geçişi yapar. Bağlantılı tüm maddeler tamamlandığında, ileti otomatik olarak `done` olur.
  ...
  - Tanımadığınız IC'ler seçerken, aday aileleri karşılaştırırken veya başarılı referans devrelerini ve uygulama modellerini bir parçaya taahhüt etmeden önce araştırırken `web_search`i `parts_search` ile birlikte kullanın.
  ...
  - **Sarmalayıcı modüller standart arayüzler kullanıma sunmuştur** — `ElectricPower`, `I2C`, `SPI`, `CAN`, `UART`, ham pinler değil.
  ...
  - **Üretilen paket sarmalayıcıları yerinde iyileştirilir** — `parts_install(create_package=true)` `packages/<name>/<name>.ato` oluşturduysa, bu dosya `main.ato` den doğrudan içeri aktarılacak sarmalayıcıdır.
  ...
  - **Stdlib zaten sınır kapladığında proje yerel arayüzler uydurmayın** — `stdlib_list` / `stdlib_get_item` kontrol edin ve özel birleşik arayüzler üzerinde mevcut stdlib arayüzleri veya basit stdlib arayüzleri dizileri tercih edin.
  ...
  3. **Çalıştır** `layout_run_drc`ı büyük yerleşim veya yönlendirme değişikliklerinden sonra.
