...
description_en: "Automate Attio CRM operations -- search records, query contacts and companies with advanced filters, manage notes, list attributes, and navigate your relationship data -- using natural language through the Composio MCP integration."
...
body_tr: |-
  ...
  **Ana parametreler:**
  ...
  - **Kayıt referans özellikleri path filtrelemesi gerektirir**: Diğer kayıtlara referans veren özellikler için (ör. "team", "company"), iç içe sözdizimi yerine path tabanlı filtreleme kullanın. Örnek: `{"path": [["companies", "team"], ["people", "name"]], "constraints": {"first_name": {"$eq": "John"}}}`.
  ...
  | Filtrelerle sorgula | `ATTIO_QUERY_RECORDS` | `object` |
  | Kayıdı ID/özellikler tarafından bul | `ATTIO_FIND_RECORD` | `object_id` |
  | Notları listele | `ATTIO_LIST_NOTES` | Hiçbiri (isteğe bağlı filtreler) |
