...
body_tr: |-
  ...
  sections: [{
    properties: {
      page: {
        size: {
          width: 12240,   // 8.5 inç DXA cinsinden
          height: 15840   // 11 inç DXA cinsinden
        },
        margin: { top: 1440, right: 1440, bottom: 1440, left: 1440 } // 1 inçlik kenar boşlukları
      }
    },
    children: [/* content */]
  }]
  ...
  **Yatay yönelim:** docx-js genişlik/yüksekliği dahili olarak değiştirir, bu nedenle portré boyutlarını geçir ve değişimi kendisine yaptır:
  ```javascript
  size: {
    width: 12240,   // KISA kenarı genişlik olarak geçir
    height: 15840,  // UZUN kenarı yükseklik olarak geçir
    orientation: PageOrientation.LANDSCAPE  // docx-js XML'de değiştirir
  },
  ...
  const doc = new Document({
    styles: {
      default: { document: { run: { font: "Arial", size: 24 } } }, // 12pt varsayılan
      paragraphStyles: [
        // ÖNEMLİ: Yerleşik stilleri geçersiz kılmak için tam ID'leri kullan
        { id: "Heading1", name: "Heading 1", basedOn: "Normal", next: "Normal", quickFormat: true,
          run: { size: 32, bold: true, font: "Arial" },
          paragraph: { spacing: { before: 240, after: 240 }, outlineLevel: 0 } }, // outlineLevel TOC için gerekli
    ...
    rows: [
      new TableRow({
        children: [
          new TableCell({
            borders,
            width: { size: 4680, type: WidthType.DXA }, // Her hücrede de ayarla
            shading: { fill: "D5E8F0", type: ShadingType.CLEAR }, // SOLID değil CLEAR
            margins: { top: 80, bottom: 80, left: 120, right: 120 }, // Hücre dolgusu (dahili, genişliğe eklenmez)
  ...
  **Genişlik kuralları:**
  ...
  new Paragraph({
    children: [new ImageRun({
      type: "png", // Gerekli: png, jpg, jpeg, gif, bmp, svg
      data: fs.readFileSync("image.png"),
      transformation: { width: 200, height: 150 },
      altText: { title: "Title", description: "Desc", name: "Name" } // Üçü de gerekli
    })]
  })
  ```
  ...
  sections: [{
    properties: {
      column: {
        count: 2,          // sütun sayısı
        space: 720,        // sütunlar arası boşluk DXA cinsinden (720 = 0.5 inç)
        equalWidth: true,
        separate: true,    // sütunlar arasında dikey çizgi
      },
    },
  ...
  sections: [{
    properties: {
      page: { margin: { top: 1440, right: 1440, bottom: 1440, left: 1440 } } // 1440 = 1 inç
    },
    headers: {
  ...
  - **Hiçbir zaman Unicode işaretleri kullanma** - numaralandırma config ile `LevelFormat.BULLET` kullan
  - **PageBreak Paragraph'ta olmalı** - bağımsız olarak geçersiz XML oluşturur
  - **ImageRun `type` gerektirir** - her zaman png/jpg/vb. belirt
  - **Her zaman tablo `width` DXA ile ayarla** - `WidthType.PERCENTAGE` hiçbir zaman kullanma (Google Docs'ta bozulur)
  - **Tablolar çift genişlik gerektirir** - `columnWidths` dizisi VE hücre `width`, her ikisi de eşleşmeli
  - **Tablo genişliği = columnWidths toplamı** - DXA için bunların tam olarak toplamının eşit olduğundan emin ol
  - **Her zaman hücre kenar boşlukları ekle** - okunabilir dolgu için `margins: { top: 80, bottom: 80, left: 120, right: 120 }` kullan
  - **`ShadingType.CLEAR` kullan** - tablo gölgelemesi için hiçbir zaman SOLID
  - **Hiçbir zaman tabloları ayırıcı/kurallar olarak kullanma** - hücreler minimum yüksekliğe sahiptir ve boş kutular olarak render olur (üstbilgiler/altbilgiler de dahil); bunun yerine bir Paragraph'ta `border: { bottom: { style: BorderStyle.SINGLE, size: 6, color: "2E75B6", space: 1 } }` kullan. İki sütunlu altbilgiler için tabloları değil tab duraklarını kullan (Tab Duraklarını bölümüne bak)
  ...
  **Yorum ekleme:** Birden çok XML dosyası arasında ortak şeyleri işlemek için `comment.py` kullan (metin önceden kaçıştırılmış XML olmalı):
  ...
  python scripts/comment.py unpacked/ 1 "Reply text" --parent 0  # yorum 0'a yanıt
  ...
  **Tüm paragrafları/liste öğelerini silme** - bir paragraftaki TÜM içeriği kaldırırken, paragraf işaretini de sil, böylece sonraki paragrafla birleşir. `<w:pPr><w:rPr>` içine `<w:del/>` ekle:
  ...
  <w:p>
    <w:pPr>
      <w:numPr>...</w:numPr>  <!-- mevcutsa numaralandırması listele -->
      <w:rPr>
