...
stars: 64852
url: "https://github.com/ComposioHQ/awesome-claude-skills/blob/HEAD/composio-skills/callerapi-automation/SKILL.md"
...
related_files: []
body_tr: |-
  # Callerapi Otomasyonu Rube MCP ile

  Composio'nun Callerapi toolkit'i aracılığıyla Rube MCP üzerinden Callerapi operasyonlarını otomatikleştirin.

  **Toolkit dokümanları**: [composio.dev/toolkits/callerapi](https://composio.dev/toolkits/callerapi)

  ## Ön Koşullar

  - Rube MCP bağlı olmalıdır (RUBE_SEARCH_TOOLS kullanılabilir)
  - `RUBE_MANAGE_CONNECTIONS` ile aktif Callerapi bağlantısı (toolkit: `callerapi`)
  - Mevcut tool şemalarını almak için her zaman `RUBE_SEARCH_TOOLS` çağırın

  ## Kurulum

  **Rube MCP'yi Alın**: MCP sunucu konfigürasyonunuza `https://rube.app/mcp` adresini ekleyin. API anahtarı gerekmez — sadece endpoint'i ekleyin ve çalışır.

  1. `RUBE_SEARCH_TOOLS` yanıt verdiğini doğrulayarak Rube MCP'nin kullanılabilir olduğunu kontrol edin
  2. `RUBE_MANAGE_CONNECTIONS` çağırın ve toolkit'i `callerapi` olarak ayarlayın
  ...
  | İşlem | Yaklaşım |
  |-----------|----------|
  | Tool bul | Callerapi'ye özgü use case ile `RUBE_SEARCH_TOOLS` |
  | Bağlan | Toolkit `callerapi` ile `RUBE_MANAGE_CONNECTIONS` |
