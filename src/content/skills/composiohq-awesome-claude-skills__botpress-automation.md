...
stars: 64852
url: "https://github.com/ComposioHQ/awesome-claude-skills/blob/HEAD/composio-skills/botpress-automation/SKILL.md"
...
related_files: []
body_tr: |-
  # Botpress Otomasyonu via Rube MCP

  Composio'nun Botpress toolkit'i aracılığıyla Rube MCP üzerinden Botpress operasyonlarını otomatikleştirin.

  **Toolkit dokümanları**: [composio.dev/toolkits/botpress](https://composio.dev/toolkits/botpress)

  ## Ön Koşullar

  - Rube MCP bağlı olmalıdır (RUBE_SEARCH_TOOLS kullanılabilir)
  - `RUBE_MANAGE_CONNECTIONS` aracılığıyla aktif Botpress bağlantısı (toolkit `botpress` ile)
  - Mevcut tool şemalarını almak için her zaman `RUBE_SEARCH_TOOLS` çağırın

  ## Kurulum

  **Rube MCP'yi Edinin**: İstemci yapılandırmanızda `https://rube.app/mcp` adresini bir MCP sunucusu olarak ekleyin. API anahtarı gerekli değildir — sadece endpoint'i ekleyin ve çalışır.

  1. `RUBE_SEARCH_TOOLS` yanıt verdiğini doğrulayarak Rube MCP'nin kullanılabilir olduğunu kontrol edin
  2. `RUBE_MANAGE_CONNECTIONS` öğesini toolkit `botpress` ile çağırın
  ...
  - **Her zaman önce arama yapın**: Tool şemaları değişir. `RUBE_SEARCH_TOOLS` çağrısı yapmadan hiçbir zaman tool slug'larını veya argümanları hardcode etmeyin
  ...
  | İşlem | Yaklaşım |
  |-------|----------|
  | Tool'ları bul | Botpress'e özgü use case ile `RUBE_SEARCH_TOOLS` |
  | Bağlan | `RUBE_MANAGE_CONNECTIONS` ve toolkit `botpress` |
