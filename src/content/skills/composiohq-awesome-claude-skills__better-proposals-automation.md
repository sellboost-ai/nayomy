...
description_en: "Automate Better Proposals tasks via Rube MCP (Composio). Always search tools first for current schemas."
description_tr: "Better Proposals görevlerini Rube MCP (Composio) üzerinden otomatize edin. Her zaman en güncel şemaları için önce araçları arayın."
category: "Development"
repo: "ComposioHQ/awesome-claude-skills"
stars: 65266
url: "https://github.com/ComposioHQ/awesome-claude-skills/blob/HEAD/composio-skills/better-proposals-automation/SKILL.md"
...
related_files: []
body_tr: |-
  # Rube MCP ile Better Proposals Otomasyonu
  
  Better Proposals işlemlerini Rube MCP üzerinden Composio'nun Better Proposals toolkit'i ile otomatikleştirin.
  
  **Toolkit dokümanları**: [composio.dev/toolkits/better_proposals](https://composio.dev/toolkits/better_proposals)
  
  ## Ön Koşullar
  
  - Rube MCP bağlı olmalıdır (RUBE_SEARCH_TOOLS mevcut)
  - `RUBE_MANAGE_CONNECTIONS` üzerinden aktif Better Proposals bağlantısı ve `better_proposals` toolkit'i
  - Geçerli tool şemalarını almak için her zaman `RUBE_SEARCH_TOOLS` çağrısı yapın
  
  ## Kurulum
  
  **Rube MCP'yi Alın**: İstemci konfigürasyonunuzda `https://rube.app/mcp` öğesini MCP sunucusu olarak ekleyin. API anahtarı gerekmez — sadece endpoint'i ekleyin ve çalışır.
  
  1. `RUBE_SEARCH_TOOLS` yanıt verdiğini doğrulayarak Rube MCP'nin mevcut olduğunu kontrol edin
  2. `RUBE_MANAGE_CONNECTIONS` öğesini `better_proposals` toolkit'i ile çağırın
  ...
  - **Her zaman önce ara**: Tool şemaları değişir. `RUBE_SEARCH_TOOLS` çağrısı yapmadan tool slug'larını veya argümanları hiçbir zaman hardcode etmeyin
  ...
  | İşlem | Yaklaşım |
  |-----------|----------|
  | Tool'ları bul | Better Proposals'a özgü use case ile `RUBE_SEARCH_TOOLS` |
  | Bağlan | `better_proposals` toolkit'i ile `RUBE_MANAGE_CONNECTIONS` |
