...
description_en: "Automate Centralstationcrm tasks via Rube MCP (Composio). Always search tools first for current schemas."
description_tr: "Centralstationcrm görevlerini Rube MCP (Composio) aracılığıyla otomatikleştirin. Her zaman güncel şemaları için tools içinde arama yapın."
category: "Development"
repo: "ComposioHQ/awesome-claude-skills"
stars: 65377
url: "https://github.com/ComposioHQ/awesome-claude-skills/blob/HEAD/composio-skills/centralstationcrm-automation/SKILL.md"
...
related_files: []
body_tr: |-
  # Rube MCP ile Centralstationcrm Otomasyonu
  
  Composio'nun Centralstationcrm toolkit'ini Rube MCP aracılığıyla kullanarak Centralstationcrm işlemlerini otomatikleştirin.
  
  **Toolkit dokümanları**: [composio.dev/toolkits/centralstationcrm](https://composio.dev/toolkits/centralstationcrm)
  
  ## Ön Koşullar
  
  - Rube MCP bağlı olmalıdır (RUBE_SEARCH_TOOLS kullanılabilir olmalıdır)
  - `RUBE_MANAGE_CONNECTIONS` aracılığıyla aktif Centralstationcrm bağlantısı `centralstationcrm` toolkit'i ile
  - Mevcut tool şemalarını almak için her zaman önce `RUBE_SEARCH_TOOLS` çağırın
  
  ## Kurulum
  
  **Rube MCP'yi Edinin**: İstemci konfigürasyonunuza MCP sunucusu olarak `https://rube.app/mcp` ekleyin. API anahtarı gerekmez — sadece endpoint'i ekleyin ve çalışır.
  
  1. `RUBE_SEARCH_TOOLS` yanıt vererek Rube MCP'nin kullanılabilir olduğunu doğrulayın
  2. `RUBE_MANAGE_CONNECTIONS`'ı `centralstationcrm` toolkit'i ile çağırın
  ...
  İş akışlarını yürütmeden önce her zaman kullanılabilir tool'ları keşfedin:
  ...
  - **Her zaman önce ara**: Tool şemaları değişebilir. `RUBE_SEARCH_TOOLS` çağırmadan tool slug'ları veya argümanları asla hardcode etmeyin
  - **Bağlantıyı kontrol edin**: Tool'ları yürütmeden önce `RUBE_MANAGE_CONNECTIONS`'ın ACTIVE durumu gösterdiğini doğrulayın
  ...
  - **Pagination**: Yanıtları pagination token'ları için kontrol edin ve tamamlanana kadar almaya devam edin
  
  ## Hızlı Referans
  
  | İşlem | Yaklaşım |
  |-----------|----------|
  | Tool'ları bul | `RUBE_SEARCH_TOOLS` ile Centralstationcrm'e özgü use case |
  | Bağlan | `RUBE_MANAGE_CONNECTIONS` ile `centralstationcrm` toolkit'i |
