...
description_en: "Automate Callingly tasks via Rube MCP (Composio). Always search tools first for current schemas."
...
related_files: []
body_tr: |-
  # Callingly Otomasyonu Rube MCP aracılığıyla
  
  Composio'nun Callingly toolkit'i aracılığıyla Rube MCP ile Callingly işlemlerini otomatikleştirin.
  
  **Toolkit dokümantasyonu**: [composio.dev/toolkits/callingly](https://composio.dev/toolkits/callingly)
  
  ## Ön Koşullar
  
  - Rube MCP bağlı olmalıdır (RUBE_SEARCH_TOOLS kullanılabilir)
  - `RUBE_MANAGE_CONNECTIONS` aracılığıyla aktif Callingly bağlantısı ve `callingly` toolkit'i
  - Mevcut tool şemalarını almak için her zaman önce `RUBE_SEARCH_TOOLS` çağırın
  
  ## Kurulum
  
  **Rube MCP'yi Alın**: MCP sunucusu olarak `https://rube.app/mcp` adresini istemci yapılandırmanıza ekleyin. API anahtarı gerekmez — endpoint'i ekleyin ve çalışır.
  
  1. `RUBE_SEARCH_TOOLS` yanıt verdiğini doğrulayarak Rube MCP'nin kullanılabilir olduğunu kontrol edin
  2. `RUBE_MANAGE_CONNECTIONS` öğesini `callingly` toolkit'i ile çağırın
  ...
  | İşlem | Yaklaşım |
  |-----------|----------|
  | Tool'ları bulun | `RUBE_SEARCH_TOOLS` öğesini Callingly'ye özel use case ile kullanın |
  | Bağlan | `RUBE_MANAGE_CONNECTIONS` öğesini `callingly` toolkit'i ile kullanın |
  ...
  | Tam şema | `RUBE_GET_TOOL_SCHEMAS` öğesini `schemaRef` olan tool'lar için kullanın |
