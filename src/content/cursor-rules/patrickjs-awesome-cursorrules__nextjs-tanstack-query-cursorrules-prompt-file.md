...
description_tr: "Cursor rules for Next.js App Router ile TanStack Query v5 için Türkçe rehber, HydrationBoundary pattern, Server Actions as mutations ve optimistic updates konularını kapsamaktadır."
category: "Frontend"
...
file_extension: ".mdc"
body_tr: |-
  # Next.js App Router + TanStack Query v5 Rehberi

  ## Mimari Felsefesi
  - Server Components verileri doğrudan alır (orada TanStack Query'ye gerek yok)
  - TanStack Query Client Components'te etkileşimli, gerçek zamanlı veya kullanıcı tarafından tetiklenen veriler için kullanılır
  - React Server Components'i ilk sayfa verisi için kullanın; TanStack Query'yi mutasyonlar, polling ve iyimser güncellemeler için kullanın
  - Query cache'ini sunucudan client'a aktarın, ilk yüklemede istemci cascading'lerinden kaçınmak için

  ## Provider Kurulumu ve Hidrasyon
  ```tsx
  ...
  - Server Components'te prefetch yapın, durumu dehidre edin, client'ta rehidre edin
  - Bu ilk render'da client tarafı yükleme durumlarını ortadan kaldırır
  ```tsx
  ...
  - Bu size tip-güvenli sunucu mutasyonları VE iyimser güncelleme/rollback yetenekleri sağlar
  ```tsx
  ...
  ## Server Actions ile Iyimser Güncellemeler
  ```tsx
  const mutation = useMutation({
  ...
  ## Server Components vs TanStack Query Ne Zaman Kullanılır
  | Server Components Kullanılması Gerektiğinde | TanStack Query Kullanılması Gerektiğinde |
  |---|---|
  | Statik veya nadiren değişen veriler | Gerçek zamanlı veya sık güncellenen veriler |
  | SEO açısından kritik ilk içerik | Kullanıcı etkileşimleri (formlar, toggle'lar) |
  | Client'ta yeniden fetch'e gerek yok | Iyimser güncellemeler gerekli |
  | Veriler component'ler arasında paylaşılmaz | Veriler birçok component'te paylaşılır |
  | Yükleme durumları istenmez | İnce taneli yükleme/hata UI'si gerekli |

  ## Route Handler'lar (API Rotaları) Query Hedefleri Olarak
  - `src/app/api/` route handler'larını TanStack Query fetcher'ları için API katmanı olarak kullanın
  - Route handler'ları sade tutun — sadece input'u parse/doğrulayın ve servis katmanını çağırın
  ```ts
  ...
  ## Sonsuz Sorgular (Sayfalama / Sonsuz Scroll)
  ```tsx
  'use client'
  ...
  ## Temel Kurallar
  - Sunucu tarafında istek başına bir `QueryClient` oluşturun (Server Components içinde)
  - Client'ta tarayıcı oturumu başına bir `QueryClient` oluşturun (provider'da `useState` aracılığıyla)
  - Sunucu tarafından önceden fetch'lenmiş verileri client component'lere iletirken her zaman `HydrationBoundary` kullanın
  - Client Components içinde hiçbir zaman `fetch`'i doğrudan çağırmayın — her zaman `queryFn` üzerinden geçin
  - TanStack Query hook'larını kullanan tüm component'leri `'use client'` ile işaretleyin
---
