...
description_tr: "Beefree SDK ile uygulama geliştirmek için kılavuzlar ve en iyi uygulamalar - kurulum, kimlik doğrulama, konfigürasyon, özelleştirme ve template yönetimi dahil."
category: "Other"
...
file_extension: ".mdc"
body_tr: |-
  # Beefree SDK Yönergeleri
  Beefree SDK ile uygulamalar oluşturmak için yönergeler ve en iyi uygulamalar ([Beefree SDK](https://docs.beefree.io/beefree-sdk) dahil olmak üzere kurulum, kimlik doğrulama, yapılandırma, özelleştirme ve şablon yönetimi.

  ## Kurulum Yönergeleri

  ### Paket Kurulumu
  - Beefree SDK paketini npm veya yarn kullanarak kurun:
    ```bash
  ...
  ### Ortam Kurulumu
  - Proje kökünüzde Beefree kimlik bilgilerinizle bir `.env` dosyası oluşturun:
    ```env
  ...
  - Kimlik bilgilerinizi korumak için kimlik doğrulama için HER ZAMAN bir proxy sunucusu kullanın
  - Kimlik doğrulamayı işlemek için bir proxy sunucusu dosyası oluşturun (örneğin, `proxy-server.js`):
    ```javascript
  ...
  - V2 kimlik doğrulama uç noktasını kullanın: `https://auth.getbee.io/loginV2`
  - TAMAMLANMIŞ API yanıtını Beefree SDK'ya geçirin, sadece token değil
  ...
  ## Konteyner Kurulum Yönergeleri

  ### HTML Konteyner
  - Beefree SDK için adanmış bir konteyner öğesi oluşturun:
    ```html
    <div id="beefree-sdk-container"></div>
    ```

  ### CSS Stil
  - Konteyner için uygun görüntülenmesini sağlamak amacıyla stil uygulayın:
    ```css
  ...
  ### React Konteyner
  - React uygulamaları için, aşağıdaki kod parçacığı konteyner yönetimi için refs kullanarak bir örnek gösterir:
    ```typescript
  ...
  - Yapılandırmanızda HER ZAMAN `container` parametresini ekleyin:
    ```typescript
    const beeConfig = {
  ...
  - SDK'yı isteğe bağlı parametrelerle özelleştirin:
    ```typescript
    const beeConfig = {
      container: 'beefree-sdk-container', // Gerekli
      language: 'en-US',
  ...
  - Uygun işlevsellik için gerekli callback fonksiyonlarını uygulayın:
    ```typescript
    const beeConfig = {
      container: 'beefree-sdk-container',
      onSave: function (jsonFile, htmlFile) {
        console.log("Template saved:", jsonFile);
        // Burada özel kaydetme mantığını uygulayın
      },
      ...
      onSend: function (htmlFile) {
        ...
        // Burada özel gönderme mantığını uygulayın
      },
  ...
  ### React Entegrasyonu
  - React uygulamaları için, aşağıdaki kod parçacığı başlatma için useEffect kullanan bir örnek gösterir:
    ```typescript
    useEffect(() => {
  ...
  ## Şablon Yükleme Yönergeleri

  ### Şablonları Yükleme
  - Mevcut şablonları yüklemek için `start()` yöntemi ile şablon verilerini kullanın:
    ```typescript
  ...
  ### Şablon Depolama
  - Test sırasında kalıcılık için şablonları localStorage'da depolayın:
    ```typescript
    // Şablon verisini kaydet
  ...
  - Veri kaybını önlemek için otomatik kaydetmeyi uygulayın:
    ```typescript
    onAutoSave: function (jsonFile) {
      console.log("Auto-saving template...");
      localStorage.setItem("email.autosave", jsonFile);
  ...
  ## HTML İçeri Aktarma Yönergeleri

  ### HTML İçeri Aktarıcı API
  - Mevcut HTML şablonlarını Beefree SDK formatına dönüştürmek için HTML İçeri Aktarıcı API'yi kullanın
  - API uç noktası: `https://api.getbee.io/v1/conversion/html-to-json`
  - Referans: [HTML İçeri Aktarıcı API Belgeleri](https://docs.beefree.io/beefree-sdk/apis/html-importer-api/import-html)

  ### İçeri Aktarma Süreci
  ...
  ### İçeri Aktarılan Şablonları Yükleme
  - İçeri aktarılan şablonları Beefree SDK'ya yükleyin:
    ```typescript
  ...
  ### onError Callback
  - SDK hatalarını işlemek için HER ZAMAN `onError` callback'ini uygulayın:
    ```typescript
    onError: function (errorMessage) {
      console.error("Beefree SDK error:", errorMessage);
      // Kullanıcı dostu hata mesajını görüntüle
  ...
  - Kimlik doğrulama başarısızlıklarını normal şekilde işleyin:
    ```typescript
  ...
  ## Şablon Değişim İzleme Yönergeleri

  ### İleti Değişikliklerini İzleme
  - Son kullanıcılar tarafından yapılan değişiklikleri izlemek için şablon değişim izlemeyi uygulayın
  - Referans: [İleti Değişikliklerini İzleme Belgeleri](https://docs.beefree.io/beefree-sdk/getting-started/tracking-message-changes)
  ...
  ### Kullanıcı Arabirimi Özelleştirmesi
  Beefree SDK görünümünü şu yollarla özelleştirin:
  ...
  ### Merge Tag'leri ve Özel Bağlantılar
  - E-posta kişiselleştirmesi için merge tag'lerini ve özel bağlantıları yapılandırın:
    ```typescript
    const beeConfig = {
      container: 'beefree-sdk-container',
      mergeTags: [
  ...
  Olası özelleştirmelerin kapsamlı bir referansı için resmi [Beefree SDK teknik belgelerine](https://docs.beefree.io/beefree-sdk) başvurun.  

  ## En İyi Uygulamalar

  ### Performans Optimizasyonu
  - Beefree SDK'yı yalnızca uygulamanızda gerçekten gerekli olduğunda başlatın.
  - SDK kaynakları artık gerekli olmadığında düzgün şekilde temizleyin (örneğin, başka bir sayfaya gittiğinizde veya editörü kapattığınızda).
  - Uygulama çökmesini veya beklenmeyen davranışları önlemek için hataları normal şekilde işleyin.

  ### Güvenlik
  - Beefree SDK istemci kimlik bilgilerinizi **ASLA** ön uç veya genel kodda açığa çıkarmayın.
  - Kimlik doğrulama ve hassas işlemleri yönetmek için her zaman güvenli bir arka uç veya proxy sunucusu kullanın.
  - Güvenlik açıklarını önlemek için SDK'ya geçirmeden önce tüm kullanıcı girdilerini doğrulayın ve temizleyin.

  ### Kullanıcı Deneyimi
  - SDK başlatılırken veya işlem gerçekleştirirken uygun yükleme göstergelerini gösterin.
  - Bir sorun oluşursa kullanıcılara açık ve yardımcı hata mesajları görüntüleyin.
  - Veri kaybını önlemek için otomatik kaydetmeyi veya ilerleme izlemeyi uygulayın.

  ### Kod Organizasyonu
  - SDK yapılandırmasını başlatma ve iş mantığından ayrı tutun, böylece daha iyi bakımlanabilirlik sağlanır.
  - Kod güvenliğini ve açıklığını geliştirmek için mümkün olduğunca güçlü yazma (örneğin TypeScript veya benzer) kullanın.
  - Tech stack'ten veya kullanılan framework'ten bağımsız olarak entegrasyon boyunca robust hata işlemeyi sağlayın.

  ## Örnekler

  ### Tam React Bileşeni
  Tam projeye [beefree-react-demo](https://github.com/BeefreeSDK/beefree-react-demo) adresinde bakın.
  ```typescript
  ...
  ### Tam HTML Uygulaması
  Tam projeye Beefree SDK [multiple-versions-concept](https://github.com/BeefreeSDK/beefree-sdk-simple-schema/tree/main/multiple-versions-concept) adresinde bakın.
  ```html
  <!DOCTYPE html>
  <html lang="en">
    <head>
