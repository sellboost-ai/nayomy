...
description_tr: "Snowpark Python (DataFrames, UDFs, stored procedures) ve dbt ile Snowflake adapter için Cursor rules."
category: "Other"
...
body_tr: |-
  ...
  // Snowpark Python geliştirme ve dbt (Snowflake adapter ile) için uzman rehberlik

  Snowpark Python (Snowflake'in sunucu tarafı Python API'si) ve dbt-snowflake adapter konusunda uzmanısınız. Hem araçları kullanarak production-grade veri dönüştürme pipeline'ları oluşturursunuz.

  // ═══════════════════════════════════════════
  // SNOWPARK PYTHON
  // ═══════════════════════════════════════════

  // Snowpark, Python'u Snowflake warehouse'larında sunucu tarafında çalıştırır. Veriler hiçbir zaman Snowflake'i terk etmez.
  ...
  // DataFrame API — Tembel değerlendirme (lazy evaluation), collect()/show() sırasında yürütülen sorgu planı oluşturur.
  ...
  // Temel işlemler: .filter(), .select(), .group_by().agg(), .join(), .sort(),
  ...
  // Vektörleştirilmiş UDF'ler (ML inference için 10-100x daha hızlı):
  import pandas as pd
  ...
  // UDTF'ler (girdi başına birden fazla satır döndür):
  class Tokenizer:
  ...
  // Stored Procedure'ler (sunucu tarafı çok adımlı mantık):
  ...
  // cluster_by=['col1', 'col2']  — Kümeleme (yalnızca büyük tablolar)
  // transient=true               — Fail-safe yok (düşük depolama maliyeti)
  // query_tag='finance_daily'    — İş yükü atama
  // copy_grants=true             — Değiştirilirken erişim korunur
  // snowflake_warehouse='lg_wh'  — Model başına warehouse geçersiz kılma
  // secure=true                  — Güvenli görünümler
  ...
  // En İyi Uygulamalar
  - ML inference için vektörleştirilmiş UDF'leri (pandas) tercih edin — scalar UDF'lerden çok daha hızlı.
  - Production UDF'ler ve stored procedure'lerde paket sürümlerini sabitleyin.
  - Yeniden kullanılabilir Python pipeline'larında ham SQL string'leri yerine DataFrame API'sini kullanın.
  - Yeniden adlandırma/tür dönüştürme için staging modelleri (stg_*), işletme tabloları için mart modelleri kullanın.
  - Fact table'lar için incremental, neredeyse gerçek zamanlı için dynamic_table kullanın.
  - Incremental modellerde on_schema_change='sync_all_columns' ayarlayın.
  - İzin sorunlarından kaçınmak için copy_grants=true kullanın. Seçici yürütme için modelleri etiketleyin.
  - dbt çalıştırmaları ile analist sorguları için ayrı warehouse'lar kullanın.

  // Anti-Desenler
  - Büyük DataFrame'leri istemciye collect() etmeyin — sunucu tarafında işleyin.
  - Satırlar üzerinde Python döngüleri kullanmayın — DataFrame işlemlerini veya vektörleştirilmiş UDF'leri kullanın.
  - {% if is_incremental() %} guard olmadan {{ this }} kullanmayın.
  - Küçük tablolar (< 1TB) üzerinde cluster_by ayarlamayın.
  - Tüm şeyler için materialized='table' kullanmayın — görünümler ücretsizdir.
  - Veritabanı/schema'yı kodlamayın — {{ ref() }} ve {{ source() }} kullanın.
  ```
---
