...
body_tr: |-
  ...
  - Her zaman her değişkenin ve fonksiyonun türünü (parametreler ve dönüş değeri) bildirin.
  - Gerekli türleri ve sınıfları oluşturun.
  - Public sınıflar ve metodları belgelemek için Doxygen stil yorumlar kullanın.
  - Bir fonksiyonun içinde boş satırlar bırakmayın.
  - Tek Tanım Kuralı'nı (ODR) izleyin.
  ...
  - Magic number'lardan kaçının ve sabitleri tanımlayın.
  ...
  - Boolean döndürürse, isX, hasX, canX vb. kullanın.
  - Hiçbir şey döndürmezse (void), executeX, saveX vb. kullanın.
  - Blokları yuvalamaktan kaçının:
    - Erken kontrol ve dönüşler.
    - Yardımcı fonksiyonlara çıkartma.
  - Fonksiyon yuvalamaktan kaçınmak için standart kütüphane algoritmaları kullanın (std::for_each, std::transform, std::find, vb.).
  ...
  - Soyutlamanın tek bir seviyesini kullanın.

  ## Veri

  - İlkel türleri kötüye kullanmayın ve verileri bileşik türlere kapsülleyin.
  - Fonksiyonlarda veri doğrulamalarından kaçının ve dahili doğrulama ile sınıflar kullanın.
  - Verilerin immutability'sini tercih edin.
  ...
  - SOLID ilkelerini izleyin.
  - Mirası kalıtımdan daha çok tercih edin.
  - Arayüzleri soyut sınıflar veya kavramlar olarak bildirin.
  - Tek bir amacı olan küçük sınıflar yazın.
    - 200 komuttan az.
    - 10'dan az public metod.
    - 10'dan az özellik.
  - Kaynak yönetimi için Rule of Five'ı (veya Rule of Zero'yu) kullanın.
  - Üye değişkenleri private yapın ve gerekli yerlerde getter/setter sağlayın.
  - Üye fonksiyonları için const-correctness kullanın.

  ## İstisnalar

  - Beklemediğiniz hataları işlemek için istisnaları kullanın.
  - Bir istisnayı yakalaarsanız, bu aşağıdakiler için olmalıdır:
  ...
  - Ham göstericiler yerine akıllı göstericileri tercih edin (`std::unique_ptr`, `std::shared_ptr`).
  - RAII (Kaynak Edinimi İnisiyalizasyondur) ilkelerini kullanın.
  ...
  - Testler için Arrange-Act-Assert konvansiyonunu izleyin.
  - Test değişkenlerini açık şekilde adlandırın.
  - Konvansiyonu izleyin: inputX, mockX, actualX, expectedX, vb.
  - Her public fonksiyon için unit test yazın.
  - Bağımlılıkları simüle etmek için test double'larını kullanın.
    - Yürütülmesi pahalı olmayan üçüncü taraf bağımlılıklar hariç.
  - Her modül için entegrasyon testleri yazın.
  - Given-When-Then konvansiyonunu izleyin.
  ...
  - Kodu mantıksal şekilde organize etmek için namespace'leri kullanın.
  - Temel bileşenler için bir core namespace oluşturun.
  - Yardımcı fonksiyonlar için bir utils namespace oluşturun.

  ## Standart Kütüphane

  - Mümkün olan her zaman C++ Standart Kütüphanesini kullanın.
  - C tarzı stringler yerine `std::string` tercih edin.
  ...
  - Thread tabanlı paralelizmden daha çok görev tabanlı paralelizmı tercih edin.
  - Atomik işlemler için `std::atomic` kullanın.
  - Uygun senkronizasyonla data race'lerden kaçının.
  - Gerekli olduğunda thread-safe veri yapıları kullanın.
---
