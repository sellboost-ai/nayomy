...
description_tr: "Guide Cursor, modern C++ ve CMake kodlarını net yapıyla, RAII, const-correctness ve güvenli error handling uygulamalarıyla yazmanız için rehberlik eder."
category: "Languages"
...
body_tr: |-
  ...
  - Her zaman her değişkenin ve fonksiyonun türünü deklarе edin (parametreler ve dönüş değeri).
  - Gerekli türleri ve sınıfları oluşturun.
  - Public sınıfları ve metotları dokümante etmek için Doxygen stil yorumlar kullanın.
  ...
  - Magic number'lardan kaçının ve sabitler tanımlayın.
  - Her fonksiyonu bir fiil ile başlatın.
  - Boolean değişkenler için fiiller kullanın. Örnek: isLoading, hasError, canDelete, vb.
  - Kısaltmalar yerine tam kelimeleri kullanın ve doğru yazılımı sağlayın.
    - API, URL gibi standart kısaltmalar hariç.
    - İyi bilinen kısaltmalar hariç:
      - i, j, k döngüler için
      - err hatalar için
      - ctx bağlamlar için
      - req, res request/response parametreleri için

  ## Fonksiyonlar

  - Tek bir amacı olan kısa fonksiyonlar yazın. 20 komuttan az.
  - Fonksiyonları bir fiil ve başka bir şey ile adlandırın.
  - Boolean döndürüyorsa, isX, hasX, canX vb. kullanın.
  - Hiçbir şey döndürmüyorsa (void), executeX, saveX vb. kullanın.
  - Blokları iç içe yerleştirmekten kaçının:
    - Erken kontroller ve dönüşler yapın.
    - Utility fonksiyonlara çıkarın.
  - Fonksiyon iç içe yerleştirmesinden kaçınmak için standart kütüphane algoritmalarını (std::for_each, std::transform, std::find, vb.) kullanın.
  - Basit işlemler için lambda fonksiyonları kullanın.
  - Basit olmayan işlemler için adlandırılmış fonksiyonlar kullanın.
  - Null veya nullptr kontrolü yerine varsayılan parametre değerleri kullanın.
  - Fonksiyon parametrelerini struct veya sınıf kullanarak azaltın.
    - Birden fazla parametreyi geçmek için bir nesne kullanın.
    - Birden fazla sonucu döndürmek için bir nesne kullanın.
    - Giriş argümanları ve çıktısı için gerekli türleri deklarе edin.
  - Tek bir abstraksiyon seviyesi kullanın.

  ## Veri

  - Primitive türleri kötüye kullanmayın ve verileri composite türlerde kapsülleyin.
  - Fonksiyonlarda veri doğrulamalarından kaçının ve içsel doğrulama içeren sınıflar kullanın.
  - İmmutability'i tercih edin.
  - Değişmeyen veriler için const kullanın.
  - Derleme zamanı sabitleri için constexpr kullanın.
  - Olası null değerler için std::optional kullanın.

  ## Sınıflar

  - SOLID prensiplerine uyun.
  - Mirası tercih etmek yerine composition'ı tercih edin.
  - Interface'leri abstract sınıflar veya kavramlar olarak deklarе edin.
  ...
  - Member değişkenleri private yapın ve gerekli yerlerde getter/setter sağlayın.
  - Member fonksiyonlar için const-correctness kullanın.

  ## İstisnalar

  - Beklenmediğiniz hataları işlemek için istisnalar kullanın.
  - Bir istisna yakalamanız gerekirse, şunlar için olmalıdır:
    - Beklenen bir problemi düzeltin.
    - Bağlam ekleyin.
    - Aksi takdirde, global bir handler kullanın.
  - Beklenen başarısızlıklar için std::optional, std::expected veya hata kodlarını kullanın.

  ## Bellek Yönetimi

  - Raw pointer'lar yerine smart pointer'ları (std::unique_ptr, std::shared_ptr) tercih edin.
  ...
  - C-style diziler yerine std::vector ve diğer standart konteynerleri kullanın.
  ...
  - Her public fonksiyon için unit testler yazın.
  - Bağımlılıkları simüle etmek için test doubles kullanın.
    - Yürütülmesi pahalı olmayan üçüncü taraf bağımlılıkları hariç.
  - Her modül için integration testler yazın.
  - Given-When-Then kuralına uyun.

  ## Proje Yapısı

  - Modüler mimariyi kullanın.
  ...
  - CMake veya benzer bir build sistemi kullanın.
  - Interface (.h) ve implementation (.cpp) ayrı tutun.
  - Kodu mantıksal olarak organize etmek için namespace'ler kullanın.
  - Temel bileşenler için core namespace oluşturun.
  - Utility fonksiyonları için utils namespace oluşturun.

  ## Standart Kütüphane

  - Mümkün olduğunda C++ Standart Kütüphanesini kullanın.
  - C-style stringler yerine std::string tercih edin.
  - Koleksiyonlar için std::vector, std::map, std::unordered_map vb. kullanın.
  - Modern type safety için std::optional, std::variant, std::any kullanın.
  ...
  - Thread-tabanlı paralelizm yerine task-tabanlı paralelizmi tercih edin.
  - Atomik işlemler için std::atomic kullanın.
  - Uygun senkronizasyon ile data race'lerden kaçının.
  - Gerektiğinde thread-safe veri yapıları kullanın.
---
