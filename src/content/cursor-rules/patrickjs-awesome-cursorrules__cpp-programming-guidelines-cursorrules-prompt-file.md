...
description_tr: "C++ geliştirme için Cursor kuralları ve programlama yönergeleri entegrasyonu."
category: "Languages"
...
body_tr: |-
  ...
  - Her zaman her değişken ve fonksiyonun türünü (parametreler ve dönüş değeri) belirtin.
  - Gerekli türleri ve sınıfları oluşturun.
  - Genel sınıfları ve metotları dokümante etmek için Doxygen stil yorumlar kullanın.
  - Bir fonksiyon içinde boş satır bırakmayın.
  - Tek Tanım Kuralını (ODR) izleyin.
  ...
  - Her fonksiyonu bir fiille başlatın.
  ...
  - Tek bir amaca sahip kısa fonksiyonlar yazın. 20 yönergenin altında.
  - Fonksiyonlara bir fiil ve başka bir şey ile ad verin.
  - Boolean dönüş yapıyorsa, isX veya hasX, canX vb. kullanın.
  - Hiçbir şey dönmüyorsa (void), executeX veya saveX vb. kullanın.
  - Blok iç içeliğini şunlarla önleyin:
    - Erken kontroller ve dönüşler.
    - Yardımcı fonksiyonlara çıkartma.
  - Fonksiyon iç içeliğini önlemek için standart kütüphane algoritmaları (std::for_each, std::transform, std::find vb.) kullanın.
  - Basit işlemler için lambda fonksiyonlar kullanın.
  - Karmaşık olmayan işlemler için adlandırılmış fonksiyonlar kullanın.
  - Null veya nullptr kontrolü yerine varsayılan parametre değerleri kullanın.
  - Yapılar veya sınıflar kullanarak fonksiyon parametrelerini azaltın.
    - Birden fazla parametreyi geçmek için bir nesne kullanın.
    - Birden fazla sonucu dönmek için bir nesne kullanın.
    - Giriş argümanları ve çıkışı için gerekli türleri tanımlayın.
  - Tek bir soyutlama düzeyi kullanın.

  ## Veri

  - İlkel türleri kötüye kullanmayın ve verileri bileşik türlerde kapsülleyin.
  - Fonksiyonlarda veri doğrulamasından kaçının ve içsel doğrulamaya sahip sınıflar kullanın.
  - Veri değişmezliğini tercih edin.
  - Değişmeyen veri için const kullanın.
  - Derleme zamanı sabitleri için constexpr kullanın.
  - Muhtemelen null olabilecek değerler için std::optional kullanın.

  ## Sınıflar

  - SOLID ilkelerine uyun.
  - Kalıtım yerine bileşimi tercih edin.
  - Arayüzleri soyut sınıflar veya konseptler olarak tanımlayın.
  - Tek bir amaca sahip küçük sınıflar yazın.
    - 200 yönergenin altında.
    - 10'dan az genel metot.
    - 10'dan az özellik.
  - Kaynak yönetimi için Beş Kuralını (veya Sıfır Kuralını) kullanın.
  - Üye değişkenlerini private yapın ve gerektiğinde getter/setter sağlayın.
  - Üye fonksiyonlar için const doğruluğu kullanın.

  ## İstisnalar

  - Beklemediğiniz hataları işlemek için istisnalar kullanın.
  - Bir istisna yakalarsanız, bunun amacı şu olmalıdır:
    - Beklenen bir sorunu düzeltmek.
    - Bağlam eklemek.
    - Aksi takdirde global bir işleyici kullanın.
  - Beklenen hatalar için std::optional, std::expected veya hata kodlarını kullanın.

  ## Bellek Yönetimi

  - Ham işaretçilerden ziyade akıllı işaretçileri (std::unique_ptr, std::shared_ptr) tercih edin.
  - RAII (Kaynak Edinme İnicializmedir) ilkelerini kullanın.
  - Uygun kaynak yönetimi ile bellek sızıntılarından kaçının.
  - C stili diziler yerine std::vector ve diğer standart konteynerları kullanın.
  ...
  - Her genel fonksiyon için birim testler yazın.
  ...
  - CMake veya benzeri bir derleme sistemi kullanın.
  - Arayüzü (.h) uygulamadan (.cpp) ayırın.
  - Kodu mantıksal olarak organize etmek için ad alanları kullanın.
  ...
  - Zamanla ilgili işlemler için std::chrono kullanın.
  ...
  - Gerektiğinde iş parçacığı güvenli veri yapıları kullanın.
---
