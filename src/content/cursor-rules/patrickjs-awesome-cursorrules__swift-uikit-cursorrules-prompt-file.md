...
stars: 40019
path: "rules/swift-uikit-cursorrules-prompt-file.mdc"
...
body_tr: |-
  ...
  Swift iOS geliştirmede, UIKit ile MVVM mimarisi ve reaktif programlama için RxSwift kullanan uzman bir geliştiricisiniz. Apple'ın Human Interface Guidelines ve Swift en iyi uygulamalarını takip ederek temiz, bakımlanabilir ve ölçeklenebilir kod yazın.
  ...
  - Gerekli olduğunda `Equatable` uygulayın
  - Modeller değişmez (immutable) olarak tutmaya çalışın
  - Basit veri konteynerları için struct kullanın
  ...
  - Yalnızca UI ile ilgili mantığı işleyin
  - RxSwift kullanarak ViewModels'e bağlantı yapın
  ...
  ### ViewModel
  - Veri bağlantısı için RxSwift subject'lerini kullanın
  - İş mantığını ve veri dönüşümünü işleyin
  - View'nin abone olacağı observable'ları açığa çıkarın
  - Input/output pattern'i uygulayın
  ...
  ### Bağlantı Yönergeleri
  - Bellek sızıntılarını önlemek için her zaman `disposed(by: disposeBag)` kullanın
  - Retain cycle'lardan kaçınmak için closures'da `weak self` kullanın
  - UI bağlantısı için `bind(to:)` yerine `drive()` tercih edin
  ...
  - Kod organizasyonu için `MARK:` yorumlarını kullanın

  ### Bellek Yönetimi
  - Delegate'ler ve closure'lar için `weak` referans kullanın
  - RxSwift subscriptions'ın uygun şekilde dispose edilmesini uygulayın
  - `unowned` yalnızca referans nil olmayacaksa emin olduğunuzda kullanın

  ### UI Yapılandırması
  - Yeniden kullanılabilir UI bileşenleri oluşturun
  - UI kurulumu için extension'ları kullanın
  ...
  ## Test Yönergeleri
  - ViewModels için unit test yazın
  - Reaktif stream'ler için RxTest kullanın
  ...
  - **Kingfisher**: Resim yükleme ve caching
  - **SnapKit**: Auto Layout (isteğe bağlı)

  SOLID prensiplerini her zaman takip etmeyi, kodunuzu test edilebilir tutmayı ve Model, View ve ViewModel katmanları arasında kaygıların ayrılığını korumayı unutmayın.

  # .cursorrules-mvvm-rxswift_2

  # iOS Swift MVVM + RxSwift Genel Kuralları

  MVVM mimarisi ve RxSwift ile iOS Swift geliştirmede uzman geliştiricisiniz. Apple'ın en son yönergeleri ve Swift en iyi uygulamalarını takip ederek temiz, bakımlanabilir ve test edilebilir kod yazın.
  ...
  ### 1. Model Katmanı
  Modelleri basit ve veri temsili üzerine odaklanmış tutun.
  ...
  ### 2. ViewModel Pattern
  Kaygıların net ayrılması için Input/Output pattern'ini kullanın.
  ...
  View controller'ları UI bağlantısı ve kullanıcı etkileşimine odaklanmış tutun.
  ...
  Veri kaynaklarını soyutlayın ve reaktif interface'ler sağlayın.
  ...
  ### 1. Bellek Yönetimi
  ```swift
  // Her zaman subscriptions'ları dispose edin
  .disposed(by: disposeBag)

  // Closures'da weak self kullanın
  .subscribe(onNext: { [weak self] value in
      self?.handleValue(value)
  })

  // Referans var olacaksa unowned kullanın
  ...
  ### 2. UI Bağlantısı
  ```swift
  // UI bağlantısı için Driver kullanın (main thread, error yok)
  ...
  ### Dosya Organizasyonu
  - Türe değil, özelliğe göre gruplandırın
  - Anlamlı klasör adları kullanın
  - İlgili dosyaları beraber tutun

  ### Mimari Kuralları
  - ViewModels UIKit import etmemelidir
  - Views iş mantığı içermemelidir
  - Test edilebilirlik için dependency injection kullanın
  - Ağ ve yerel veri kaynaklarını ayrı tutun

  ### RxSwift Desenleri
  - ViewModels için Input/Output pattern'ini kullanın
  - UI bağlantısı için Driver/Signal tercih edin
  - Her zaman subscriptions'ları dispose edin
  - Yükleme durumları için ActivityIndicator kullanın
  - Uygun hata işleme uygulayın

  Unutmayın: Basit, test edilebilir ve bakımlanabilir tutun. Reaktif stream'ler ve net kaygı ayrılığına odaklanın.
---
