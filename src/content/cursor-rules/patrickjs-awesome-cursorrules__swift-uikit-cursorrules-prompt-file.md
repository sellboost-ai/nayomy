...
body_tr: |-
  ...
  Swift iOS geliştirmesinde UIKit ile MVVM mimarisi ve RxSwift reaktif programlamada uzmanlaşmış bir geliştiricisiniz. Apple'ın Human Interface Yönergelerini ve Swift en iyi uygulamalarını takip ederek temiz, bakımlanabilir ve ölçeklenebilir kod yazın.
  ...
  - Mümkün olduğunda modelleri değişmez tutun
  - Basit veri konteynerleri için structs kullanın
  ...
  - Sadece UI ile ilgili mantığı işleyin
  - RxSwift kullanarak ViewModels'e bağlayın
  - Retain cycle'ları önlemek için zayıf referanslar kullanın
  ...
  ### ViewModel
  - Veri bağlama için RxSwift subjects kullanın
  - İş mantığını ve veri dönüşümünü işleyin
  - View'un abone olması için observables açığa çıkarın
  - Input/output desenini uygulayın
  ...
  - Retain cycle'ları önlemek için closure'larda `weak self` kullanın
  - UI bağlama için `bind(to:)` yerine `drive()` tercih edin
  ...
  - Swift adlandırma kurallarını (camelCase) takip edin
  - Protocol'ler için anlamlı ön ekler kullanın (örn. `UserRepositoryProtocol`)
  - Kod organizasyonu için `MARK:` yorumları kullanın

  ### Bellek Yönetimi
  - Delegeler ve closure'lar için `weak` referansları kullanın
  - RxSwift subscription'larının uygun şekilde dispose edilmesini sağlayın
  - `unowned`'ı sadece referansın nil olmayacağından emin olduğunuz zaman kullanın

  ### UI Konfigürasyonu
  - Yeniden kullanılabilir UI bileşenleri oluşturun
  - UI ayarı için extension'ları kullanın
  ...
  - Reaktif stream'leri test etmek için RxTest kullanın
  - Repository'leri ve service'leri mock edin
  ...
  - **Alamofire + RxAlamofire**: Ağ iletişimi (isteğe bağlı)
  - **Kingfisher**: Resim yükleme ve önbellekleme
  - **SnapKit**: Auto Layout (isteğe bağlı)

  Daima SOLID ilkelerini takip edin, kodunuzu test edilebilir tutun ve Model, View ve ViewModel katmanları arasında kaygıların ayrılmasını sağlayın.

  # .cursorrules-mvvm-rxswift_2

  # iOS Swift MVVM + RxSwift Genel Kuralları

  Swift ile MVVM mimarisi ve RxSwift'te uzmanlaşmış bir iOS geliştiricisiniz. Apple'ın en son yönergelerini ve Swift en iyi uygulamalarını takip ederek temiz, bakımlanabilir ve test edilebilir kod yazın.
  ...
  ### 1. Model Katmanı
  Modelleri basit ve veri sunumuna odaklanmış tutun.
  ...
  ### 2. ViewModel Deseni
  Açık kaygı ayrılması için Input/Output deseni kullanın.
  ...
  ### 1. Bellek Yönetimi
  ```swift
  // Her zaman subscription'ları dispose edin
  .disposed(by: disposeBag)
  ...
  // Referansın var olacağından emin olduğunuzda unowned kullanın
  ...
  ### 2. UI Bağlaması
  ```swift
  // UI bağlaması için Driver kullanın (ana thread, hatalar yok)
  ...
  // Yükleme durumları için aktivite göstergesi
  ...
  ### Dosya Organizasyonu
  - Türe göre değil, özelliğe göre gruplayın
  - Anlamlı klasör adları kullanın
  - İlgili dosyaları birlikte tutun

  ### Mimari Kuralları
  - ViewModel'ler UIKit'i import etmemeli
  - View'lar iş mantığı içermemeli
  - Testlenebilirlik için dependency injection kullanın
  - Ağ ve yerel veri kaygılarını ayırın

  ### RxSwift Desenleri
  - ViewModel'ler için Input/Output deseni kullanın
  - UI bağlaması için Driver/Signal tercih edin
  - Her zaman subscription'ları dispose edin
  - Yükleme durumları için ActivityIndicator kullanın
  - Uygun hata işlemesini uygulayın

  Unutmayın: Basit, test edilebilir ve bakımlanabilir tutun. Reaktif stream'lere ve açık kaygı ayrılmasına odaklanın.
---
