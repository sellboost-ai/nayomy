...
file_extension: ".mdc"
body_tr: |-
  React + TanStack Router + TanStack Query konusunda uzman olduğunuzu biliyorum, ve TanStack Router v1, TanStack Query v5, TypeScript, Vite kullanarak tamamen tip-güvenli tek sayfalı uygulamalar oluşturuyorsunuz.
  ...
  - React bileşenleri saf UI'dır — Query cache'inden okur ve mutation tetikler
  - Loader'lar Router ve Query'yi bağlar: render öncesinde Query cache'ine prefetch yapar
  - Bu, route seviyesi veri için yükleme göstericilerini ortadan kaldırır; Suspense bileşen seviyesi yüklemesi idare eder
  ...
  src/
    routes/
      ...
      posts/
        index.tsx
        $postId.tsx
    queries/            ← Query tanımları (queryOptions factory'leri)
      posts.ts
      users.ts
  ...
  ## Query Tanımları (queryOptions factory'leri)
  - Query key, fetcher ve staleTime'ı aynı yerde bir araya getirin
  - Router loader'ları ve component hook'ları arasında paylaşın
  ```ts
  ...
  - Loader'lar `queryClient.ensureQueryData` çağırır — cache'i doldurur, spinner olmadan hemen render eder
  - Bileşenler daha sonra aynı seçeneklerle `useQuery` çağırır — cache'den senkron olarak okur
  ```tsx
  ...
  function PostDetail() {
    ...
    // veri loader'dan cache'de zaten var — yükleme state'i yok
  ...
  - TanStack Router search param'larını filter/pagination state'inin gerçek kaynağı olarak kullanın
  - Search param'larını queryOptions'a geçin query key'i ve fetcher'ı yönlendirmek için
  ```tsx
  ...
  function CreatePostForm() {
    ...
    const mutation = useMutation({
      ...
      onSuccess: (newPost) => {
        // Detay cache'ini hemen doldur
        queryClient.setQueryData(postKeys.detail(newPost.id), newPost)
        // Liste query'lerini geçersiz kıl
        queryClient.invalidateQueries({ queryKey: postKeys.lists() })
        // Yeni post'a git (yükleme yok — cache ısınmış)
  ...
  // src/routes/_auth.tsx (korumalı rotalar için path'siz layout)
  ...
  ## DevTools (Yalnızca Geliştirme Sırasında)
  ```tsx
  // __root.tsx içinde
  ...
  ## Önemli Kurallar
  - Her zaman `queryOptions`'ı bileşenlerin dışında tanımlayın — `useQuery()` içine inline yerleştirmeyin
  - Veri getirmek için hiçbir zaman `useEffect` kullanmayın — loader'ları veya `useQuery`'yi kullanın
  - Her zaman router context'ini tip olarak belirtin — `declare module '@tanstack/react-router'` kaydı gereklidir
  - Search param'ları URL tarafından yönlendirilen filter state'inin tek gerçek kaynağı olarak kullanın
  - Mutation'lar sadece invalidate etmeyin, `setQueryData` + `invalidateQueries` yapmalı, anında UI geri bildirimi için
---
