...
stars: 40010
path: "rules/beefreeSDK-nocode-content-editor-cursorrules-prompt-file.mdc"
...
body_tr: |-
  ...
  [Beefree SDK](https://docs.beefree.io/beefree-sdk) ile uygulama geliştirmek için yönergeler ve en iyi uygulamalar; kurulum, kimlik doğrulama, yapılandırma, özelleştirme ve şablon yönetimini içerir.

  ## Kurulum Yönergeleri

  ### Paket Kurulumu
  - Beefree SDK paketini npm veya yarn kullanarak yükleyin:
    ```bash
  ...
  ### Ortam Kurulumu
  - Beefree kimlik bilgilerinizi içeren `.env` dosyasını proje köküne oluşturun:
    ```env
  ...
  - Kimlik bilgilerinizi korumak için DAIMA bir proxy sunucu kullanın
  - Kimlik doğrulamayı işlemek için bir proxy sunucu dosyası oluşturun (ör. `proxy-server.js`):
    ```javascript
  ...
  - TÜMLÜGÜ API yanıtını Beefree SDK'ya geçirin, sadece token'ı değil
  ...
  ### HTML Konteyneri
  - Beefree SDK için özel bir konteyner öğesi oluşturun:
    ```html
    <div id="beefree-sdk-container"></div>
    ```

  ### CSS Şekillendirmesi
  - Konteynerı uygun şekilde gösterilmesini sağlamak için şekillendirin:
    ```css
  ...
  ### React Konteyneri
  - React uygulamaları için, aşağıdaki kod parçacığı ref'leri kullanarak konteyner yönetiminin bir örneğini gösterir:
    ```typescript
  ...
  - DAIMA yapılandırmanıza `container` parametresini dahil edin:
    ```typescript
    const beeConfig = {
  ...
  ## SDK İlklendirilmesi Yönergeleri

  ### Temel İlklendirilme
  - Beefree SDK'yı uygun hata işleme ile ilklendirin:
    ```typescript
  ...
  ### React Entegrasyonu
  - React uygulamaları için, aşağıdaki kod parçacığı ilklendirilme için useEffect kullanmanın bir örneğini gösterir:
    ```typescript
    useEffect(() => {
  ...
  ### Şablonları Yükleme
  - Mevcut şablonları yüklemek için `start()` metodunu şablon verileriyle kullanın:
    ```typescript
    // localStorage'dan şablon yükleyin
  ...
  ## HTML İthalatı Yönergeleri

  ### HTML İthalatçı API'si
  - Mevcut HTML şablonlarını Beefree SDK formatına dönüştürmek için HTML İthalatçı API'sini kullanın
  - API uç noktası: `https://api.getbee.io/v1/conversion/html-to-json`
  - Referans: [HTML İthalatçı API Belgeleri](https://docs.beefree.io/beefree-sdk/apis/html-importer-api/import-html)
  ...
  ### İthalatı Yapılan Şablonları Yükleme
  - İthalatı yapılan şablonları Beefree SDK'ya yükleyin:
    ```typescript
  ...
  ### onError Callback'i
  - SDK hatalarını işlemek için DAIMA `onError` callback'ini uygulayın:
    ```typescript
    onError: function (errorMessage) {
      console.error("Beefree SDK error:", errorMessage);
      // Kullanıcı dostu hata mesajı görüntüleyin
  ...
  - Kimlik doğrulama başarısızlıklarını duyarlı bir şekilde işleyin:
    ```typescript
  ...
  ## Şablon Değişikliği İzleme Yönergeleri

  ### Mesaj Değişikliklerini İzleme
  - Son kullanıcılar tarafından yapılan değişiklikleri izlemek için şablon değişikliği izlemesini uygulayın
  ...
  Beefree SDK görünümünü aşağıdakilerle özelleştirin:
  ...
  ### Merge Etiketleri ve Özel Bağlantılar
  - E-posta kişiselleştirmesi için merge etiketlerini ve özel bağlantıları yapılandırın:
    ```typescript
    const beeConfig = {
      container: 'beefree-sdk-container',
      mergeTags: [
  ...
  Olası özelleştirmelerin kapsamlı bir referansı için resmi [Beefree SDK teknik belgelerine](https://docs.beefree.io/beefree-sdk) bakın.  

  ## En İyi Uygulamalar

  ### Performans Optimizasyonu
  - Beefree SDK'yı yalnızca uygulamanızda gerçekten ihtiyaç duyulduğunda ilklendirin.
  - SDK kaynakları artık gerekli olmadığında düzgün bir şekilde temizleyin (ör. gezinme veya editörü kapatırken).
  - Uygulama çökmelerini veya beklenmedik davranışları önlemek için hataları duyarlı bir şekilde işleyin.

  ### Güvenlik
  - Beefree SDK istemci kimlik bilgilerinizi **asla** ön uç veya genel kodda açıklamayın.
  - Kimlik doğrulama ve hassas işlemleri işlemek için her zaman güvenli bir arka uç veya proxy sunucusu kullanın.
  - Güvenlik açıklarını önlemek için SDK'ya geçirmeden önce tüm kullanıcı girdilerini doğrulayın ve temizleyin.

  ### Kullanıcı Deneyimi
  - SDK ilklendirilirken veya işlemler gerçekleştirirken uygun yükleme göstergelerini gösterin.
  - Bir sorun ortaya çıkarsa, kullanıcılara net ve yararlı hata mesajları görüntüleyin.
  - Veri kaybını önlemek için otomatik kaydetme veya ilerleme izlemesini uygulayın.

  ### Kod Organizasyonu
  - Daha iyi bakım için SDK yapılandırmasını ilklendirilme ve iş mantığından ayrı tutun.
  - Kod güvenliğini ve netliğini iyileştirmek için mümkün olduğunda güçlü yazı (ör. TypeScript veya benzeri) kullanın.
  - Kullandığınız teknoloji yığını veya çerçeve ne olursa olsun, tüm entegrasyonda sağlam hata işlemesini sağlayın.

  ## Örnekler

  ### Tam React Bileşeni
  Tam proje için [beefree-react-demo](https://github.com/BeefreeSDK/beefree-react-demo) adresine bakın.
  ```typescript
  ...
  ### Tam HTML Uygulaması
  Tam proje için Beefree SDK [multiple-versions-concept](https://github.com/BeefreeSDK/beefree-sdk-simple-schema/tree/main/multiple-versions-concept) adresine bakın.
  ```html
  <!DOCTYPE html>
  <html lang="en">
    <head>
