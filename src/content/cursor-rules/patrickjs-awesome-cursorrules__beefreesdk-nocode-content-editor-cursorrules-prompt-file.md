...
description_tr: "Beefree SDK'nin no-code içerik editörlerini (e-postalar, sayfalar ve popup'lar için) web uygulamanıza entegre etmek için Cursor kuralları."
category: "Other"
...
body_tr: |-
  ...
  [Beefree SDK](https://docs.beefree.io/beefree-sdk) ile uygulama geliştirmeye yönelik yönergeler ve en iyi uygulamalar; kurulum, kimlik doğrulama, yapılandırma, özelleştirme ve şablon yönetimi dahil.
  ...
  ### Bağımlılıklar
  - Beefree SDK aşağıdaki çekirdek bağımlılıkları gerektirir:
    ```json
    {
      "dependencies": {
  ...
  ### Ortam Kurulumu
  - Proje kökünüzde Beefree kimlik bilgileriniz ile bir `.env` dosyası oluşturun:
    ```env
  ...
  - Kimlik bilgilerinizi korumak için kimlik doğrulama için HER ZAMAN bir proxy sunucu kullanın
  - Kimlik doğrulamayı işlemek için bir proxy sunucu dosyası (örneğin, `proxy-server.js`) oluşturun:
    ```javascript
  ...
  - Tüm API yanıtını Beefree SDK'ya geçirin, sadece token'ı değil
  - Kimlik doğrulama çağrısı örneği:
    ```typescript
  ...
  ### CSS Stili
  - Doğru görüntülenmeyi sağlamak için konteyneri stillendirin:
    ```css
  ...
  ### React Konteyner
  - React uygulamaları için, aşağıdaki kod parçası konteyneri yönetmek üzere ref'ler kullanarak bir örnek gösterir:
    ```typescript
  ...
  - Yapılandırmanıza `container` parametresini HER ZAMAN ekleyin:
    ```typescript
    const beeConfig = {
  ...
  - Doğru işlevsellik için gerekli callback fonksiyonlarını uygulayın:
    ```typescript
    const beeConfig = {
  ...
  ### Şablonları Yükleme
  - Mevcut şablonları yüklemek için `start()` metodunu şablon verileri ile kullanın:
    ```typescript
  ...
  ### İthal Edilen Şablonları Yükleme
  - İthal edilen şablonları Beefree SDK'ya yükleyin:
    ```typescript
  ...
  ### onError Callback'i
  - SDK hatalarını işlemek için `onError` callback'ini HER ZAMAN uygulayın:
    ```typescript
    onError: function (errorMessage) {
      console.error("Beefree SDK error:", errorMessage);
  ...
  - Kimlik doğrulama hatalarını zarif bir şekilde işleyin:
    ```typescript
  ...
  ### UI Özelleştirme
  Beefree SDK görünümünü şunlar ile özelleştirin:
  ...
  Olası özelleştirmelerin kapsamlı bir referansı için resmi [Beefree SDK teknik belgelerini](https://docs.beefree.io/beefree-sdk) referans alın.

  ## En İyi Uygulamalar

  ### Performans Optimizasyonu
  - Beefree SDK'yı yalnızca uygulamanızda gerçekten gerekli olduğunda başlatın.
  - SDK kaynakları artık gerekli olmadığında düzgün bir şekilde temizleyin (örneğin, başka bir sayfaya giderken veya editörü kapatırken).
  - Uygulama çökmelerini veya beklenmeyen davranışları önlemek için hataları zarif bir şekilde işleyin.

  ### Güvenlik
  - Beefree SDK istemci kimlik bilgilerinizi **ASLA** hiçbir frontend veya genel kodda ortaya koymayın.
  - Kimlik doğrulama ve hassas işlemleri işlemek için her zaman güvenli bir arka uç veya proxy sunucu kullanın.
  ...
  - Bir şeyler yanlış giderse kullanıcılara açık ve yardımcı hata mesajları gösterin.
  - Veri kaybını önlemek için otomatik kaydetme veya ilerleme izlemeyi uygulayın.

  ### Kod Organizasyonu
  - SDK yapılandırmasını başlatma ve iş mantığından ayrı tutun, daha iyi bakım için.
  - Kod güvenliğini ve netliğini iyileştirmek için mümkün olan yerlerde güçlü yazımı (örneğin, TypeScript) kullanın.
  - Kullanılan teknoloji yığını veya çerçevesi ne olursa olsun, entegrasyonun tamamında sağlam hata işleme sağlayın.

  ## Örnekler

  ### Tam React Bileşeni
  Tam projeye [beefree-react-demo](https://github.com/BeefreeSDK/beefree-react-demo) adresinden referans verin.
  ```typescript
  ...
  ### Tam HTML Uygulaması
  Tam projeye Beefree SDK [multiple-versions-concept](https://github.com/BeefreeSDK/beefree-sdk-simple-schema/tree/main/multiple-versions-concept) adresinden referans verin.
  ```html
  <!DOCTYPE html>
  <html lang="en">
    <head>
