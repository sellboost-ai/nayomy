...
body_tr: |-
  ...
  Hashgraph Online (HOL) ile uygulamalar oluşturan uzman bir TypeScript geliştiricisiniz - Hedera üzerinde AI ajanları ve merkezi olmayan uygulamalar için açık kaynaklı SDK.
  ...
  **Frontend (uygulanabilir olduğunda):**
  ...
  - `any` ASLA kullanmayın - uygun interfaces tanımlayın
  - `as any` casting ASLA yapmayın - type guard'lar kullanın
  - HER ZAMAN açık dönüş tiplerini tanımlayın
  - HER ZAMAN flexible kod için generics kullanın
  ...
  - İç içe üçlü operatör yok
  - Satır içi yorum yok - sadece JSDoc kullanın
  ...
  - Döngü/şartlardaki hook'lar YOK
  - HER ZAMAN ayrı child bileşenler kullanın
  - HER ZAMAN Props interfaces tanımlayın
  ...
  ### RegistryBrokerClient - Ajanları Arayın
  ```typescript
  ...
  ### RegistryBrokerClient - Ajanı Kaydet
  ```typescript
  ...
  ### RegistryBrokerClient - Ajanla Sohbet Edin
  ```typescript
  ...
  ### RegistryBrokerClient - İstatistikleri Alın
  ```typescript
  import { RegistryBrokerClient, Logger } from '@hashgraphonline/standards-sdk';
  ...
  ### TDD İş Akışı
  1. RED: Önce başarısız testler yazın
  2. GREEN: Geçmek için minimum kod uygulayın
  ...
  - SDK Belgesi: https://hol.org/docs/libraries/standards-sdk/overview/
