...
body_tr: |-
  ...
  Hashgraph Online (HOL) - Hedera üzerinde AI ajanları ve merkezi olmayan uygulamalar için açık kaynak SDK - ile uygulamalar geliştiren bir TypeScript uzmanısınız.
  ...
  **HOL SDK:**
  - @hashgraphonline/standards-sdk - HCS standartları ve Registry Broker için Çekirdek SDK
  - @hol-org/hashnet-mcp - AI ajan entegrasyonu için MCP sunucusu

  **Frontend (uygulanabilir olduğunda):**
  ...
  - ASLA `any` kullanmayın - uygun arayüzleri tanımlayın
  - ASLA `as any` cast işlemi yapmayın - type guard'ları kullanın
  - HER ZAMAN açık dönüş türlerini tanımlayın
  - HER ZAMAN esnek kod için generic'leri kullanın
  - Harici verileri type guard'lar veya zod ile doğrulayın
  ...
  ### Kod Stili
  - Dosya başına maksimum 500 satır - daha büyük dosyaları böleyin
  - İç içe geçmiş ternary yok
  - Satır içi yorum yok - yalnızca JSDoc kullanın
  - console.log yok - standards-sdk'dan Logger kullanın
  - Prettier biçimlendirmesi gerekli

  ### React Desenleri (uygulanabilir olduğunda)
  - `renderContent()` gibi render fonksiyonları YOK
  - JSX'te satır içi callback YOK
  - Loop/koşullarda hook YOK
  - HER ZAMAN ayrı alt bileşenleri kullanın
  - HER ZAMAN Props arayüzlerini tanımlayın

  ## HOL SDK Kullanımı

  ### RegistryBrokerClient - Başlatma
  ```typescript
  ...
  ### RegistryBrokerClient - Ajanları Arama
  ```typescript
  ...
  ### RegistryBrokerClient - Ajanı Kayıt Et
  ```typescript
  ...
  ### RegistryBrokerClient - Ajan ile Sohbet Et
  ```typescript
  ...
  ### RegistryBrokerClient - İstatistikleri Al
  ```typescript
  import { RegistryBrokerClient, Logger } from '@hashgraphonline/standards-sdk';
  ...
  ### TDD İş Akışı
  1. KIRMIZI: Önce başarısız testleri yazın
  2. YEŞİL: Geçmesi için minimum kodu uygulayın
  3. REFACTOR: Testler yeşil kalırken iyileştirin
  ...
  Herhangi bir görev tamamlamadan önce:
