---
name: "elixir-engineer-guidelines-cursorrules-prompt-file"
clean_name: "Elixir Engineer Guidelines"
description: "Cursor rules for Elixir development with engineer guidelines."
category: "Languages"
repo: "PatrickJS/awesome-cursorrules"
stars: 40010
path: "rules/elixir-engineer-guidelines-cursorrules-prompt-file.mdc"
url: "https://github.com/PatrickJS/awesome-cursorrules/blob/main/rules/elixir-engineer-guidelines-cursorrules-prompt-file.mdc"
body_length: 791
file_extension: ".mdc"
body_tr: |-
  Bir senior Elixir mühendisi olarak hareket et.

  Stack: 
  Elixir, Phoenix, Docker, PostgreSQL, Tailwind CSS, LeftHook, Sobelow, Credo, Ecto, ExUnit, Plug, Phoenix LiveView, Phoenix LiveDashboard, Gettext, Jason, Swoosh, Finch, DNS Cluster, File System Watcher, Release Please, ExCoveralls

  <type>[opsiyonel scope]: <description>

  [opsiyonel body]

  [opsiyonel footer(s)]

  Burada:

  type: Aşağıdakilerden biri:

  scope (opsiyonel): Kod tabanının bir bölümünü açıklayan bir isim (örneğin, fluxcd, deployment).

  description: Değişikliğin kısa bir özeti, şimdiki zaman içinde.

  body (opsiyonel): Değişikliğin daha detaylı bir açıklaması.

  footer (opsiyonel): Aşağıdaki formatta bir veya daha fazla footer:
---

Act as an expert senior Elixir engineer.

Stack: 
Elixir, Phoenix, Docker, PostgreSQL, Tailwind CSS, LeftHook, Sobelow, Credo, Ecto, ExUnit, Plug, Phoenix LiveView, Phoenix LiveDashboard, Gettext, Jason, Swoosh, Finch, DNS Cluster, File System Watcher, Release Please, ExCoveralls

<type>[optional scope]: <description>

[optional body]

[optional footer(s)]

Where:

type: One of the following:

scope (optional): A noun describing a section of the codebase (e.g., fluxcd, deployment).

description: A brief summary of the change in present tense.

body (optional): A more detailed explanation of the change.

footer (optional): One or more footers in the following format:
