...
body_tr: |-
  ...
  # Bu kural tüm dosya işlemleri için geçerlidir ve tutarlı şekilde uygulanmalıdır.

  Stojanovic-One web uygulaması projesi için bir yapay zeka asistanısınız. Bu yönergelere uyun:

  Lütfen bu çok önemli - düzenledığiniz, oluşturduğunuz veya sildiğiniz her dosya için tam dosya yolları sağlayın.
  Her zaman şu formatta sağlayın: bu dosyayı şimdi düzenle: E:\Stojanovic-One\src\routes\Home.svelte veya bu dosyayı bu yolda oluştur: E:\Stojanovic-One\src\routes\Home.svelte
  Ayrıca, bir dosyayı güncelleyelim veya oluşturalım dediğinizde her zaman @AI.MD'de belirtildiği gibi yolları sağlayın.

  1. Teknoloji Yığını:
    - Frontend & Backend: SvelteKit
    - Veritabanı: PostgreSQL (Supabase aracılığıyla)
    - UI Stillemesi: Tailwind CSS
    - Dağıtım: Vercel
    - Kimlik Doğrulama: Supabase Auth

  2. Elon Musk'ın Verimlilik Algoritmasını Takip Edin:
    a. Her gereksinimi kritik şekilde sorguayın
    b. Gereksiz parçaları silin
    c. Kalan bileşenleri basitleştirin ve optimize edin
    d. Döngü zamanını hızlandırın
    e. Son adım olarak otomasyonu uygulayın

  3. Test-Driven Development (TDD) Uygulayın:
    - Önce başarısız testler yazın
    - Testleri geçmek için minimum kod uygulayın
    - Testler geçerli durumda kalacak şekilde yeniden düzenleyin

  4. Dosya Yönetimi:
    - Her dosyanın başında tam dosya yolunu yorum olarak ekleyin
    - Yeni dosya/dizin eklerken proje yapısını AI.MD'de güncelleyin
    - package.json'u güncel tutun

  5. Test Etme:
    - Unit ve integration testler için Vitest kullanın
    - Yüksek test kapsamını hedefleyin (%80 veya daha fazla)
  ...
  7. Dokümantasyon:
    - Net açıklamalar yazın ve uygun yerlerde JSDoc kullanın
  ...
  8. Doğruluk ve Netlik:
    ...
    - Bir şey bilmediğinizi itiraf edin
    - Netliği sağlarken kısa olun

  9. Geliştirme İş Akışı:
    - Gereksinimleri sorguayın ve iyileştirin
    - Görevleri küçük, yönetilebilir sorunlara bölün
    - Her görev için:
     a. Başarısız testler yazın
     b. Testleri geçmek için minimum kod uygulayın
     c. Yeniden düzenleyin ve optimize edin
    - Merge önerisinden önce self-review yapın
    - Değişiklikleri sonuçlandırmadan önce CI'nin geçtiğinden emin olun

  10. En İyi Uygulamalar:
    - Uygun olduğunda RESTful API tasarım ilkelerine uyun
    - Bileşenleri için responsive tasarım uygulayın
    - Veri doğrulaması için Zod kullanın
    - Bağımlılıkları düzenli olarak güncelleyin ve güvenlik açıkları için kontrol edin

  11. Sürekli İyileştirme:
    - Uygulanabilir olduğunda süreç iyileştirmeleri önerin
    - Kod ve iş akışlarını basitleştirme ve optimize etme fırsatları arayın

  12. Windows Uyumluluğu:
    - Windows kullanıcıları için PowerShell komutları sağlayın
    - Unix'e özgü komutlardan kaçının (`rm` yerine `Remove-Item` kullanın)
    - Mümkün olduğunda platformlar arası Node.js komutlarını kullanın

  Her zaman detaylı proje özgü yönergeler ve güncel uygulamalar için AI.MD'ye başvurun. Geliştirme süreci boyunca Elon Musk'ın verimlilik ilkelerini sürekli uygulayın.

  13. Tasarım ve Kullanıcı Deneyimi:
    - Dark mod uyumluluğunu uygulayın
    - Mobil dostu ve responsive tasarım sağlayın
    - Performans açısından optimize edin
    - Modern ve güzel bir kullanıcı arayüzü oluşturun
