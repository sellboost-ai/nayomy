...
file_extension: ".mdc"
body_tr: |-
  Siz bir R programlama asistanısınız, R'de programlama yaparken en iyi uygulamaları kullandığınızdan emin olun:

  ## Proje Yapısı ve Dosya Organizasyonu
  - Projeleri açık dizinlere organize edin: 'R/' (scriptler), 'data/' (ham ve işlenmiş), 'output/' (sonuçlar, grafikler), 'docs/' (raporlar). R paketleri için 'inst/' dosyalarını harici dosyalar için kullanın; paket olmayan projeler için 'assets/' düşünün.
  - Her proje için çalışma dizinlerini ve ayarları yönetmek üzere bir 'Rproj' dosyası kullanın.
  - Yeniden kullanılabilir fonksiyonlar oluşturun ve bunları 'R/' klasörü altında ayrı script dosyalarında tutun.
  - Kodu ve sonuçları birleştiren tekrarlanabilir raporlar için RMarkdown veya Quarto kullanın. Quarto mevcutsa ve kuruluysa tercih edin.
  - Ham verileri değişmez tutun; yalnızca 'data/processed/' içinde işlenmiş verilerle çalışın.
  - Bağımlılık yönetimi ve tekrarlanabilirlik için 'renv' kullanın. Tüm bağımlılıklar kurulu, senkronize ve kilitli olmalıdır.
  - Tüm projeleri Git ile sürüm kontrol altına alın ve net commit mesajları kullanın.
  - Dosya adları için snake_case tutarlı adlandırması yapın. Dosya adları çok uzun olmamalıdır.
  - Gereksiz bağımlılıkları kullanmaktan kaçının. Bir görev base R kullanılarak nispeten kolay bir şekilde gerçekleştirilebiliyorsa base R kullanın ve diğer paketleri yalnızca gerekli olduğunda içeri aktarın (örn., ölçülebilir olarak daha hızlı, daha sağlam veya daha az kod satırı).

  ## Paket Yapısı
  - R projesi bir R paketi ise, paket içinde kullanılan bağımlılıkları 'DESCRIPTION' dosyasında belirtin. Tüm bağımlılıkların sürüm numarası belirtilmelidir (örn: R6 (>= 2.6.1))
  - R projesi bir R paketi ise, 'LICENSE' dosyasının bulunduğundan emin olun.
  - R projesi bir R paketi ise, paketin geliştirme değişikliklerini izlemesi gereken 'NEWS.md' dosyasının bulunduğundan emin olun.
  - R projesi bir R paketi ise, paket içinde kullanılan her harici dosya 'inst' klasörü içinde kaydedilmelidir. Dosya okuma işlemi 'system.file' fonksiyonu kullanılarak yapılmalıdır.
  - R projesi bir R paketi ise, yeni fonksiyonları test etmeden önce her zaman 'devtools::load_all' kullanın.
  - R projesi bir R paketi ise, paketin sorun olmadığından emin olmak için 'devtools::check()' çalıştırın. Notlar tamam; uyarılardan ve hatalardan kaçının.
  - R projesi bir R paketi ise, roxygen2 kullanarak fonksiyonları belgelendirin. Gerekli belgelendirmeyi (.Rd dosyaları) ve 'NAMESPACE' dosyasını oluşturmak için 'devtools::document()' kullanın.
  ...
  - Fonksiyon adları için fiil kullanın (örn., `plot_data`, `calculate_mean`).
  - R tarafından zaten atanmış olan fonksiyon veya değişken adlarından kaçının, örneğin 'sd' kullanmaktan kaçının, zaten R'de bir fonksiyondur. Başka bir örnek 'data' olabilir.
  - R6 sınıflarıyla çalışırken, her zaman private metodlara ve alanlara bir '.' ön eki ekleyin. Bir metod örneği '.get_data()' olur ve 'private$.get_data()' olarak kullanılır.

  ## Kodlama Stili
  - [tidyverse stil rehberini](https://style.tidyverse.org/) takip edin.
  - Operatörlerin etrafına boşluk koyun (`a + b`, `a+b` değil).
  - Okunabilirlik için satır uzunluğunu <= 80 karakterle tutun.
  - Tutarlı girintileme kullanın (2 boşluk tercih edilir).
  - Satır içi yorumlar ve bölüm başlıkları için '#' kullanın. Yalnızca gerekli olduğunda yorum yazın (örn., açıklama gereken karmaşık kod). Kod kendini açıklamalıdır.
  ...
  - Daha sonra doldurulmak üzere boş bir nesne oluştururken, mümkün olduğunda türü ve uzunluğu önceden ayırın (örn., 'x <- c()' yerine 'x <- character(length = 100)').
  - Değişken ataması için her zaman `<-` kullanın, 'R6' sınıflarıyla çalışırken hariç. 'R6' sınıfları içindeki metodlar '=' kullanılarak atanır.
  - Bir paketteki fonksiyona referans verirken her zaman '::' sözdizimini kullanın, örneğin 'dplyr::select'
  ...
  - Açık döngüler yerine vektörleştirilmiş fonksiyonları ve apply ailesi ('apply', 'lapply', 'sapply', 'vapply', 'mapply', 'tapply') veya 'purrr' tercih edin. Döngü kullanırken, öncelikle türü ve belleği önceden ayırın.
  - Performans kritik olduğunda ve veriler belleğe sığdığında büyük veri setleri için 'data.table' kullanın.
  - CSV okurken, kod tabanına bağlı olarak 'data.table::fread' veya 'readr::read_csv' tercih edin. Kod tabanı tidyverse odaklıysa 'readr' tercih edin; aksi takdirde 'data.table' kullanın.

  - Veriler bellek dışındayken 'duckdb' kullanın.
  - Büyük nesneleri gereksiz yere kopyalamaktan kaçının; mümkün olduğunda referansları kullanın.

  ## Test ve Doğrulama
  - `testthat` ile birim testleri yazın.
  - Tutarlı sonuçlar için tekrarlanabilir rasgele tohumlar (`set.seed()`) kullanın.
  - Fonksiyonları uç durumlar (boş girişler, eksik değerler, aykırı değerler) ile test edin.
  - Paket geliştirmesi için R CMD check veya `devtools::check()` kullanın.

  ## Tekrarlanabilirlik
  - Kodu ve sonuçları birleştiren tekrarlanabilir raporlar için RMarkdown veya Quarto kullanın. Zaten mevcutsa ve kuruluysa 'Quarto' tercih edin.
  - `sessionInfo()` veya `sessioninfo::session_info()` ile oturum bilgisini yakala.
  - Paket sürümlerini `renv` ile sabitle.
  - Scriptleri, verileri ve sonuçları sürüm kontrolünde saklayın.
  ...
  - Proje hedefleri, kurulum talimatları ve kullanım hakkında açık bir README tutun.
  ...
  - Açıklık için kodu yorumlayın, ancak kendi kendini açıklayan değişken ve fonksiyon adlarını tercih edin.
  ...
  - Her modül küçük sorumlulukları olmalıdır: UI, server (reaktif girişler/çıkışlar) ve birim test için yardımcı fonksiyonlar.
  - UI kodunu bildirimsel tutun ve veri işleme mantığından ayrı tutun.
  - Session kapsamındaki durum için `session$userData` veya oturum başına `reactiveValues` kullanın, global değişkenleri değil.
  - Statik varlıklar için 'www/' kullanın (JS/CSS/resimler), Shiny tarafından otomatik olarak sunulur.
  - 'UIOutput' ve 'renderUI' kullanmaktan kaçının, çünkü reaktivite mantığını daha karmaşık hale getirirler. Yalnızca gerekli olduğunda kullanın.

  ## İleri Uygulamalar
  - Karmaşık nesneler için S3/S4/S7 veya R6 sınıfları kullanın. Bağlama bağlı olarak seçin ama R6 tercih edin.
  - Projeler arasında yeniden kullanılabilir kod için özel paketler yazın.
  - Tekrarlanabilir boru hatları için iş akışlarını `targets` ile otomatikleştirin.
  ...
  - 'list' nesnelerinin manipülasyonu ve fonksiyonel programlama için purrr
  - web uygulaması geliştirmesi için shiny
  - bellek içinde veri işleme için 'data.table' veya 'dplyr'
  - verimli veri alma (CSV/TSV, vb.) için 'data.table' veya 'dplyr'
  - 'parquet' dosyaları ile çalışırken 'arrow'
  - bellek dışı veri setleriyle çalışırken 'duckdb'
  - çizim için 'ggplot2'
  - girişler assertion için 'checkmate'
  ...
  - R projesinde C++ kodu integre etme için 'Rcpp'
---
