...
body_tr: |-
  ...
  # Üretim için hazır FastAPI servisleri ilkeleri.
  ...
  İçe aktarmalar sadece aşağıya doğru akışkanlaşır. Her katmanın geçilmemesi gereken sert sınırları vardır.

  ### Router kuralları (app/routers/**)
  - Handler'lar İNCE olmalı: her handler için ≤10 satır çalıştırılabilir kod
  - İzin verilen içe aktarmalar: fastapi, app.schemas.*, app.core.deps, app.services.*
  - YASAK içe aktarmalar: sqlalchemy, httpx, boto3, app.models.*, app.repositories.*
  - Her endpoint response_model= ile OpenAPI sadakatini bildirir
  - Her korunan/iş endpoint'i user_id: str = Depends(get_current_user_id) gerektirir
  - Genel endpoint'ler (sağlık kontrolleri, webhook'lar, callback'ler) kimlik doğrulamadan muaftır
  - İş mantığı service'lerde yaşar. Router'lar girişi ayrıştırır, bir service metodunu çağırır, yanıtı döndürür.
  ...
  - YASAK içe aktarmalar: sqlalchemy, httpx, boto3, redis, FastAPI Request/Response/HTTPException
  - Constructor Protocol-tipli bağımlılıkları enjekte eder, somut sınıfları değil
  - Domain istisnalarını (InsufficientFundsError) yükselt, HTTPException'ı değil
  ...
  class WalletUserService:
      def __init__(self, db: Session): ...  # Yanlış — service altyapıya bağlı
  ```

  ### Repository kuralları (app/repositories/**)
  - SADECE sqlalchemy'i içe aktarmaya izin verilen katman
  - app/repositories/protocols.py'den Protocol'ü uygular
  - Domain nesneleri döndürür, ORM modellerini değil
  - Her sorgu user_id kapsamında (çok kiracılılık)

  ### Provider kuralları (app/providers/**)
  - SADECE httpx'i doğrudan içe aktarmaya izin verilen katman
  - GenerateResult | ProviderError döndürür — asla ham dict değil
  ...
  |----------|--------|--------------------------------------------------|
  | 0–399    | Yeşil  | Hiçbiri.                                        |
  | 400–599  | Sarı   | Bölme planla. TODO(decompose) başlığı ekle.     |
  | 600+     | Kırmızı| MERGE'ı ENGELLE. Önce ayrıştır.                 |

  Dosya aşağıdakilerden BİRİ doğruysa pakete dönüştür:
  - 400 LOC'yi geçer ve sonraki değişiklik 500'ü geçer
  - 2+ ayrık alt-alan içerir (görüntü vs video, kullanıcı vs admin)
  - HTTP handler'larını worker handler'larıyla karıştırır
  - Her biri sadece bir sembolü içe aktaran 2+ çağrıcısı var

  Güvenli bölme deseni (atomik PR):
  1. <file>/__init__.py oluştur (şimdilik boş)
  2. Parçaları alt-dosyalara taşı (a.py, b.py, c.py)
  3. Eski genel adları __init__.py'den yeniden dışa aktar
  4. Testleri çalıştır — değişiklik olmadan geçmeli
  5. Çağrıcıları eski takma addan uzaklaştır (takip eden PR)
  ...
  ## DIŞ ENTEGRASYON KURALLARI (İlkeler B1-B10)
  ...
  Her dış provider KENDİ httpx.AsyncClient'ine KENDİ Limits ile sahip. ASLA paylaşma.
  ...
  KÖTÜ:
  ```python
  HTTP = httpx.AsyncClient()  # tüm provider'lar arasında paylaşılan — bulkhead yalıtımı yok
  ```

  Kapatma temizliği — FastAPI lifespan'inde tüm provider client'larını kapat
  ```python
  ...
  ### Kural 3: İdempotency Keys
  Her yan-etki işlemi bir idempotency_key: UUID kabul eder. Yeniden denemeden önce ara.

  ### Kural 4: Yapılandırılmış Günlükleme contextvars ile
  Provider, user_id, request_id'yi async çağrı yığını boyunca bağlamak için ContextVar kullan.
  ...
  # Günlüklerde oku: provider_var.get(), user_id_var.get(), request_id_var.get()
  # JSON formatlayıcı bunu extra={} veya ContextVar.get() aracılığıyla otomatik olarak alır
  ```

  ### Kural 5: Tek-Yazar İlkesi (İlke B10)
  Güvenlik-kritik durumu için: tam olarak BİR service katmanı modülü yazma yapar.
  Belirlenmiş yazıcı service sadece repo.hold() çağırabilir.
  Router'lar ve provider'lar repo.hold() doğrudan çağıraMAYABİLİR.
  Admin servisleri service katmanında uygulanırsa
  bu alan için belirlenmiş yazıcıysalar repo.hold() çağırabilirler.
  Uygula: kod-inceleme grep kontrolü (`grep -r "repo\.hold(" --include="*.py"`)
  ve repo.hold() çağrı-menşeisini iddia eden birim testleri.

  ## ANTI-PATTERN'LER — GÖRÜR GÖRMEZ REDDET

  1. def some_method(self, db: Session, ...) → Protocol-tipli repo kullan
  2. from app.models.user import User bir service içinde → repo'dan domain türleri döndür
  3. httpx.AsyncClient() bir fonksiyon içinde oluşturulmuş → provider başına paylaşılan client kullan
  4. raise HTTPException(...) bir service içinde → domain istisnası yükselt
  5. db.query(...) bir router içinde → service'e sonra repo'ya taşı
  6. async def call(self, req) -> dict: return resp.json() → ACL ihlali
  7. HTTP = httpx.AsyncClient() paylaşılan → bulkhead ihlali
  8. result["vendor_field"]["nested"] service kodunda → ACL ihlali
  9. logger.info(f"{user_id} did X") → extra={} ile yapılandırılmış günlükleme kullan
  10. idempotency_key olmayan yan-etki işlemi → çift-ücretleme riski

  ## BAĞIMLILIK ENJEKSİYONU

  FastAPI Depends() + app/core/deps.py'de factory fonksiyonları kullan.
  dependency-injector, punq veya başka DI konteyneri yüklemeyin.
  ...
  ## DOMAIN İSTİSNA DESENI

  Service'ler domain hatalarını yükseltir. Router'lar HTTP'ye eşler.
