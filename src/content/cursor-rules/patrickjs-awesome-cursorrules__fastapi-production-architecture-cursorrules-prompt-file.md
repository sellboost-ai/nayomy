...
description_tr: "FastAPI hizmetleri için cursor rules; router/service/repository sınırları, typed provider adapter'lar, bulkhead isolation, idempotency ve domain exceptions desteği ile gelir."
category: "Backend"
...
file_extension: ".mdc"
body_tr: |-
  # FastAPI Üretim Mimarisi Kuralları
  # Üretim ortamına hazır FastAPI hizmetleri için ilkeler.

  ## KATMAN MİMARİSİ (İlkeler A1-A8)

  Bu kod tabanı katı 4 katmanlı mimariyi takip eder: Router → Service → Repository → ORM/HTTP/Storage.
  İçe aktarımlar sadece aşağıya doğru akar. Her katmanın geçilmesi yasak sınırları vardır.

  ### Router kuralları (app/routers/**)
  - Handler'lar İNCE olmalı: her handler başına ≤10 satır çalıştırılabilir kod
  - İzin verilen içe aktarımlar: fastapi, app.schemas.*, app.core.deps, app.services.*
  - YASAK içe aktarımlar: sqlalchemy, httpx, boto3, app.models.*, app.repositories.*
  - Her endpoint response_model= bildirimi gerektirir (OpenAPI doğruluğu için)
  - Her korumalı/iş endpoint'i user_id: str = Depends(get_current_user_id) gerektirir
  - Kamu endpoint'leri (sağlık kontrolleri, webhook'lar, callback'ler) kimlik doğrulamadan muaftır
  - İş mantığı service'lerde yaşar. Router'lar girdi ayrıştırır, bir service metodunu çağırır, yanıt döndürür.
  ...
  - YASAK içe aktarımlar: sqlalchemy, httpx, boto3, redis, FastAPI Request/Response/HTTPException
  - Constructor Protocol türünde bağımlılıkları injection'lar, somut sınıfları değil
  - Domain exception'ları (InsufficientFundsError) kaldırır, HTTPException değil
  ...
  class WalletUserService:
      def __init__(self, db: Session): ...  # Yanlış — service altyapıya bağımlı
  ```

  ### Repository kuralları (app/repositories/**)
  - SADECE sqlalchemy'yi içe aktarabilen katman
  - app/repositories/protocols.py'den Protocol'ü implements eder
  - Domain nesneleri döndürür, ORM modelleri değil
  - Her query user_id'ye göre kapsamlandırılmış (multi-tenancy)

  ### Provider kuralları (app/providers/**)
  - SADECE httpx'i doğrudan içe aktarabilen katman
  - GenerateResult | ProviderError döndürür — asla raw dict değil
  - Provider başına httpx.AsyncClient kullanır (bulkhead pattern)

  ## DOSYA BOYUTU KURALLARI (İlke A1)

  | LOC      | Durum  | İşlem                                        |
  |----------|--------|----------------------------------------------|
  | 0–399    | Yeşil  | Yok.                                         |
  | 400–599  | Sarı   | Bölünmeyi planla. TODO(decompose) başlığı ekle. |
  | 600+     | Kırmızı | Birleştirmeyi ENGELLE. Önce ayrıştır.        |

  Dosya pakete dönüştürülür, bunlardan BİRİ doğruysa:
  - 400 LOC'yi aşıyor ve sonraki değişiklik 500'ü geçiriyor
  - 2+ ayrık alt-domain içeriyor (resim vs video, kullanıcı vs admin)
  - HTTP handler'ları worker handler'larıyla karıştırıyor
  - 2+ çağrıcı varsa, her biri sadece bir sembol import ediyor

  Güvenli bölünme deseni (atomik PR):
  1. <file>/__init__.py oluştur (şimdilik boş)
  2. Parçaları alt-dosyalara taşı (a.py, b.py, c.py)
  3. Eski kamu adlarını __init__.py'den yeniden dışa aktar
  4. Testleri çalıştır — değişiklik olmadan geçmeli
  5. Sonraki PR'de çağrıcıları eski alias'ın dışında geçir

  __init__.py deseni:
  ```
  from .user import WalletUserService
  from .admin import WalletAdminService
  WalletService = WalletUserService  # geriye dönük uyumluluk alias'ı
  __all__ = ["WalletUserService", "WalletAdminService", "WalletService"]
  ```

  ## DIŞ İNTEGRASYON KURALLARI (İlkeler B1-B10)

  ### Kural 1: Anti-Corruption Katmanı (ACL)
  Provider'lar GenerateResult | ProviderError döndürür, asla dict değil.
  ...
  Her dış provider'ın KENDİ httpx.AsyncClient'ı ve KENDİ Limits'i vardır. HİÇBİR ZAMAN paylaşma.
  ...
  KÖTÜ:
  ```
  HTTP = httpx.AsyncClient()  # tüm provider'lar arasında paylaşılmış — bulkhead yalıtımı yok
  ```

  Kapatma temizliği — FastAPI lifespan'inde tüm provider client'larını kapatın
  ```
  from contextlib import asynccontextmanager

  @asynccontextmanager
  async def lifespan(app):
      yield  # uygulama başlangıcı
  ...
  Her yan etkili işlem bir idempotency_key: UUID kabul eder. Yeniden denemeden önce ara.

  ### Kural 4: Contextvars ile Yapılandırılmış Logging
  async çağrı yığınları boyunca provider, user_id, request_id'yi iletmek için ContextVar kullanın.
  ...
  # JSON formatter'ı bu değerleri otomatik olarak extra={} veya ContextVar.get() yoluyla alır
  ```

  ### Kural 5: Single-Writer İlkesi (İlke B10)
  Güvenlik açısından kritik durum için: TAM OLARAK BİR service katmanı modülü yazma işlemi yapar.
  Yalnızca domain için belirtilen yazar service'i repo.hold() çağırabilir.
  Router'lar ve provider'lar doğrudan repo.hold() çağırmamalıdır.
  Admin service'leri, domain için belirtilen yazar iseler service katmanında implement edilebilir
  ve yalnızca bu durumda repo.hold() çağırabilirler.
  Uygula: kod inceleme grep kontrolü (`grep -r "repo\.hold(" --include="*.py"`)
  ve repo.hold() çağrısının kaynağını ileri süren unit test'ler.

  ## ANTİ-PATTERN'LER — GÖRÜNCE RED

  1. `def some_method(self, db: Session, ...)` → Protocol türü repo kullan
  2. `from app.models.user import User` service içinde → repo'dan domain türleri döndür
  3. `httpx.AsyncClient()` fonksiyon içinde instantiate edilmiş → provider başına paylaşılan client kullan
  4. `raise HTTPException(...)` service içinde → domain exception'ı kaldır
  5. `db.query(...)` router içinde → service'e taşı sonra repo'ya
  6. `async def call(self, req) -> dict: return resp.json()` → ACL ihlali
  7. `HTTP = httpx.AsyncClient()` paylaşılmış → bulkhead ihlali
  8. `result["vendor_field"]["nested"]` service kodunda → ACL ihlali
  9. `logger.info(f"{user_id} did X")` → yapılandırılmış logging'i extra={} ile kullan
  10. Idempotency_key olmayan yan etkili işlem → çift ücret riski

  ## BAĞIMLILIK INJECTION'I

  FastAPI Depends() + app/core/deps.py'deki factory fonksiyonları kullan.
  dependency-injector, punq veya başka DI container yükleme.
  ...
  ## DOMAIN EXCEPTION'I DESENI

  Service'ler domain hataları kaldırır. Router'lar HTTP'ye eşler.
