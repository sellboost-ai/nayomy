...
body_tr: |-
  ...
  - Görevleri en küçük birim haline bölün ve sorunları adım adım çözün
  ...
  - **Teknik katmanlara değil, iş özelliklerine göre organize edin**
  - Her özellik tüm ilgili bileşenlerle kendi içinde yer alır
  - Modülarite, yeniden kullanılabilirlik ve daha iyi ekip işbirliğini teşvik eder
  - Kod tabanını gezinmesi ve bakımı daha kolay hale getirir
  - Farklı özellikler üzerinde paralel geliştirme imkanı sağlar
  ```
  src/main/kotlin/com/company/app/
  ├── common/              # Paylaşılan utility'ler, extension'lar
  ├── config/              # Uygulama konfigürasyonu, DI
  └── features/
      ├── auth/            # Özellik dizini
      │   ├── models/
      │   ├── repositories/
  ...
  1. Route handler'ları: Sadece request/response'u işleyin
  2. Service'ler: İş mantığını içerir, repository'leri çağırır
  3. Repository'ler: Veritabanı işlemlerini yönetir
  4. Entity sınıfları: Veritabanı modelleri için data sınıfları
  5. DTO'lar: Katmanlar arasında veri transferi

  ## Entity'ler & Data Sınıfları
  - Uygun doğrulama ile Kotlin data class'larını kullanın
  ...
  ### Test Kapsam Gereksinimleri
  - **Minimum kapsam**: %80 genel kod kapsamı gereklidir
  - **Kritik bileşenler**: Repository'ler, service'ler ve doğrulama için %90+ kapsam
  - **Tüm edge case'leri test edin**: Boş koleksiyonlar, null değerler, sınır koşulları
  - **Başarısızlık yollarını test edin**: İstisna işleme, doğrulama hataları, zaman aşımları
  - **Tüm public API'lar**: Entegrasyon testlerine sahip olmalıdır
  - **Performans açısından kritik yollar**: Benchmarking testlerine sahip olmalıdır
  ...
  ## Test Edilebilir Kod için Temel Prensipler
  1. **Tek Sorumluluk**: Her method'un bir şeyi iyi yapması gerekir
  2. **Saf Fonksiyonlar**: Aynı girdi her zaman aynı çıktı üretir
  ...
  5. **Küçük Method'lar**: Karmaşık mantığı test edilebilir yardımcı fonksiyonlara çıkarın

  ## Konfigürasyon Yönetimi
  ```kotlin
  // Tür-güvenli konfigürasyon
  interface AppConfig {
  ...
  - **Connection Pooling**: İş yüküne göre uygun boyutlandırma ile HikariCP'yi yapılandırın
  - **Caching**: Sık erişilen verilerin bellekte tutulması için Caffeine kullanın
  - **Coroutine'ler**: Asynchronous işleme için yapılandırılmış eşzamanlılık kullanın
  - **Veritabanı Sorguları**: Uygun indexleme, batch işlemler ve sayfalandırma ile optimize edin
---
