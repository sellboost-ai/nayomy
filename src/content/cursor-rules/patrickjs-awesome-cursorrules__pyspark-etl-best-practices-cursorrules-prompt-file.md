...
description_tr: "PySpark ETL geliştirme için cursor kuralları; kod stili, join operasyonları, window function'lar, map işlemleri ve Iceberg pattern'lerini kapsar."
category: "Other"
...
file_extension: ".mdc"
body_tr: |-
  PySpark, Spark SQL, Apache Iceberg ve production veri mühendisliğinde uzman bir kişi olursunuz. Performanslı, idiomatik ETL kodu yazarsınız ve bu kod testlenebilir, okunabilir ve kümülatif/snapshot tablolar için güvenlidir.
  ...
  ### ETL sınıf iskelet yapısı

  SparkSession yaşam döngüsünü yöneten bir temel sınıf oluşturun. Testlerin yerel bir oturum enjekte edebilmesi için isteğe bağlı bir `spark_session` parametresi kabul edin. İş mantığı için soyut bir method kullanın.
  ...
  Dataclass'ı saf veri olarak tutun ve CLI ayrıştırmasını bağımsız bir factory fonksiyonunda yapın. Bu, testlerde `sys.argv`'ye dokunmadan yapılandırmaların oluşturulmasını kolaylaştırır.
  ...
  `run_job`'u orkestrasyonda tutun. Her adım adlandırılmış bir method'tur.

  ```python
  events = self.read_source().transform(self.enrich).transform(self.merge_with_existing)
  ```

  ### Partition-farkında okumalar için paylaşılan reader kullanın

  Partition mekaniklerini (tarih filtreleri, saat aralıkları, en son partition aramaları) işleyen genel bir reader yardımcı sınıfı oluşturun. Her tablo için tek seferlik reader sınıfları oluşturmayın — domain-spesifik filtreleri, görülebildikleri ETL içinde tutun.
  ...
  # Reader partition işlemini yönetir
  events = PartitionedReader.read_by_date(spark, "catalog.my_table", "event_date", 20260319)

  # İş filtreleri ETL içinde kalır
  events = events.filter(F.col("event_type").isin("login", "purchase"))
  ```

  ### Paylaşılan merge yardımcı fonksiyonları

  Basit outer-join-with-coalesce merge'leri için, aliasing, join key coalescing ve sütun başına varsayılanları işleyen yeniden kullanılabilir bir merge fonksiyonu oluşturun. Per-key conflict resolution gerektiğinde (timestamp-farkında merge'ler) `map_zip_with` kullanın.

  ## 2. Kod Stili

  ### `F.col()` kullanın — her zaman `F.` önekini kullanın

  Fonksiyonları `import pyspark.sql.functions as F` olarak içeri aktarın ve `F.col()`, `F.when()`, `F.lit()` vb. kullanın. Bu, PySpark ifadelerini hemen tanınabilir ve taranabilir hale getirir.

  `df.colA` attribute erişiminden kaçının — bu, sütunu belirli bir DataFrame değişkenine bağlar ve join'lerden sonra veya değişken yeniden atandığında çalışmaz. Ayırım gerekiyorsa `F.col()` ile `.alias()` kullanın.

  ```python
  # KÖTÜ — sütunu belirli bir DataFrame değişkenine bağlar, join'lerden sonra çalışmaz
  ...
  `.filter()` veya `F.when()` içindeki mantığı 3 ifadeyle sınırlayın. Kalanını çıkarın.

  ```python
  # KÖTÜ — iç içe parantezlere gizlenmiş gereksiz mantık
  ...
  ### `withColumn` zincirlerine karşı `select` tercih edin

  `select`, çıktı şemasını tek bir geçişte belirtir. `withColumn` zincirleri ara DataFrameler oluşturur ve performansı düşürebilir — her çağrı, sorgu planında yeni bir projection tetikler.
  ...
  Zincir başına maksimum 5 statement. İşlem türüne göre ayırın (select/filter vs withColumn vs join).

  ```python
  # KÖTÜ — karıştırılmış kaygılar tek zincirde
  ...
  ### Sağ join'lerden ziyade sol join'leri tercih edin

  DataFrame sırasını çevirin ve okunabilirlik için `right` yerine `left` kullanın — birincil DataFrame sol tarafta kalır.

  ```python
  flights = flights.join(aircraft, 'aircraft_id', how='left')
  ```

  ### Join'lerden sonra ayırım için `.alias()` kullanın
  ...
  # İYİ — tüm DataFrame'i alias yapın
  ...
  ### Küçük boyut tabloları broadcast yapın

  Büyük fact DataFrame'ini küçük bir lookup/dimension tablosuyla birleştirirken, küçük tarafı `F.broadcast()` içine sarın ve küçük tarafta shuffle'ı atlayın.

  Executor belleğine sığabilecek kadar küçük tablolar için broadcast kullanın — tipik olarak dimension/lookup tablolar (kategori aramaları, ülke kodları, config eşlemeleri). Spark varsayılan olarak 10MB altındaki tabloları otomatik olarak broadcast eder (`spark.sql.autoBroadcastJoinThreshold`), ancak Spark boyutu tahmin edemediğinde (örneğin filtrelerden veya transformasyonlardan sonra) açık bir ipucu faydalıdır.

  Geliştirme sırasında bir tablonun broadcast için uygun olup olmadığını kontrol etmek için:
  - **Spark UI**: bir çalıştırdan sonra, SQL sekmesini kontrol edin — scan boyutları tablo başına gösterilir
  - **Hızlı satır sayısı**: `spark.read.table("catalog.my_dim").count()` (yalnızca dev, production kodunda değil)
  - **Sorgu planı**: `df.explain()` — Spark otomatik olarak broadcast ederse `BroadcastHashJoin` gösterir, aksi takdirde `SortMergeJoin` gösterir

  ```python
  df.join(F.broadcast(category_dim), 'category_id', how='left')
  ```

  ### `.dropDuplicates()` kullanmayı kesinlikle bir tıkaç olarak kullanmayın

  Duplicate satırlar görünürse, kök nedenini bulun. `.dropDuplicates()` sorunu maskeler ve shuffle overhead'i ekler.

  ## 4. Window Fonksiyonları

  `from pyspark.sql import Window as W` öğesini `import pyspark.sql.functions as F` ile birlikte kullanın.

  ### Her zaman açık bir frame belirtin

  Frame olmadan, Spark `orderBy`'ın bulunup bulunmadığına bağlı olarak değişen birini seçer.

  ```python
  # KÖTÜ — F.sum orderBy ile çalışan toplam, olmadan toplam verir. Şaşırtıcı.
  w = W.partitionBy('key').orderBy('num')

  # İYİ — istediğiniz şeyin açık
  ...
  - `row_number` + filter = **satırları sil**, en iyi olanı tut
  - `first` over window = **sütun değerini üzerine yaz**, tüm satırları tut

  ### `first` ve `last` ile `ignorenulls=True` kullanın

  Olmadan, ilk satırda null olması tüm partition için null döndürür.
  ...
  ### Boş `partitionBy()` kullanmayın

  Tüm verileri bir partition'a zorlar. Genel aggregation'lar için `.agg()` kullanın.

  ```python
  # KÖTÜ — tek partition, performansı öldürür
  w = W.partitionBy()
  ...
  ## 5. Map & Array Üst Seviye Fonksiyonları

  ### Karmaşık mantık ile map'leri birleştirirken `map_zip_with` kullanın

  `map_concat` anahtar çakışması olmayan basit merge'ler için iyidir. Anahtar başına özel mantık gerektiğinde (örneğin daha yeni timestamp'i tut, daha yüksek değeri seç), `map_zip_with` kullanın — bu, bir tarafın kör şekilde kazanmasına izin vermek yerine anahtar başına merge fonksiyonu verir.

  ```python
  # KÖTÜ — conflict resolution'ın kontrol edilmiş
  map_concat(existing_map, new_map)

  # İYİ — daha yeni timestamp'e sahip giriş tutun
  ...
  ```python
  # Struct'ların map'inden maksimum event_ts'yi ayıkla
  array_max(transform(map_values(my_map), lambda x: x.event_ts))
  ```

  ### UDF'lerden kaçının — önce native Spark fonksiyonlarını kullanın

  UDF'ler Catalyst optimizasyonunu kırarlar ve serileştirme overhead'i eklerler. Bir tane yazmadan önce, yerleşik bir Spark fonksiyonu veya üst seviye fonksiyonun işi yapıp yapamayacağını kontrol edin.

  ## 6. Kümülatif / Snapshot Tablo Desenleri

  ### Merge'ler idempotent olmalıdır

  Aynı veriyle yeniden çalıştırmak aynı sonucu üretmelidir, duplicate oluşturmamalıdır.

  ### Merge'ler sıra-bağımsız olmalıdır

  Eski verileri geri doldurmak yeni verileri üzerine yazmamalıdır. Çatışmaları çözmek için açık bir sıralama kriteri (örneğin event timestamp, sürüm numarası, partition tarihi) kullanın — `coalesce` argümanı sırası gibi konumsal üstünlüğe güvenmeyin.

  ### Yazma işleminden sonra birincil anahtar benzersizliğini doğrulayın

  Birincil anahtar benzersizliğini doğrulayan ve anahtar sütunlarında null'ları kontrol eden audit adımları ekleyin.

  ## 7. Veri Kalitesi & Performans

  ### Boş sütunlar için `F.lit(None)` kullanın, asla boş stringler değil
  ...
  Bilinmeyen değerler sessizce otherwise bucket'ına çöker ve veri kalitesi sorunlarını gizler.

  ```python
  # KÖTÜ — tahmin etmediğiniz yeni bir platform_type sessizce "Other" olur
  F.when(F.col('platform_type') == 'android', 'Mobile')
   .when(F.col('platform_type') == 'ios', 'Mobile')
   .otherwise('Other')

  # İYİ — harita dışı değerler null kalır, mantığınızdaki boşlukları ortaya çıkarır
  F.when(F.col('platform_type') == 'android', 'Mobile')
   .when(F.col('platform_type') == 'ios', 'Mobile')
  ```

  ### Production'da `.show()`, `.collect()`, `.printSchema()` yok

  Bunlar tam materyalizasyon tetiklerler veya gereksiz driver overhead'i eklerler. Bunları yalnızca yerel debugging'de kullanın, asla deployed ETL kodunda değil. `.count()` kasıtlı olarak kullanıldığında kabul edilebilir (örneğin, monitoring için satır sayılarını logging yapma, DAG fork'u öncesinde materyalizasyon zorla).

  ### `persist()` kasıtlı olarak kullanın

  Bir DataFrame sonraki birden fazla işlemde referans alındığında persist edin — aksi takdirde yazma işlemi onu sizin için materyalize edecektir. `.persist()` + `.count()` debugging için satır sayılarını zorlayarak materyalize etmek ve log etmek için yaygın bir desen; gerekli olduğunda kullanın ancak tam bir scan eklediğinin farkında olun.

  Depolama seviyesini kullanım durumunuza göre seçin:
  - `MEMORY_AND_DISK` — güvenli varsayılan, bellek sıkışık ise diske taşar
  - `MEMORY_ONLY` — daha hızlı ama çıkartılırsa yeniden hesaplama riski
  - `DISK_ONLY` — belleğe sığmayan çok büyük DataFrame'ler için

  ## 8. Iceberg Yazma Desenleri

  ### Schema evrimi güvenliği için `.byName()` kullanın

  Sütun sırası önemli değil — Spark konuma göre değil isme göre eşleşir.

  ```python
  df.write.byName().mode("overwrite").insertInto("catalog.my_table")
  ```

  ### En son partition okumalarını için `__partitions` metadata tablo kullanın

  Iceberg bir `__partitions` metadata tablosunu açığa çıkarır. En son snapshot'ı bulmak için tam tabloyu taramak yerine bunu kullanın.
  ...
  ### `write.distribution-mode` anla

  - `"none"` — yazma öncesinde yeniden shuffle yok. En hızlı, ancak çıktı dosya boyutları upstream partition'lamaya bağlı.
  - `"hash"` — verileri partition anahtarına göre yeniden dağıtır. Eşit boyutta dosyalar üretir ancak shuffle ekler.
  - `"range"` — yazma öncesinde verileri partition anahtarına göre sıralar. Sıralanmış scan performansı için iyi ancak en pahalı.
---
