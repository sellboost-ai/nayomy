...
body_tr: |-
  ...
  **B. Proje-yerel extension (klonla + çalıştır):** bu repo `.pi/extensions/mcp-nixos.ts` içerir, klonlanan repo'da `pi` çalıştırdığınızda otomatik yüklenir. İsteğe bağlı: editor tip çözümü için `cd .pi && npm install`. Pi bunu her iki şekilde de çalıştırır.
  ...
  - **nix-darwin** - Apple'ın belgelemediği 1K+ macOS ayarı
  - **Nixvim** - [NuschtOS search](https://github.com/NuschtOS/search) üzerinden Neovim yapılandırması için 5K+ seçenek
  - **FlakeHub** - [FlakeHub.com](https://flakehub.com) registry'sinden 600+ flake
  - **Noogle** - [noogle.dev](https://noogle.dev) üzerinden type signature'li 2K+ Nix function
  - **NixOS Wiki** - [wiki.nixos.org](https://wiki.nixos.org) üzerinden topluluk belgeleri ve kılavuzları
  - **nix.dev** - [nix.dev](https://nix.dev) üzerinden resmi Nix öğreticileri ve kılavuzları
  ...
  **Toplam ~1.030 token.** Hepsi bu. Diğer MCP serverları context'i Black Friday'miş gibi hoplayıp atarken, biz köşede minimalist çay içiyoruz. AI'ınız NixOS süper güçlerini şişkin olmadan alır.
  ...
  | `info` | Paket veya seçenek hakkında ayrıntılı bilgi al |
  | `stats` | Sayı ve kategoriler al |
  | `options` | Home Manager/Darwin seçeneklerine önekle göz at |
  ...
  | `home-manager` | Home Manager seçenekleri |
  | `darwin` | nix-darwin seçenekleri |
  | `flakes` | Topluluk flake'leri (search.nixos.org) |
  | `flakehub` | FlakeHub registry (flakehub.com) |
  | `nixvim` | Nixvim Neovim yapılandırma seçenekleri |
  | `noogle` | Nix function signature'ları ve belgeleri (noogle.dev) |
  ...
  **Nix yüklü olmanız GEREKMİYOR.** Python çalışan herhangi bir yerde çalışır.
  ...
  - **[Jetify](https://www.jetify.com)** - Devbox ve NixHub'ın yaratıcıları
