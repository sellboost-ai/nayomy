...
description_tr: "Microsoft'un resmi Playwright MCP sunucusu, LLM'lerin yapılandırılmış erişilebilirlik snapshot'ları aracılığıyla web sayfalarıyla etkileşime geçmesini sağlar."
...
body_tr: |-
  ...
  **Örnek: Yerel Kurulum**
  ...
  | --allowed-origins <origins> | tarayıcıya istekte bulunmasına izin verilebilecek GÜVENILIR kökenlerin noktalı virgülle ayrılmış listesi. Varsayılan tüm kökenler için izin vermektir. Önemli: *değildir* güvenlik sınırı olarak davranır ve *değildir* yönlendirmeleri etkiler.<br>*env* `PLAYWRIGHT_MCP_ALLOWED_ORIGINS` |
  | --allow-unrestricted-file-access | çalışma alanı kökleri dışındaki dosyalara erişime izin ver. Ayrıca file:// URL'lerine kısıtlanmamış erişime izin verir. Varsayılan olarak dosya sistemi erişimi yalnızca çalışma alanı kök dizinleriyle (veya konfigüre edilen kökler yoksa cwd) sınırlandırılır ve file:// URL'lerine gezinti engellenir.<br>*env* `PLAYWRIGHT_MCP_ALLOW_UNRESTRICTED_FILE_ACCESS` |
  | --blocked-origins <origins> | tarayıcının istekte bulunmasını engelleyecek kökenlerin noktalı virgülle ayrılmış listesi. Engel listesi izin listesinden önce değerlendirilir. İzin listesi olmadan kullanılırsa, engel listesine uymayan istekler yine de izin verilir. Önemli: *değildir* güvenlik sınırı olarak davranır ve *değildir* yönlendirmeleri etkiler.<br>*env* `PLAYWRIGHT_MCP_BLOCKED_ORIGINS` |
  | --block-service-workers | service worker'ları engelle<br>*env* `PLAYWRIGHT_MCP_BLOCK_SERVICE_WORKERS` |
  | --browser <browser> | kullanılacak tarayıcı veya chrome kanalı, olası değerler: chrome, firefox, webkit, msedge.<br>*env* `PLAYWRIGHT_MCP_BROWSER` |
  | --caps <caps> | etkinleştirilecek ek yeteneklerin virgülle ayrılmış listesi, olası değerler: vision, pdf, devtools.<br>*env* `PLAYWRIGHT_MCP_CAPS` |
  | --cdp-endpoint <endpoint> | bağlanılacak CDP endpoint'i.<br>*env* `PLAYWRIGHT_MCP_CDP_ENDPOINT` |
  | --cdp-header <headers...> | bağlantı isteği ile gönderilecek CDP başlıkları, birden fazla belirtilebilir.<br>*env* `PLAYWRIGHT_MCP_CDP_HEADER` |
  | --cdp-timeout <timeout> | CDP endpoint'ine bağlanma zaman aşımı milisaniye cinsinden, varsayılan olarak 30000ms<br>*env* `PLAYWRIGHT_MCP_CDP_TIMEOUT` |
  | --codegen <lang> | kod üretimi için kullanılacak dili belirt, olası değerler: "typescript", "none". Varsayılan "typescript".<br>*env* `PLAYWRIGHT_MCP_CODEGEN` |
  | --config <path> | yapılandırma dosyasının yolu.<br>*env* `PLAYWRIGHT_MCP_CONFIG` |
  | --console-level <level> | döndürülecek konsol iletilerinin seviyesi: "error", "warning", "info", "debug". Her seviye daha ciddi seviyelerin mesajlarını içerir.<br>*env* `PLAYWRIGHT_MCP_CONSOLE_LEVEL` |
  | --device <device> | taklit edilecek cihaz, örneğin: "iPhone 15"<br>*env* `PLAYWRIGHT_MCP_DEVICE` |
  | --executable-path <path> | tarayıcı yürütülebilir dosyasının yolu.<br>*env* `PLAYWRIGHT_MCP_EXECUTABLE_PATH` |
  | --extension | Çalışan bir tarayıcı örneğine bağlan (sadece Edge/Chrome). "Playwright Extension"in yüklenmesini gerektirir.<br>*env* `PLAYWRIGHT_MCP_EXTENSION` |
  | --endpoint <endpoint> | bağlanılacak sınırlı tarayıcı endpoint'i.<br>*env* `PLAYWRIGHT_MCP_ENDPOINT` |
  | --grant-permissions <permissions...> | tarayıcı bağlamına verilecek izinlerin listesi, örneğin "geolocation", "clipboard-read", "clipboard-write".<br>*env* `PLAYWRIGHT_MCP_GRANT_PERMISSIONS` |
  | --headless | tarayıcıyı başlıksız modda çalıştır, varsayılan olarak başlıklı<br>*env* `PLAYWRIGHT_MCP_HEADLESS` |
  | --host <host> | sunucunun bağlanacağı ana bilgisayar. Varsayılan localhost'tur. Tüm arayüzlere bağlanmak için 0.0.0.0 kullanın.<br>*env* `PLAYWRIGHT_MCP_HOST` |
  | --ignore-https-errors | https hatalarını yoksay<br>*env* `PLAYWRIGHT_MCP_IGNORE_HTTPS_ERRORS` |
  | --init-page <path...> | Playwright sayfa nesnesi üzerinde değerlendirilecek TypeScript dosyasının yolu<br>*env* `PLAYWRIGHT_MCP_INIT_PAGE` |
  | --init-script <path...> | başlatma script'i olarak eklenecek JavaScript dosyasının yolu. Script, sayfanın kendi script'lerinden önce her sayfada değerlendirilecektir. Birden fazla kez belirtilebilir.<br>*env* `PLAYWRIGHT_MCP_INIT_SCRIPT` |
  | --isolated | tarayıcı profilini bellekte tut, diske kaydetme<br>*env* `PLAYWRIGHT_MCP_ISOLATED` |
  | --image-responses <mode> | istemciye görüntü yanıtları gönderilip gönderilmeyeceği. "allow" veya "omit" olabilir, varsayılan "allow".<br>*env* `PLAYWRIGHT_MCP_IMAGE_RESPONSES` |
  | --no-sandbox | normally korumalı tüm işlem türleri için sandbox'ı devre dışı bırak.<br>*env* `PLAYWRIGHT_MCP_NO_SANDBOX` |
  | --output-dir <path> | çıkış dosyaları için dizinin yolu.<br>*env* `PLAYWRIGHT_MCP_OUTPUT_DIR` |
  | --output-mode <mode> | anlık görüntülerin, konsol mesajlarının, ağ günlüklerinin bir dosyaya mı yoksa standart çıktıya mı kaydedileceği. "file" veya "stdout" olabilir. Varsayılan "stdout".<br>*env* `PLAYWRIGHT_MCP_OUTPUT_MODE` |
  | --port <port> | SSE taşıması için dinlenecek port.<br>*env* `PLAYWRIGHT_MCP_PORT` |
  | --proxy-bypass <bypass> | proxy'yi atlayacak virgülle ayrılmış etki alanları, örneğin ".com,chromium.org,.domain.com"<br>*env* `PLAYWRIGHT_MCP_PROXY_BYPASS` |
  | --proxy-server <proxy> | proxy sunucusunu belirt, örneğin "http://myproxy:3128" veya "socks5://myproxy:8080"<br>*env* `PLAYWRIGHT_MCP_PROXY_SERVER` |
  | --sandbox | normally sandbox'lanmayan tüm işlem türleri için sandbox'ı etkinleştir.<br>*env* `PLAYWRIGHT_MCP_SANDBOX` |
  | --save-session | Playwright MCP oturumunun çıkış dizinine kaydedilip kaydedilmeyeceği.<br>*env* `PLAYWRIGHT_MCP_SAVE_SESSION` |
  | --secrets <path> | dotenv formatında gizli dizileri içeren dosyanın yolu<br>*env* `PLAYWRIGHT_MCP_SECRETS_FILE` |
  | --shared-browser-context | tüm bağlı HTTP istemcileri arasında aynı tarayıcı bağlamını yeniden kullan.<br>*env* `PLAYWRIGHT_MCP_SHARED_BROWSER_CONTEXT` |
  | --snapshot-mode <mode> | yanıtlar için anlık görüntüler alırken, kullanılacak modu belirtir. "full" veya "none" olabilir. Varsayılan "full".<br>*env* `PLAYWRIGHT_MCP_SNAPSHOT_MODE` |
  | --storage-state <path> | yalıtılmış oturumlar için depolama durumu dosyasının yolu.<br>*env* `PLAYWRIGHT_MCP_STORAGE_STATE` |
  | --test-id-attribute <attribute> | test kimlikleri için kullanılacak özniteliği belirt, varsayılan "data-testid"<br>*env* `PLAYWRIGHT_MCP_TEST_ID_ATTRIBUTE` |
  | --timeout-action <timeout> | eylem zaman aşımını milisaniye cinsinden belirt, varsayılan 5000ms<br>*env* `PLAYWRIGHT_MCP_TIMEOUT_ACTION` |
  | --timeout-navigation <timeout> | gezinti zaman aşımını milisaniye cinsinden belirt, varsayılan 60000ms<br>*env* `PLAYWRIGHT_MCP_TIMEOUT_NAVIGATION` |
