...
description_tr: "AI ajanları için deterministik çalıştırma motoru; 78 kategoride 412 modül (browser, file, Docker, data, crypto, scheduling) içerir. Execution trace, evidence snapshot, herhangi bir adımdan replay ve STDIO ile Streamable HTTP transport desteği sunar."
...
body_tr: |-
  ...
  > **Hata ayıklanabilir bir otomasyon motoru. Her adımı takip et. Herhangi bir noktadan yeniden oynat.**
  >
  > **[flyto2.com](https://flyto2.com)** · [Desktop Uygulaması](https://flyto2.com/app.html) · [Dokümantasyon](https://docs.flyto2.com) · [YouTube](https://www.youtube.com/@Flyto2)
  ...
  ## 8. adım başarısız olursa ne olur?
  ...
  **flyto-core** — 12 adım
  ...
  ## v2.19.0'da Yenilikler

  - **Akıllı validate_params** — yanlış alan adlarını takma ad haritalaması (örn. `site` → `url`) ile otomatik düzeltir ve var olmayan bir module istendiğinde alternatifler önerir
  - **Geliştirilmiş search_modules** — eski substring-only aramasının yerine kelime düzeyinde ve etiket eşleştirme puanlaması, doğal dil sorguları için daha alakalı sonuçlar üretir
  - **browser.extract varsayılan metin modu** — `fields` belirtilmediğinde, `browser.extract` artık eşleştirilen öğelerin metin içeriğini varsayılan olarak döndürür (önceden boş nesneler döndürüyordu)
  - **Tarayıcı kanal desteği** — `browser.launch` ile `channel: 'chrome'` geçirin sistem yüklü Chrome'u kullanmak için, headless tarayıcıları parmak izi alan siteler için anti-bot tespitini atlamak için yararlı

  ## Motor Özellikleri

  - **Yürütme İzleme** — her adımın yapılandırılmış kaydı: giriş, çıkış, zamanlama, durum
  - **Yeniden Oynatma** — orijinal (veya değiştirilmiş) bağlamla herhangi bir adımdan yeniden çalıştır
  - **Kesme Noktaları** — herhangi bir adımda yürütmeyi duraklat, durumu incele, devam et
  - **Kanıt Anlık Görüntüleri** — her adım sınırından önce ve sonra tam durum
  - **Veri Soysoyu** — adımlar arasında veri akışını takip et, bağımlılık grafikleri oluştur
  - **Timeout Koruması** — workflow seviyesi ve adım başına yapılandırılabilir timeout koruması

  ---

  ## 467 Module, 78 Kategori

  | Kategori | Sayı | Örnekler |
  |----------|------|----------|
  | `browser.*` | 38 | launch, goto, click, extract, screenshot, form doldur, bekle |
  ...
  | `data.*` | 8 | json/xml/yaml/csv parse ve generate |
  ...
  <details>
  <summary><b>CLI</b> — terminalden workflow çalıştır</summary>
  ...
  <details>
  <summary><b>MCP Sunucusu</b> — Claude Code, Cursor, Windsurf için</summary>
  ...
  <details>
  <summary><b>HTTP API</b> — entegrasyonlar ve uzak yürütme için</summary>
  ...
  | Endpoint | Amaç |
  |----------|------|
  | `POST /v1/workflow/run` | Workflow'u kanıt + izleme ile çalıştır |
  | `POST /v1/workflow/{id}/replay/{step}` | Herhangi bir adımdan yeniden oynat |
  | `POST /v1/execute` | Tek bir module çalıştır |
  | `GET /v1/modules` | Tüm modulları keşfet |
  | `POST /mcp` | MCP Streamable HTTP taşıma |

  </details>

  <details>
  <summary><b>Python API</b> — programatik kullanım için</summary>
  ...
  **[Desktop Uygulaması](https://flyto2.com/app.html)** · **[Fiyatlandırma](https://flyto2.com/pricing.html)** · **[flyto2.com](https://flyto2.com)**

  ## Barındırılan dağıtım

  Barındırılan bir dağıtım [Fronteir AI](https://fronteir.ai/mcp/flytohub-flyto-core) üzerinde mevcuttur.
---
