---
name: "PipedreamHQ/pipedream"
description: "Connect with 2,500 APIs with 8,000+ prebuilt tools, and manage servers for your users, in your own app."
description_tr: "2.500'den fazla API'ye bağlanın, 8.000+ hazır araçtan yararlanın ve kendi uygulamanızda kullanıcılarınızın sunucularını yönetin."
category: "Aggregators"
repo: "PipedreamHQ/pipedream"
stars: 11395
url: "https://github.com/PipedreamHQ/pipedream"
body_length: 12082
license: "NOASSERTION"
language: "JavaScript"
homepage: "https://pipedream.com"
body_tr: |-
  ![pipedream](https://i.ibb.co/LPhXtH1/logo.png)

  <p align="center">
    <a href="https://pipedream.com/community"></a>
    <a href="https://pipedream.com/support"></a>
    <a href="https://twitter.com/intent/follow?original_referer=https%3A%2F%2Fpublish.twitter.com%2F%3FbuttonType%3DFollowButton%26query%3Dhttps%253A%252F%252Ftwitter.com%252Fpipedream%26widget%3DButton&ref_src=twsrc%5Etfw&region=follow_link&screen_name=pipedream&tw_p=followbutton"></a>
    <a href="https://wellfound.com/company/pipedreamhq/jobs"></a>
  </p>

  Pipedream, geliştiriciler için bir entegrasyon platformudur.

  Pipedream, uygulamaları bağlamak ve olay odaklı otomasyonlar geliştirmek için ücretsiz, barındırılan bir platform sağlar. Platform 1.000'den fazla tam olarak entegre edilmiş uygulamaya sahiptir, böylece önceden oluşturulmuş bileşenleri kullanarak Slack'e hızlı mesaj gönderebilir, Google Sheets'e yeni bir satır ekleyebilir ve daha fazlasını yapabilirsiniz. Özel logic'e ihtiyacınız olduğunda Node.js, Python, Golang veya Bash kodu çalıştırabilirsiniz. Pipedream, SOC 2 uyumluluğunu göstermiştir ve istek üzerine bir SOC 2 Type 2 raporu sağlayabilir (lütfen support@pipedream.com adresine e-posta gönderin).

  <p align="center">
    <br />
    
    <br />
  </p>

  Bu depo şunları içerir:

  - [Önceden oluşturulmuş tüm entegrasyon bileşenlerinin kodu](https://github.com/PipedreamHQ/pipedream/tree/master/components)
  - [Ürün yol haritası](https://github.com/PipedreamHQ/pipedream/issues)
  - [Pipedream dokümantasyonu](https://github.com/PipedreamHQ/pipedream/tree/master/docs)
  - Ve Pipedream ile ilgili diğer kaynak kodlar.

  Bu `README`, platformun temel özelliklerini ve nasıl başlanacağını açıklar.

  Destek almak için lütfen [https://pipedream.com/support](https://pipedream.com/support) adresini ziyaret edin.

  ## Temel Özellikler

  - [Workflows](#workflows) - Workflows otomasyonları çalıştırır. Workflows, bir olay tarafından tetiklenen adımların serileridir - önceden oluşturulmuş actionlar veya özel [Node.js](https://pipedream.com/docs/code/nodejs/), [Python](https://pipedream.com/docs/code/python/), [Golang](https://pipedream.com/docs/code/go/) veya [Bash](https://pipedream.com/docs/code/bash/) kodu - (HTTP request, timer, Google Sheets'e yeni satır eklenmesi ve daha fazlası).
  - [Event Sources](#event-sources) - Sources, workflows'ü tetikler. GitHub, Slack, Airtable, RSS ve [daha fazlası](https://pipedream.com/apps) gibi hizmetlerden olayları yayınlarlar. Herhangi bir üçüncü taraf uygulamada bir olay meydana geldiğinde bir workflow çalıştırmak istediğinizde, bir event source kullanıyorsunuz.
  - [Actions](#actions) - Actions, bir workflow'da Pipedream'in 1.000+ API entegrasyonunda yaygın işlemleri gerçekleştirmek için kullanabileceğiniz önceden oluşturulmuş kod adımlarıdır. Örneğin, e-posta göndermek, Google Sheet'e satır eklemek ve [daha fazlasını](https://pipedream.com/apps) yapmak için actionları kullanabilirsiniz.
  - [Custom code](#code) - Çoğu entegrasyonda özel logic gereklidir. Kod genellikle bu logic'i ifade etmenin en iyi yoludur, bu nedenle Pipedream herhangi bir [Node.js](https://pipedream.com/docs/code/nodejs/), [Python](https://pipedream.com/docs/code/python/), [Golang](https://pipedream.com/docs/code/go/) veya [Bash](https://pipedream.com/docs/code/bash/) kodu çalıştırmanıza izin verir. Dil paket yöneticilerinden herhangi bir paketi içe aktarabilir, herhangi bir Pipedream bağlı uygulamasına bağlanabilir ve daha fazlasını yapabilirsiniz. Pipedream, en iyi şekilde "low-code"dir: yaygın işlemler gerçekleştirirken önceden oluşturulmuş bileşenleri kullanabilir, ancak ihtiyacınız olduğunda özel kod yazabilirsiniz.
  - [Destinations](#destinations) - Olayları Amazon S3, Snowflake, HTTP ve e-posta gibi yaygın hedeflere asenkron olarak iletir.
  - [Ücretsiz](#pricing) - Bireysel geliştiriciler için ücret yok (bkz. [limitler](https://docs.pipedream.com/limits/))

  ## Demo

  Aşağıdaki resmi tıklayarak YouTube'da kısa bir demoyu izleyin.

  <p align="center">
    <br />
    <a href="https://bit.ly/3ytGgyR">
      
    </a>
  </p>

  ### Workflows

  Workflows, bir olay tarafından tetiklenen (HTTP request gibi veya Google sheet'e yeni bir satır eklendiğinde) lineer [adımların](https://pipedream.com/docs/workflows/steps) serileridir. Workflows'ü kullanarak karmaşık otomasyonlar hızlı bir şekilde geliştirebilir ve 1.000'den fazla entegre uygulamadan herhangi birine bağlanabilirsiniz.

  [Workflow hızlı başlangıç kılavuzumuza bakın](https://pipedream.com/docs/quickstart/) başlamak için.

  ### Event Sources

  [Event Sources](https://pipedream.com/docs/sources/), GitHub, Slack, Airtable, RSS ve [daha fazlası](https://pipedream.com/apps) gibi hizmetlerden yeni verileri izler. Bir source yeni bir olay bulduğunda, onu yayınlar ve bağlı olan tüm workflows'ü tetikler.

  [Pipedream'in REST API](https://pipedream.com/docs/api/rest/) kullanarak veya özel, gerçek zamanlı bir [SSE stream](https://pipedream.com/docs/api/sse/) kullanarak sources tarafından yayınlanan olayları tüketebilirsiniz.

  Kullanım durumunuz için önceden oluşturulmuş bir source yoksa, [kendinizin oluşturabilirsiniz](https://pipedream.com/docs/components/quickstart/nodejs/sources/). İşte en basit event source: herhangi bir isteği gönderebileceğiniz bir HTTP endpoint'i ortaya çıkarır ve çağrıldığında isteğin içeriğini yazdırır:

  ```javascript
  export default {
    name: "http",
    version: "0.0.1",
    props: {
      http: "$.interface.http",
    },
    run(event) {
      console.log(event); // event contains the method, payload, etc.
    },
  };
  ```

  <a href="https://pipedream.com/sources/new?app=http"></a>

  Önceden oluşturulmuş tüm sources'ların kodunu [components dizininde](https://github.com/PipedreamHQ/pipedream/tree/master/components) bulabilirsiniz. Bir hata bulursanız veya bir özellik katkısı yapmak istiyorsanız, [katkı kılavuzumuza bakın](https://pipedream.com/docs/components/guidelines/#process).

  ### Actions

  [Actions](https://pipedream.com/docs/components/actions/), bir workflow'da Pipedream'in 500+ API entegrasyonunda yaygın işlemleri gerçekleştirmek için kullanabileceğiniz önceden oluşturulmuş kod adımlarıdır. Örneğin, e-posta göndermek, Google Sheet'e satır eklemek ve [daha fazlasını](https://pipedream.com/apps) yapmak için actionları kullanabilirsiniz.

  [Kendi actionlarınızı oluşturabilirsiniz](https://pipedream.com/docs/components/quickstart/nodejs/actions/), bu actionları workflows'de yeniden kullanabilirsiniz. [Actionları tüm Pipedream topluluğuna yayınlayabilirsiniz](https://pipedream.com/docs/components/guidelines/), böylece herkes tarafından kullanılabilir hale gelirler.

  İşte `name` girdi olarak kabul eden ve onu workflow'ün loglarına yazdıran bir action:

  ```javascript
  export default {
    name: "Action Demo",
    description: "This is a demo action",
    key: "action_demo",
    version: "0.0.1",
    type: "action",
    props: {
      name: {
        type: "string",
        label: "Name",
      },
    },
    async run() {
      return `hello ${this.name}!`;
    },
  };
  ```

  Önceden oluşturulmuş tüm actionların kodunu [components dizininde](https://github.com/PipedreamHQ/pipedream/tree/master/components) bulabilirsiniz. Bir hata bulursanız veya bir özellik katkısı yapmak istiyorsanız, [katkı kılavuzumuza bakın](https://pipedream.com/docs/components/guidelines/#process).

  ### Custom code

  Çoğu entegrasyonda özel logic gereklidir. Kod genellikle bu logic'i ifade etmenin en iyi yoludur, bu nedenle Pipedream bir workflow'da özel kod çalıştırmanıza izin verir:

  <table align="center">
    <tr>
      <td>
        <a href="https://pipedream.com/docs/code/nodejs/">
          
        </a>
      </td>
      <td>
        <a href="https://pipedream.com/docs/code/python/">
          
        </a>
      </td>
    </tr>
    </tr>
      <td>
        <a href="https://pipedream.com/docs/code/go/">
          
        </a>
      </td>
      <td>
        <a href="https://pipedream.com/docs/code/bash/">
          
        </a>
      </td>
    </tr>
  </table>

  Dil paket yöneticilerinden herhangi bir paketi kodu doğrudan içe aktararak bildirebilirsiniz. Pipedream, gerekli bağımlılıkları ayrıştırır ve indirir.

  ```javascript
  // Node.js
  import axios from "axios";
  ```

  ```python
  # Python
  import pandas as pd
  ```

  ```golang
  // Go
  import (
      "fmt"
      pd "github.com/PipedreamHQ/pipedream-go"
  )
  ```

  [Özel kod adımlarında herhangi bir Pipedream bağlı uygulamasına bağlanabilirsiniz](https://pipedream.com/docs/code/nodejs/auth/). Örneğin, Slack hesabınızı bağlayıp bir kanala mesaj gönderebilirsiniz:

  ```javascript
  import { WebClient } from "@slack/web-api";

  export default defineComponent({
    props: {
      // This creates a connection called "slack" that connects a Slack account.
      slack: {
        type: "app",
        app: "slack",
      },
    },
    async run({ steps, $ }) {
      const web = new WebClient(this.slack.$auth.oauth_access_token);

      return await web.chat.postMessage({
        text: "Hello, world!",
        channel: "#general",
      });
    },
  });
  ```

  ### Destinations

  [Destinations](https://pipedream.com/docs/destinations/), actionlar gibi, olayları Amazon S3 veya HTTP ve e-posta gibi hedeflere göndermek için gerekli olan bağlantı, toplu işleme ve dağıtım logic'ini soyutlar.

  Örneğin, verileri Amazon S3 bucket'ına göndermek, `$send.s3()` çağırmak kadar basittir:

  ```javascript
  $send.s3({
    bucket: "your-bucket-here",
    prefix: "your-prefix/",
    payload: event.body,
  });
  ```

  Pipedream aşağıdaki destinations'ı destekler:

  - [Amazon S3](https://docs.pipedream.com/destinations/s3/)
  - [Snowflake](https://docs.pipedream.com/destinations/snowflake/)
  - [HTTP](https://docs.pipedream.com/destinations/http/)
  - [Email](https://docs.pipedream.com/destinations/email/)
  - [SSE](https://docs.pipedream.com/destinations/sse/)

  ## Katkıcılar

  Pipedream kod tabanına katkıda bulunan herkese teşekkür ederiz. Siz harikasınız!

  <a href="https://github.com/PipedreamHQ/pipedream/graphs/contributors">
    
  </a>

  ## Fiyatlandırma

  Pipedream, [cömert bir ücretsiz katman](https://pipedream.com/docs/pricing/#developer-tier) sunmaktadır. Ücretsiz katmanın sınırları içinde sources ve workflows'ü ücretsiz olarak çalıştırabilirsiniz. Bu sınırları aşarsanız, [ücretli katmanlarımızdan](https://pipedream.com/docs/pricing/) birine yükseltebilirsiniz.

  ## Limitler

  Pipedream platformu sources ve workflows'e bazı runtime limitleri uygular. [Dokümantasyonumuzda bunlar hakkında daha fazla bilgi edinin](https://pipedream.com/docs/limits/).

  ## Hata Buldunuz? Öneriniz Mi Var?

  Bir issue eklemeden önce, lütfen [mevcut issues'leri](https://github.com/PipedreamHQ/pipedream/issues) arayın veya benzer bir isteğin zaten var olup olmadığını görmek için [takımımıza ulaşın](https://pipedream.com/support/).

  Bir issue varsa, lütfen [bir tepki ekleyin](https://help.github.com/en/github/collaborating-with-issues-and-pull-requests/about-conversations-on-github) veya belirli kullanım durumunuzu ayrıntılı bir şekilde açıklayan bir yorum ekleyin.

  Bir issue henüz _yoksa_ ve oluşturmanız gerekiyorsa, lütfen [issue şablonlarını kullanın](https://github.com/PipedreamHQ/pipedream/issues/new/choose).

  ## Güvenlik

  Platform güvenliği ve gizliliği hakkında [burada](https://pipedream.com/docs/privacy-and-security/) okuyabilirsiniz.

  Şüpheli bir açık bulgu veya güvenlik sorunu bildirmek istiyorsanız veya ürünün güvenliği hakkında sorularınız varsa, lütfen güvenlik takımımıza **security@pipedream.com** adresinde iletişim kurun.
---

![pipedream](https://i.ibb.co/LPhXtH1/logo.png)

<p align="center">
  <a href="https://pipedream.com/community"></a>
  <a href="https://pipedream.com/support"></a>
  <a href="https://twitter.com/intent/follow?original_referer=https%3A%2F%2Fpublish.twitter.com%2F%3FbuttonType%3DFollowButton%26query%3Dhttps%253A%252F%252Ftwitter.com%252Fpipedream%26widget%3DButton&ref_src=twsrc%5Etfw&region=follow_link&screen_name=pipedream&tw_p=followbutton"></a>
  <a href="https://wellfound.com/company/pipedreamhq/jobs"></a>
</p>

Pipedream is an integration platform for developers.

Pipedream provides a free, hosted platform for connecting apps and developing event-driven automations. The platform has over 1,000 fully-integrated applications, so you can use pre-built components to quickly send messages to Slack, add a new row to Google Sheets, and more. You can also run any Node.js, Python, Golang, or Bash code when you need custom logic. Pipedream has demonstrated SOC 2 compliance and can provide a SOC 2 Type 2 report upon request (please email support@pipedream.com).

<p align="center">
  <br />
  
  <br />
</p>

This repo contains:

- [The code for all pre-built integration components](https://github.com/PipedreamHQ/pipedream/tree/master/components)
- [The product roadmap](https://github.com/PipedreamHQ/pipedream/issues)
- [The Pipedream docs](https://github.com/PipedreamHQ/pipedream/tree/master/docs)
- And other source code related to Pipedream.

This `README` explains the key features of the platform and how to get started.

To get support, please visit [https://pipedream.com/support](https://pipedream.com/support).

## Key Features

- [Workflows](#workflows) - Workflows run automations. Workflows are sequences of steps - pre-built actions or custom [Node.js](https://pipedream.com/docs/code/nodejs/), [Python](https://pipedream.com/docs/code/python/), [Golang](https://pipedream.com/docs/code/go/), or [Bash](https://pipedream.com/docs/code/bash/) code - triggered by an event (HTTP request, timer, when a new row is added to a Google Sheets, and more).
- [Event Sources](#event-sources) - Sources trigger workflows. They emit events from services like GitHub, Slack, Airtable, RSS and [more](https://pipedream.com/apps). When you want to run a workflow when an event happens in any third-party app, you're using an event source.
- [Actions](#actions) - Actions are pre-built code steps that you can use in a workflow to perform common operations across Pipedream's 1,000+ API integrations. For example, you can use actions to send email, add a row to a Google Sheet, [and more](https://pipedream.com/apps).
- [Custom code](#code) - Most integrations require custom logic. Code is often the best way to express that logic, so Pipedream allows you to run any [Node.js](https://pipedream.com/docs/code/nodejs/), [Python](https://pipedream.com/docs/code/python/), [Golang](https://pipedream.com/docs/code/go/), or [Bash](https://pipedream.com/docs/code/bash/) code. You can import any package from the languages' package managers, connect to any Pipedream connected app, and more. Pipedream is "low-code" in the best way: you can use pre-built components when you're performing common actions, but you can write custom code when you need to.
- [Destinations](#destinations) - Deliver events asynchronously to common destinations like Amazon S3, Snowflake, HTTP and email.
- [Free](#pricing) - No fees for individual developers (see [limits](https://docs.pipedream.com/limits/))

## Demo

Click the image below to watch a brief demo on YouTube.

<p align="center">
  <br />
  <a href="https://bit.ly/3ytGgyR">
    
  </a>
</p>

### Workflows

Workflows are sequences of linear [steps](https://pipedream.com/docs/workflows/steps) triggered by an event (like an HTTP request, or when a new row is added to a Google sheet). You can quickly develop complex automations using workflows and connect to any of our 1,000+ integrated apps.

[See our workflow quickstart](https://pipedream.com/docs/quickstart/) to get started.

### Event Sources

[Event Sources](https://pipedream.com/docs/sources/) watch for new data from services like GitHub, Slack, Airtable, RSS and [more](https://pipedream.com/apps). When a source finds a new event, it emits it, triggering any linked workflows.

You can also consume events emitted by sources using [Pipedream's REST API](https://pipedream.com/docs/api/rest/) or a private, real-time [SSE stream](https://pipedream.com/docs/api/sse/).

When a pre-built source doesn't exist for your use case, [you can build your own](https://pipedream.com/docs/components/quickstart/nodejs/sources/). Here is the simplest event source: it exposes an HTTP endpoint you can send any request to, and prints the contents of the request when invoked:

```javascript
export default {
  name: "http",
  version: "0.0.1",
  props: {
    http: "$.interface.http",
  },
  run(event) {
    console.log(event); // event contains the method, payload, etc.
  },
};
```

<a href="https://pipedream.com/sources/new?app=http"></a>

You can find the code for all pre-built sources in [the `components` directory](https://github.com/PipedreamHQ/pipedream/tree/master/components). If you find a bug or want to contribute a feature, [see our contribution guide](https://pipedream.com/docs/components/guidelines/#process).

### Actions

[Actions](https://pipedream.com/docs/components/actions/) are pre-built code steps that you can use in a workflow to perform common operations across Pipedream's 500+ API integrations. For example, you can use actions to send email, add a row to a Google Sheet, [and more](https://pipedream.com/apps).

You can [create your own actions](https://pipedream.com/docs/components/quickstart/nodejs/actions/), which you can re-use across workflows. You can also [publish actions to the entire Pipedream community](https://pipedream.com/docs/components/guidelines/), making them available for anyone to use.

Here's an action that accepts a `name` as input and prints it to the workflow's logs:

```javascript
export default {
  name: "Action Demo",
  description: "This is a demo action",
  key: "action_demo",
  version: "0.0.1",
  type: "action",
  props: {
    name: {
      type: "string",
      label: "Name",
    },
  },
  async run() {
    return `hello ${this.name}!`;
  },
};
```

You can find the code for all pre-built actions in [the `components` directory](https://github.com/PipedreamHQ/pipedream/tree/master/components). If you find a bug or want to contribute a feature, [see our contribution guide](https://pipedream.com/docs/components/guidelines/#process).

### Custom code

Most integrations require custom logic. Code is often the best way to express that logic, so Pipedream allows you to run custom code in a workflow using:

<table align="center">
  <tr>
    <td>
      <a href="https://pipedream.com/docs/code/nodejs/">
        
      </a>
    </td>
    <td>
      <a href="https://pipedream.com/docs/code/python/">
        
      </a>
    </td>
  </tr>
  </tr>
    <td>
      <a href="https://pipedream.com/docs/code/go/">
        
      </a>
    </td>
    <td>
      <a href="https://pipedream.com/docs/code/bash/">
        
      </a>
    </td>
  </tr>
</table>

You can import any package from the languages' package managers by declaring the imports directly in code. Pipedream will parse and download the necessary dependencies.

```javascript
// Node.js
import axios from "axios";
```

```python
# Python
import pandas as pd
```

```golang
// Go
import (
    "fmt"
    pd "github.com/PipedreamHQ/pipedream-go"
)
```

You can also [connect to any Pipedream connected app in custom code steps](https://pipedream.com/docs/code/nodejs/auth/). For example, you can connect your Slack account and send a message to a channel:

```javascript
import { WebClient } from "@slack/web-api";

export default defineComponent({
  props: {
    // This creates a connection called "slack" that connects a Slack account.
    slack: {
      type: "app",
      app: "slack",
    },
  },
  async run({ steps, $ }) {
    const web = new WebClient(this.slack.$auth.oauth_access_token);

    return await web.chat.postMessage({
      text: "Hello, world!",
      channel: "#general",
    });
  },
});
```

### Destinations

[Destinations](https://pipedream.com/docs/destinations/), like actions, abstract the connection, batching, and delivery logic required to send events to services like Amazon S3, or targets like HTTP and email.

For example, sending data to an Amazon S3 bucket is as simple as calling `$send.s3()`:

```javascript
$send.s3({
  bucket: "your-bucket-here",
  prefix: "your-prefix/",
  payload: event.body,
});
```

Pipedream supports the following destinations:

- [Amazon S3](https://docs.pipedream.com/destinations/s3/)
- [Snowflake](https://docs.pipedream.com/destinations/snowflake/)
- [HTTP](https://docs.pipedream.com/destinations/http/)
- [Email](https://docs.pipedream.com/destinations/email/)
- [SSE](https://docs.pipedream.com/destinations/sse/)

## Contributors

Thank you to everyone who has contributed to the Pipedream codebase. We appreciate you!

<a href="https://github.com/PipedreamHQ/pipedream/graphs/contributors">
  
</a>

## Pricing

Pipedream has a [generous free tier](https://pipedream.com/docs/pricing/#developer-tier). You can run sources and workflows for free within the limits of the free tier. If you hit these limits, you can upgrade to one of our [paid tiers](https://pipedream.com/docs/pricing/).

## Limits

The Pipedream platform imposes some runtime limits on sources and workflows. [Read more about those in our docs](https://pipedream.com/docs/limits/).

## Found a Bug? Have a Feature to suggest?

Before adding an issue, please search the [existing issues](https://github.com/PipedreamHQ/pipedream/issues) or [reach out to our team](https://pipedream.com/support/) to see if a similar request already exists.

If an issue exists, please [add a reaction](https://help.github.com/en/github/collaborating-with-issues-and-pull-requests/about-conversations-on-github) or add a comment detailing your specific use case.

If an issue _doesn't_ yet exist and you need to create one, please [use the issue templates](https://github.com/PipedreamHQ/pipedream/issues/new/choose).

## Security

You can read about our platform security and privacy [here](https://pipedream.com/docs/privacy-and-security/).

If you'd like to report a suspected vulnerability or security issue, or have any questions about the security of the product, please contact our security team at **security@pipedream.com**.
