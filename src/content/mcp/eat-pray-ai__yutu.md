...
description_tr: "YouTube işlemlerini otomatikleştirmek için tam işlevsel bir MCP server ve CLI aracı."
...
body_tr: |-
  ...
  1. **GCP Projesi Oluşturun** ve `APIs & Services -> Enable APIs and services` altında bu API'leri etkinleştirin:
     ...
     - [YouTube Reporting API](https://console.cloud.google.com/apis/api/youtubereporting.googleapis.com/overview) (İsteğe bağlı)
  2. **OAuth kimlik bilgileri oluşturun**:
     - `APIs & Services -> OAuth consent screen` sayfasına gidin, kendinizi test kullanıcısı olarak ekleyerek bir onay ekranı oluşturun
     - `Credentials -> Create Credentials -> OAuth Client ID` sayfasına gidin, `Desktop app` seçin
     - Kimlik bilgisi dosyasını indirin ve `client_secret.json` olarak kaydedin, şuna benzer olmalıdır:
     ```json
     {
       "installed": {
  ...
  | `YUTU_LITE_MODEL`            | Alt agentler modeli (biçim: `provider:modelName`) | `YUTU_ADVANCED_MODEL` veya `YUTU_LITE_MODEL`'den en az biri |
  | `YUTU_LLM_API_KEY`           | Model sağlayıcı API anahtarı                      | Evet                                                     |
  | `GOOGLE_GEMINI_BASE_URL`     | Gemini API'si taban URL'si                        | Hayır                                                    |
  | `YUTU_AGENT_INSTRUCTION`     | Orkestratör agent için özel talimatlar           | Hayır                                                    |
  ...
  Kullanılabilir Komutlar:
    ...
    auth                   YouTube API'leriyle kimlik doğrulama yapın
    caption                YouTube video altyazılarını yönetin
    channel                YouTube kanallarını yönetin
    channelBanner          YouTube kanal başlıklarını yönetin
    channelSection         YouTube kanal bölümlerini yönetin
    comment                YouTube yorumlarını yönetin
    commentThread          YouTube yorum başlıklarını yönetin
    completion             Belirtilen shell için otomatik tamamlama betiğini oluşturun
    help                   Komutlar hakkında yardım
    i18nLanguage           YouTube i18n dillerini yönetin
    i18nRegion             YouTube i18n bölgelerini yönetin
    mcp                    MCP sunucusunu başlatın
    member                 YouTube kanal üyelerini yönetin
    membershipsLevel       YouTube üyelik seviyelerini yönetin
    playlist               YouTube çalma listelerini yönetin
    playlistImage          YouTube çalma listesi resimlerini yönetin
    playlistItem           YouTube çalma listesi öğelerini yönetin
    search                 YouTube aramasını yönetin
    subscription           YouTube aboneliklerini yönetin
    superChatEvent         YouTube Super Chat etkinliklerini yönetin
    thumbnail              YouTube video küçük resimlerini yönetin
    version                Yutu'nun sürümünü gösterin
    video                  YouTube videolarını yönetin
