...
description_tr: "VirusTotal API'sini sorgulamak için MCP sunucusu. Bu sunucu, URL'leri taramak, dosya hash'lerini analiz etmek ve IP adresi raporlarını almak için araçlar sağlar."
category: "Security"
...
body_tr: |-
  ...
  [VirusTotal API](https://www.virustotal.com/) sorgulama için bir Model Context Protocol (MCP) sunucusu. Bu sunucu, otomatik ilişki veri getirme özelliğine sahip kapsamlı güvenlik analiz araçları sağlar. [Claude Desktop](https://claude.ai) gibi MCP uyumlu uygulamalarla sorunsuz bir şekilde entegre edilir.
  ...
  VirusTotal Server'ı Claude Desktop'ta otomatik olarak [Smithery](https://smithery.ai/server/@burtthecoder/mcp-virustotal) aracılığıyla kurmak için:
  ...
  2. Claude Desktop konfigürasyon dosyanıza ekleyin:
  ```json
  {
    "mcpServers": {
      "virustotal": {
  ...
  2. VS Code MCP konfigürasyon dosyanızı şu konumda oluşturun veya güncelleyin:
  ...
  4. MCP sunucusunu etkinleştirmek için VS Code'u yeniden yükleyin
  ...
  2. Claude Desktop konfigürasyonunuza ekleyin:
  ```json
  {
    "mcpServers": {
      "virustotal": {
  ...
  - Geçerli bir [VirusTotal API Anahtarı](https://www.virustotal.com/gui/my-apikey)
  ...
  Bu proje MIT Lisansı altında lisanslanmıştır - ayrıntılar için [LICENSE](https://github.com/BurtTheCoder/mcp-virustotal/tree/HEAD/LICENSE) dosyasına bakın.
---
...
This project is licensed under the MIT License - see the [LICENSE](https://github.com/BurtTheCoder/mcp-virustotal/tree/HEAD/LICENSE) file for details.
