...
description_tr: "AI ajanlarını 600+ entegrasyon ile tek bir arayüzden bağlayın - OAuth, scaling ve monitoring dahil"
...
body_tr: |-
  ...
  <p align="center">
  AI ajanları için açık kaynaklı kimlik ve erişim katmanı. <br />
  Ajanları tutarlı kimlik doğrulama, izinler ve gözlemlenebilirlik ile gerçek sistemlere bağlayın.
  </p>
  
  > [!TIP]
  > _Kurulumu atlayın ve barındırılan sürümü kullanın:_ [Metorial](https://metorial.com)'u kullanmanın en hızlı, basit ve güvenilir yolu [barındırılan platformumuza](https://platform.metorial.com/) kaydolmaktır.
  >
  > ➡️ **[Başlayın (ücretsiz)](https://metorial.com)**
  ...
  - **1200+ entegrasyon** SaaS araçları, kurumsal sistemler, veri kaynakları ve özel MCP sunucuları arasında.
  - **OAuth, API anahtarları, hizmet hesapları ve diğer kimlik bilgisi akışları için kimlik doğrulama ve token yaşam döngüsü yönetimi**.
  - **RBAC, SAML SSO ve IAM yerleşik** olarak ajan erişimi mevcut güvenlik modelinizi takip eder.
  - **Ajan, workflow, takım veya ortam başına kapsamlı izinler**.
  - **Denetim günlükleri ve izlenebilirlik** ajan eylemler için, hangi ajanın kimin kimlik bilgileriyle hareket ettiği dahil.
  - **Tek seferlik entegrasyon kodu yerine takımlar ve projeler arasında paylaşılan erişim desenleri**.
  
  ## Geliştiriciler için Metorial
  
  Metorial, geliştiricilere ajanları gerçek sistemlere bağlamak için tek bir arayüz sağlar.
  
  - Claude Code, Codex ve Cursor gibi araçlar arasında entegrasyonlar kullanın, güvenlik politikalarını bozmadan.
  - Entegrasyonları herhangi bir ajan framework'üne araç olarak açığa çıkarın.
  - OAuth, API anahtarları ve diğer kimlik doğrulama akışlarını tek bir sihirli URL'de birleştirin.
  - Bağlantıları projeler, ortamlar ve kişiler arasında yeniden kullanın.
  - Her uygulama için entegrasyon ve kimlik doğrulama mantığını yeniden uygulamaktan kaçının.
  
  Tek bir API'ye karşı yazarsınız veya tek bir bağlantı URL'si kullanırsınız ve Metorial geri kalanını yönetir.
  
  ## Güvenlik için Metorial
  
  Metorial, ajanların sistemlere nasıl eriştiği hakkında yapı ve görünürlük sağlar.
  
  - RBAC, SAML SSO, IAM ve kapsamlı kimlik bilgileri ile merkezi erişim kontrolü.
  - Ajan başına net izin sınırları.
  - Tüm eylemler için denetim günlükleri, hangi ajan tarafından kimin kimlik bilgileriyle gerçekleştirildiği dahil.
  - Entegrasyonların kontrollü paylaşımı.
  - Tutarlı ve güvenli kimlik bilgisi işleme.
  ...
  - [**Metorial**](https://github.com/metorial/metorial): SaaS araçları ve kurumsal sistemleri kapsayan entegrasyon kataloğu.
  - [**Metorial Platform**](https://github.com/metorial/metorial-platform): Çekirdek motor, açık kaynaklı ve kendi kendine barındırılabilir.
  - [**Metorial CLI**](https://github.com/metorial/cli): Entegrasyonlarla etkileşim için ajan-merkezli CLI.
  - [**Lowerdeck**](https://github.com/metorial/lowerdeck): Yığın arasında paylaşılan kütüphaneler.
  - [**Starbase**](https://github.com/metorial/starbase): MCP hata ayıklama ve test yardımcı programı.
  ...
  Özel bir entegrasyon oluşturmak istiyorsanız, Metorial API'sini doğrudan nasıl kullanacağınız hakkında ayrıntılar için [API belgelerimize](https://metorial.com/api) bakın.
  
  ## Kendi Kendine Barındırma
  
  [Metorial Platform](https://github.com/metorial/metorial-platform), Metorial'in arkasındaki motorun gücünü sunan koddur. Açık kaynaktır ve kendi kendine barındırılabilir. Bunu bu repo'daki MCP sunucuları tarafından desteklenen kendi Metorial örneğinizi çalıştırmak için kullanabilirsiniz.
  ...
  ### OAuth Akışı Açıklandı
  
  1. **Kurulum oturumu oluşturun**: TypeScript'te `providerDeployments.setupSessions.create()` veya Python'da `provider_deployments.setup_sessions.create()` öğesini çağırın.
  2. **URL'yi gönderin**: Kurulum oturumu URL'sini kullanıcılara gösterin, böylece tarayıcıda kimlik doğrulaması yapabilirler.
  3. **Tamamlanmayı bekleyin**: Kurulum oturumunun bitmesini ve kimlik doğrulama yapılandırmasını döndürmesini bekleyin.
  4. **Kimlik doğrulama yapılandırmasını kullanın**: Sağlayıcıları yapılandırırken kimlik doğrulama yapılandırması ID'sini geçirin.
  
  ### Sağlayıcı Yapılandırması Örnekleri
  
  Panoda yönetilen dağıtımları, önceden oluşturulmuş kimlik doğrulama yapılandırmasını, satır içi kimlik bilgilerini ve yeniden kullanılabilir oturum şablonlarını birleştirmek için aynı `providers` listesini kullanın:
  ...
  - [`typescript-quick-start`](https://github.com/metorial/metorial-node/tree/main/examples/typescript-quick-start) - Metorial Search ile hızlı başlangıç
  - [`typescript-ai-sdk`](https://github.com/metorial/metorial-node/tree/main/examples/typescript-ai-sdk) - Vercel AI SDK + Anthropic
  - [`typescript-openai`](https://github.com/metorial/metorial-node/tree/main/examples/typescript-openai) - OpenAI fonksiyon çağrısı
  - [`typescript-anthropic`](https://github.com/metorial/metorial-node/tree/main/examples/typescript-anthropic) - Anthropic Claude araç kullanımı
  - [`typescript-google`](https://github.com/metorial/metorial-node/tree/main/examples/typescript-google) - Google Gemini fonksiyon bildirimleri
  - [`typescript-openai-compatible`](https://github.com/metorial/metorial-node/tree/main/examples/typescript-openai-compatible) - Herhangi bir OpenAI uyumlu API
  - [`typescript-langchain`](https://github.com/metorial/metorial-node/tree/main/examples/typescript-langchain) - LangChain ve LangGraph
  - [`typescript-provider-config`](https://github.com/metorial/metorial-node/tree/main/examples/typescript-provider-config) - Sağlayıcı kimlik doğrulama ve oturum şablonu desenleri
  
  ### Python
  
  - [`pydantic-ai`](https://github.com/metorial/metorial-python/tree/main/examples/pydantic-ai) - PydanticAI + Anthropic
  - [`langchain`](https://github.com/metorial/metorial-python/tree/main/examples/langchain) - LangChain ajan araçları
  - [`langgraph`](https://github.com/metorial/metorial-python/tree/main/examples/langgraph) - LangGraph akış ajanı
  - [`openai-agents`](https://github.com/metorial/metorial-python/tree/main/examples/openai-agents) - OpenAI Agents SDK
  - [`llamaindex`](https://github.com/metorial/metorial-python/tree/main/examples/llamaindex) - LlamaIndex fonksiyon ajanı
  - [`autogen`](https://github.com/metorial/metorial-python/tree/main/examples/autogen) - AutoGen asistanı araç çağrıları ile
  - [`crewai`](https://github.com/metorial/metorial-python/tree/main/examples/crewai) - CrewAI ajan araçları
  - [`google-adk`](https://github.com/metorial/metorial-python/tree/main/examples/google-adk) - Gemini ile Google ADK
  - [`haystack`](https://github.com/metorial/metorial-python/tree/main/examples/haystack) - Araçlarla Haystack pipeline
  
  ## Çoklu Sağlayıcı Desteği
  
  Aynı Metorial sağlayıcılarını farklı AI istemcileri ve framework'ler arasında kullanın.
  
  | Sağlayıcı      | TypeScript adaptörü    | Python adaptörü                | Model örnekleri                            |
  | ------------- | ---------------------- | ------------------------------ | ------------------------------------------ |
  | AI SDK        | `@metorial/ai-sdk`     | -                              | Vercel AI SDK üzerinden herhangi bir model |
  ...
  | LangChain     | `@metorial/langchain`  | `metorial_langchain()`         | LangChain üzerinden herhangi bir model     |
  | PydanticAI    | -                      | `metorial_pydantic_ai()`       | PydanticAI üzerinden herhangi bir model    |
  
  ## Motivasyon
  
  MCP, AI modellerini harici veri ve araçlara bağlamak için güçlü bir standarttır, ancak AI istemcilerinin (Claude Desktop veya Cursor gibi) araçlara ve veri kaynaklarına bağlanmasını sağlamaya odaklanır.
  Metorial, MCP'nin üzerine inşa edilir ancak geliştiricilerin AI uygulamalarını herhangi bir API'ye, veri kaynağına veya araca bağlamak için bunu tek satırla yapar.
  Böylece, geliştiricilerin diğer sistemlerle güvenilir, basit ve güvenli bir şekilde etkileşim kurabilen acentelik AI uygulamaları oluşturmasını sağlarız.
  
  ## Teknoloji Yığını
  
  - [**Model Context Protocol (MCP)**](https://modelcontextprotocol.io) - Metorial, Model Context Protocol tarafından desteklenir; AI modellerini harici veri ve araçlara bağlamak için bir standarttır.
  - [**Docker**](https://www.docker.com) - Metorial, MCP sunucularını kapsayıcılı bir ortamda çalıştırmak için Docker kullanır, dağıtım ve yönetimi kolaylaştırır.
  - [**MCP Containers**](https://github.com/metorial/mcp-containers) - Metorial, Docker kapsayıcılarında önceden oluşturulmuş MCP sunucuları koleksiyonu sağlar.
  - [**Typescript**](https://www.typescriptlang.org) - Metorial'in çoğu TypeScript'te yazılmıştır.
  - [**Bun**](https://bun.sh) - Metorial'in çekirdeği Bun üzerinde çalışır; Node.js ile uyumlu hızlı bir JavaScript çalışma zamanı.
  - [**Go**](https://go.dev) - MCP motoru Go'da yazılmıştır, Metorial için yüksek performanslı bir arka uç sağlar.
  - [**PostgreSQL**](https://www.postgresql.org) - Metorial veri depolaması için PostgreSQL kullanır.
  - [**Redis**](https://redis.io) - Metorial önbellek ve gerçek zamanlı veri işleme için Redis kullanır.
  - [**MongoDB**](https://www.mongodb.com) - Metorial kullanım verileri ve günlükleri depolamak için MongoDB kullanır.
  - [**React**](https://reactjs.org) - Metorial Paneli React ile oluşturulmuştur.
  ...
  ### Büyük Sunucu Kataloğu
  
  [Metorial sunucu dizini](https://github.com/metorial/mcp-index) zaten 5000'den fazla MCP sunucusu içeriyor. AI uygulamalarınız için MCP sunucuları bulmak ve kullanmak çok kolay. Her şey aranabilir ve düzgün organize edilmiştir, böylece kullanım durumunuz için doğru sunucuyu bulabilirsiniz.
  ...
  ### Geliştiriciler İçin İnşa Edildi
  ...
  - **Özelleştirilebilir**: Metorial son derece özelleştirilebilir, entegrasyonlarınızı ihtiyaçlarınıza uyacak şekilde yapılandırmanıza izin verir.
  - **Açık kaynaklı**: Metorial açık kaynaktır, bu nedenle kendi altyapınızda çalıştırabilir veya barındırılan platformumuzu kullanabilirsiniz.
  - **Çoklu örnek desteği**: Farklı yapılandırmaları, ortamları veya entegrasyon sürümlerini test etmek için Metorial Projelerinizin birden fazla örneğini oluşturun.
  - **Güçlü SDK'lar**: Metorial, AI uygulamalarınızla entegrasyonu kolaylaştıran JavaScript/TypeScript ve Python için güçlü SDK'lar sağlar.
  - **Ayrıntılı belgeler**: Metorial, tüm özellikleri için hızlı başlamanıza yardımcı olmak üzere örnekler ve öğreticiler dahil [ayrıntılı belgeler](https://metorial.com/docs) sağlar.
  - **Tam API erişimi**: Metorial'in her özelliği API aracılığıyla erişilebilir, özel entegrasyonlar oluşturmanıza ve iş akışlarınızı otomatikleştirmenize izin verir. Teorik olarak, API'yi kullanarak kendi panonuzu oluşturabilirsiniz.
  - **Gelişmiş pano**: Metorial Paneli, entegrasyonlarınızı yönetmek, kullanımınızı izlemek ve MCP sunucularınızda hata ayıklamak için güçlü bir arayüz sağlar.
