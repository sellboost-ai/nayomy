...
body_tr: |-
  ...
  > [!IMPORTANT]
  > :test_tube: Bu proje deneyseldir ve breaking changes'a konu olabilir.
  ...
  Tam [Massive.com](https://massive.com?utm_campaign=mcp&utm_medium=referral&utm_source=github) finansal veri API'sine LLM dostu bir arayüzle erişim sağlayan [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) sunucusu.

  Endpoint başına bir tool sunmak yerine, bu sunucu LLM'ye tam Massive.com API yüzeyini kaplayan üç composable tool sunar — **search**, **call** ve **query**. Veriler bir bellek içi SQLite veritabanında saklanabilir ve yerleşik finansal fonksiyonlarla zenginleştirilir.

  ## Araçlar (Tools)

  | Tool | Açıklama |
  |---|---|
  | `search_endpoints` | API endpoint'lerini ve yerleşik fonksiyonları doğal dil sorgusuna göre arayın. Başlıkları, yol desenlerini ve açıklamaları döndürür. Sorgu parametresi dokümanı için `detail`'i `"more"` değerine ayarlayın veya tam dokümantasyon için `"verbose"`'a ayarlayın. `max_results` ile sonuç sayısını sınırlayın. |
  | `call_api` | Herhangi bir Massive.com REST API endpoint'ini çağırın. Sonuçları bellek içi veritabanı tablosu olarak depolamayı (`store_as`) ve post-processing fonksiyonlarını uygulamayı (`apply`) destekler. Sayfalandırılmış yanıtlara sonraki sayfa ipucu dahildir. |
  | `query_data` | Depolanan SQLite DB'ye SQL çalıştırın. `SHOW TABLES`, `DESCRIBE <table>`, `DROP TABLE <table>`, CTE'ler, window fonksiyonları ve daha fazlasını destekler. Sonuçlar ayrıca `apply` ile post-process edilebilir. |

  ### Yerleşik Fonksiyonlar

  Fonksiyonlar `call_api` ve `query_data`'daki `apply` parametresi aracılığıyla API sonuçlarına veya sorgu çıktısına uygulanabilir. Bunları keşfetmek için `search_endpoints`'i `scope="functions"` ile kullanın.

  | Kategori | Fonksiyonlar |
  |---|---|
  | **Greeks** | `bs_price`, `bs_delta`, `bs_gamma`, `bs_theta`, `bs_vega`, `bs_rho` — Black-Scholes opsiyon fiyatlandırması ve greekler |
  | **Returns** | `simple_return`, `log_return`, `cumulative_return`, `sharpe_ratio`, `sortino_ratio` |
  | **Technical** | `sma` (basit hareketli ortalama), `ema` (üstel hareketli ortalama) |

  ### Veri Kapsamı

  Sunucu, başlangıçta [`llms.txt`](https://massive.com/docs/rest/llms.txt) dosyasından tüm Massive.com API endpoint'lerini dinamik olarak indeksler, böylece API ile otomatik olarak senkronize kalır. Kapsam aşağıdakileri içerir:

  - Hisse senedi, opsiyon, forex, kripto ve vadeli işlem agregasyonları
  - Gerçek zamanlı ve geçmiş işlemler ve alıntılar
  - Pazar anlık görüntüleri, kazananlar/kaybedenler
  - Ticker detayları ve referans verileri
  - Temettüler, bölünmeler, IPO'lar
  - Finansal temel veriler
  - Analist derecelendirmeleri ve haberler (Benzinga)
  - Hazine getirileri, enflasyon verileri
  - Pazar durumu ve tatiller
  ...
  - Python 3.12+
  - Bir Massive.com API anahtarı <br> [![Button]][Link]
  - [Astral UV](https://docs.astral.sh/uv/getting-started/installation/) (v0.4.0+)

  ### Claude Code
  İlk olarak [Claude Code](https://docs.anthropic.com/en/docs/agents-and-tools/claude-code/overview)'u yükleyin

  ```bash
  npm install -g @anthropic-ai/claude-code
  ```

  MCP sunucusunu yükleyin, ardından Claude Code'a kaydedin:

  ```bash
  # Sunucuyu yükleyin (tek seferlik — bağımlılıkları önceden indir)
  ...
  Daha sonra yeni bir sürüme yükseltmek için:
  ...
  > [!NOTE]
  > **`uvx` veya `uv run --with`'den yükseltiliyor musunuz?** Önceki sürümler `uvx --from ... mcp_massive` veya `uv run --with` komutlarını öneriyordu. Bu komutlar her soğuk başlangıçta bağımlılıkları indirir ve sunucunun Claude'un 30 saniyelik bağlantı zaman aşımını aşmasına neden olabilir. Yukarıda gösterildiği gibi `uv tool install`'a geçin — bağımlılıkları bir kez indirir ve bundan sonra anında başlar.

  Bu komut, geçerli projenizde MCP sunucusunu yükleyecektir.
  Bunu küresel olarak yüklemek istiyorsanız, komutu `-s <scope>` bayrağı ile çalıştırabilirsiniz.
  Daha fazla seçenek için `claude mcp add --help`'e bakın.

  Claude Code'u başlatmak için terminalinizde `claude` komutunu çalıştırın.
  - İlk kez kullanıyorsanız, kimlik doğrulaması için kurulum istemlerini takip edin

  Ayrıca MCP sunucusu Claude Desktop için zaten yüklüyse `claude mcp add-from-claude-desktop` komutunu çalıştırabilirsiniz.

  ### Claude Desktop

  1. İlk kurulumu tamamlamak ve yapılandırma dosyanızı bulmak için [Claude Desktop MCP kurulum talimatlarını](https://modelcontextprotocol.io/quickstart/user) izleyin.
  1. Sunucuyu yükleyin:
  ...
  4. Sunucuyu Claude Desktop konfigürasyon dosyanıza ekleyin. `<path_to_mcp_massive>`'i önceki adımdan çıktısıyla değiştirin ve kalan alanları doldurun.
  ...
  |---|---|---|---|
  | `MASSIVE_API_KEY` | Evet | — | Massive.com API anahtarınız |
  | `POLYGON_API_KEY` | Hayır | — | `MASSIVE_API_KEY` için kullanımdan kaldırılmış takma ad |
  | `MCP_TRANSPORT` | Hayır | `stdio` | İletim protokolü: `stdio`, `sse` veya `streamable-http` |
  | `MASSIVE_API_BASE_URL` | Hayır | `https://api.massive.com` | API istekleri için temel URL |
  | `MASSIVE_LLMS_TXT_URL` | Hayır | `https://massive.com/docs/rest/llms.txt` | Endpoint indeksi için URL |
  | `MASSIVE_MAX_TABLES` | Hayır | `50` | Maksimum bellek içi tablo sayısı |
  | `MASSIVE_MAX_ROWS` | Hayır | `50000` | Depolanan tablo başına maksimum satır |

  ### İletim (Transport)

  Varsayılan olarak STDIO iletimi kullanılır. İletim, `--transport` CLI argümanı veya `MCP_TRANSPORT` ortam değişkeni ile ayarlanabilir (CLI argümanı önceliklidir).
  ...
  Entegre edildiğinde, Claude'u Massive.com verilerine erişmesi için istemlendirebilirsiniz:

  ```
  AAPL hissesinin en son fiyatını al
  MSFT'nin dün ticaret hacmini göster
  Bugün hisse senedi piyasasındaki en büyük kazananlar nelerdi?
  BTC-USD için en son kripto pazar verilerini al
  Son 3 ay boyunca AAPL kapanış fiyatları için 20 günlük SMA hesapla
  Bu opsiyon sözleşmeleri için Black-Scholes delta'sını hesapla
  ```
  ...
  [Ön Koşullar](#ön-koşullar) bölümünde yüklü olduğundan emin olun.
  ...
  <details>
    <summary>claude_desktop_config.json için Yerel Geliştirme Yapılandırması</summary>

  Yerel çıkışınızdan yükleyin, ardından binary'yi doğrudan referans alın:
  ...
  Hata ayıklama ve test için [MCP Inspector](https://github.com/modelcontextprotocol/inspector)'u kullanmanızı öneriz:

  ```bash
  npx @modelcontextprotocol/inspector uv --directory /path/to/mcp_massive run mcp_massive
  ```

  Bu, MCP sunucunuzla doğrudan etkileşim kurabileceğiniz ve her tool için giriş/çıkışı görebileceğiniz bir tarayıcı arayüzü başlatacaktır.

  Tarayıcı UI'sinde `STDIO` iletim türünü `uv` komutu ve `run mcp_massive` argümanları ile seçin.

  ### Kod Linting

  Bu proje, yaygın geliştirme görevleri için [just](https://github.com/casey/just) kullanır. Bir PR göndermeden önce kodunuzu linting etmek için:

  ```bash
  just lint
  ```

  Bu, `ruff format` ve `ruff check --fix` komutlarını çalıştırarak kodunuzu otomatik olarak biçimlendirecek ve linting sorunlarını düzeltecektir.

  ## Bağlantılar
  - [Massive.com Dokumentasyonu](https://massive.com/docs?utm_campaign=mcp&utm_medium=referral&utm_source=github)
  - [Model Context Protocol](https://modelcontextprotocol.io)
  - [MCP Python SDK](https://github.com/modelcontextprotocol/python-sdk)

  ## Gizlilik Politikası

  Bu MCP sunucusu, pazar verilerini almak için Massive.com'un API'si ile etkileşime girer. Tüm veri istekleri Massive.com'un gizlilik politikası ve hizmet şartlarına tabidir.

  - **Massive.com Gizlilik Politikası**: https://massive.com/legal/privacy
  - **Veri İşleme**: Bu sunucu hiçbir kullanıcı verisi depolamaz veya önbelleğe almaz. Tüm istekler doğrudan Massive.com'un API'sine yönlendirilir.
  - **API Anahtarı**: Massive.com API anahtarınız yalnızca API'lerine karşı istekleri doğrulamak için kullanılır.
  - **User-Agent**: API istekleri MCP sunucusu sürümünü içeren bir User-Agent dizesi içerir (örneğin, `MCP-Massive/0.x.y`). Kişisel olarak tanımlanabilir hiçbir bilgi dahil edilmez.

  ## Katkıda Bulunma
  Bir hata bulduysanız veya yeni bir özellik için fikriniz varsa, lütfen ilk olarak yeni bir issue göndererek bunu bizimle tartışın.
  Sorunlara en geç 3 hafta içinde cevap vereceğiz.
  Açık sorunlar için PR göndermek istiyorsanız da gönüllülere açığız, ancak lütfen bunu önceden bizimle tartışın.
  Mevcut bir issue'ye bağlı olmayan veya önceden bizimle tartışılmayan PR'ler genellikle reddedilecektir.

  <!----------------------------------------------------------------------------->
  [Link]: https://massive.com/?utm_campaign=mcp&utm_medium=referral&utm_source=github 'Massive.com Ana Sayfası'
  <!---------------------------------[ Buttons ]--------------------------------->
