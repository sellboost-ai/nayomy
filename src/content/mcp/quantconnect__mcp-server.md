...
description_tr: "Yerel AI'nız (Claude Desktop vb.) ile QuantConnect API'sini bağlayan Dockerize edilmiş Python MCP sunucusu—doğal dil komutlarıyla projeleri oluşturabilir, stratejileri backtest edebilir, işbirlikçileri yönetebilir ve canlı ticaret iş akışlarını doğrudan deploy edebilirsiniz."
...
body_tr: |-
  ...
  | -------- | ------- |
  | `read_account` | Kuruluş hesap durumunu okuyun. |
  | `create_project` | Varsayılan kuruluşunuzda yeni bir proje oluşturun. |
  | `read_project` | Bir projenin veya yakın zamanda kullanılan bir dizi projenin ayrıntılarını listeleyin. |
  | `list_projects` | Tüm projelerin ayrıntılarını listeleyin. |
  | `update_project` | Bir projenin adını veya açıklamasını güncelleyin. |
  | `delete_project` | Projeyi silin. |
  | `create_project_collaborator` | Bir projeye işbirlikçi ekleyin. |
  | `read_project_collaborators` | Bir projedeki tüm işbirlikçileri listeleyin. |
  | `update_project_collaborator` | Bir projede işbirlikçi bilgilerini güncelleyin. |
  | `delete_project_collaborator` | İşbirlikçiyi projeden kaldırın. |
  | `lock_project_with_collaborators` | Projeyi düzenleyebilmeniz için kilitleyin. |
  | `read_project_nodes` | Bir projenin mevcut ve seçilen node'larını okuyun. |
  | `update_project_nodes` | Verilen node'ların etkin durumunu true olarak güncelleyin. |
  | `create_compile` | Eşzamansız olarak bir proje için derleme işi isteği oluşturun. |
  | `read_compile` | Derleme paketi işi sonucunu okuyun. |
  | `create_file` | Verilen projeye dosya ekleyin. |
  | `read_file` | Bir projeden dosya okuyun veya dosya adı belirtilmemişse projedeki tüm dosyaları okuyun. |
  | `update_file_name` | Dosyanın adını güncelleyin. |
  | `update_file_contents` | Dosyanın içeriğini güncelleyin. |
  | `patch_file` | Bir projedeki dosyaya yama (unified diff) uygulayın. |
  | `delete_file` | Projede dosya silin. |
  | `create_backtest` | Yeni bir backtest isteği oluşturun ve backtest kimliğini alın. |
  | `read_backtest` | Bir backtestin sonuçlarını okuyun. |
  | `list_backtests` | Proje için tüm backtestleri listeleyin. |
  | `read_backtest_chart` | Bir backtestinden grafik okuyun. |
  | `read_backtest_orders` | Bir backtestin emirlerini okuyun. |
  | `read_backtest_insights` | Bir backtestin öngörülerini okuyun. |
  | `update_backtest` | Bir backtestin adını veya notunu güncelleyin. |
  | `delete_backtest` | Projeden backtest silin. |
  | `estimate_optimization_time` | Belirtilen parametrelerle bir optimizasyonun yürütme süresini tahmin edin. |
  | `create_optimization` | Belirtilen parametrelerle bir optimizasyon oluşturun. |
  | `read_optimization` | Bir optimizasyonu okuyun. |
  | `list_optimizations` | Bir proje için tüm optimizasyonları listeleyin. |
  | `update_optimization` | Bir optimizasyonun adını güncelleyin. |
  | `abort_optimization` | Bir optimizasyonu durdurun. |
  | `delete_optimization` | Optimizasyonu silin. |
  | `authorize_connection` | Harici bir bağlantıyı canlı bir aracı kurum veya veri sağlayıcısıyla yetkilendirin. |
  | `create_live_algorithm` | Canlı bir algoritma oluşturun. |
  | `read_live_algorithm` | Canlı bir algoritmanın ayrıntılarını okuyun. |
  | `list_live_algorithms` | Geçmiş ve güncel tüm canlı ticaret dağıtımlarınızı listeleyin. |
  | `read_live_chart` | Canlı bir algoritma grafiğini okuyun. |
  | `read_live_logs` | Canlı bir algoritmanın log'larını alın. |
  | `read_live_portfolio` | Canlı bir algoritmanın portföy durumunu okuyun. |
  | `read_live_orders` | Canlı bir algoritmanın emirlerini okuyun. |
  | `read_live_insights` | Canlı bir algoritmanın öngörülerini okuyun. |
  | `stop_live_algorithm` | Canlı bir algoritma'yı durdurun. |
  | `liquidate_live_algorithm` | Canlı bir algoritma'yı tasfiye edip durdurun. |
  | `create_live_command` | Canlı ticaret algoritmasına komut gönderin. |
  | `broadcast_live_command` | Canlı komutu bir kuruluşun tüm canlı algoritmalarına yayınlayın. |
  | `upload_object` | Dosyaları Object Store'a yükleyin. |
  | `read_object_properties` | Belirli bir kuruluş ve anahtar için Object Store özelliklerini alın. |
  | `read_object_store_file_job_id` | Object Store'dan dosya indirmek için bir iş oluşturun ve ardından iş kimliğini okuyun. |
  | `read_object_store_file_download_url` | Object Store'dan dosya indirmek için URL'yi alın. |
  | `list_object_store_files` | Bir kuruluşta belirli bir dizin altındaki Object Store dosyalarını listeleyin. |
  | `delete_object` | Belirli bir kuruluş ve anahtar için Object Store dosyasını silin. |
  | `read_lean_versions` | Her sürüm için temel bilgileriyle birlikte LEAN sürümlerinin bir listesini döndürün. |
  | `check_initialization_errors` | Algoritma'yı başlatmak için bir backtest'i birkaç saniye çalıştırın ve başlatma hataları olup olmadığını alın. |
  | `complete_code` | Belirli bir metin girişi için kod tamamlamayı gösterin. |
  | `enhance_error_message` | Hata mesajları için ek bağlam ve öneriler gösterin. |
  | `update_code_to_pep8` | Python kodunu PEP8 stilini takip edecek şekilde güncelleyin. |
  | `check_syntax` | Bir kodun sözdizimini kontrol edin. |
  | `search_quantconnect` | QuantConnect'te içerik arayın. |
  | `read_mcp_server_version` | Çalışan QC MCP Server'ın versiyonunu döndürün. |
  | `read_latest_mcp_server_version` | Yayınlanan en son QC MCP Server versiyonunu döndürün. |
   --- 
  ## Araç Detayları
  **Araç:** `read_account`
  ...
  **Araç:** `list_backtests`
  ...
  | `includeStatistics` | `boolean` *isteğe bağlı* | True ise, yanıttaki backtest ö
---
