...
description_tr: "Keboola Connection Data Platform ile etkileşim kurun. Bu sunucu, Keboola Storage API'den verileri listelemek ve erişmek için gerekli araçları sağlar."
...
body_tr: |-
  ...
  > AI ajanlarınızı, MCP istemcilerinizi (**Cursor**, **Claude**, **Windsurf**, **VS Code** ...) ve diğer AI asistanlarınızı Keboola'ya bağlayın. Veri, dönüşümler, SQL sorguları ve iş tetikleyicilerini ortaya çıkarın—glue kod gerekmez. Ajanların ihtiyaç duyduğu doğru verileri doğru zamanda ve yerde sunun.
  
  ## Genel Bakış
  
  Keboola MCP Server, Keboola projeniz ile modern AI araçları arasında açık kaynaklı bir köprüdür. Keboola özelliklerini—depolama erişimi, SQL dönüşümleri ve iş tetikleyicileri gibi—Claude, Cursor, CrewAI, LangChain, Amazon Q ve diğerleri için çağrılabilir araçlara dönüştürür.
  
  - [Hızlı Başlangıç](#-hızlı-başlangıç-uzak-mcp-sunucusu-en-kolay-yol)
  - [Yerel Kurulum](#yerel-mcp-sunucusu-kurulumu-özel-veya-geliştirme-yolu)
  
  ## Özellikler
  
  AI Ajanı ve MCP Server ile şunları yapabilirsiniz:
  
  - **Depolama**: Tabloları doğrudan sorgulayın ve tablo veya demet açıklamalarını yönetin
  - **Bileşenler**: Çıkarıcılar, yazıcılar, veri uygulamaları ve dönüşüm yapılandırmalarını oluşturun, listeleyin ve inceleyin
  - **SQL**: Doğal dil ile SQL dönüşümleri oluşturun
  - **İşler**: Bileşenleri ve dönüşümleri çalıştırın ve iş yürütme ayrıntılarını alın
  - **Akışlar**: Koşullu Akışlar ve Orchestrator Akışları kullanarak iş akışı boru hatlarını oluşturun ve yönetin
  - **Veri Uygulamaları**: Depolama verileriniz üzerinde sorgularınızı görüntüleyen Keboola Streamlit Veri Uygulamalarını oluşturun, dağıtın ve yönetin
  - **Meta veri**: Proje belgelerini ve nesne meta verilerini doğal dil kullanarak arayın, okuyun ve güncelleyin
  - **Geliştirme Şubeleri**: Prodüksiyonun dışında geliştirme şubelerinde güvenle çalışın; tüm işlemler seçilen şubeye kapsamlıdır
  
  ---
  
  ## 🚀 Hızlı Başlangıç: Uzak MCP Sunucusu (En Kolay Yol)
  
  Keboola MCP Server'ı kullanmanın en kolay yolu **Uzak MCP Sunucusu** aracılığıyladır. Bu barındırılan çözüm, yerel kurulum, yapılandırma veya yükleme ihtiyacını ortadan kaldırır.
  
  ### Uzak MCP Sunucusu Nedir?
  
  Uzak sunucumuz her çok kiracılı Keboola yığınında barındırılır ve OAuth kimlik doğrulamayı destekler. Uzak Akışlı HTTP bağlantısı ve OAuth kimlik doğrulamayı destekleyen herhangi bir AI asistanından bağlanabilirsiniz.
  
  ### Nasıl Bağlanılır
  
  1. **Uzak sunucu URL'nizi alın**: Keboola Proje Ayarlarına gidin → `MCP Server` sekmesi
  ...
  4. **Kimlik doğrulaması yapın**: Keboola hesabınız ile kimlik doğrulaması yapmanız ve projenizi seçmeniz istenecektir
  ...
  - **[Make](https://make.com)**: Uzak sunucu URL'si ile yapılandırın
  - **Diğer MCP istemcileri**: Uzak sunucu URL'si ile yapılandırın
  
  #### Claude Code Kurulumu
  
  Claude Code, terminalinizi kullanarak Claude ile etkileşime girmek için izin veren bir komut satırı arayüzü aracıdır. Basit bir komut kullanarak Keboola MCP Server entegrasyonunu yükleyebilirsiniz.
  
  **Yükleme:**
  
  Terminalinizde aşağıdaki komutu çalıştırın, `<YOUR_REGION>` yerine Keboola bölgenizi koyun:
  ...
  **Kullanım:**
  ...
  Yüklendikten sonra, Claude Code'da konuşmanızda `/mcp` yazıp kullanmak istediğiniz Keboola araçlarını seçerek Keboola MCP Server'ı kullanabilirsiniz.
  
  **Kimlik Doğrulaması:**
  
  Claude Code'da Keboola MCP Server'ı ilk kez kullandığınızda, bir tarayıcı penceresi açılacak ve sizden aşağıdakileri yapmanız istenecektir:
  1. Keboola hesabınız ile oturum açın
  2. Bağlanmak istediğiniz projeyi seçin
  3. Bağlantıyı yetkilendirin
  
  Kimlik doğrulamasından sonra, Claude Code'dan doğrudan Keboola araçlarını kullanmaya başlayabilirsiniz.
  
  Ayrıntılı kurulum talimatları ve bölgeye özel URL'ler için [Uzak Sunucu Kurulum belgelerine](https://help.keboola.com/ai/mcp-server/#remote-server-setup) bakın.
  
  ### Geliştirme Şubelerini Kullanma
  Prodüksiyondaki verileri etkilemeden [Keboola geliştirme şubelerinde](https://help.keboola.com/components/branches/) güvenle çalışabilirsiniz. Uzaktan barındırılan MCP Sunucuları `KBC_BRANCH_ID` parametresini saygı gösterir ve tüm işlemleri belirtilen şubeye kapsamlar. Geliştirme şube kimliğini, kullanıcı arayüzünde geliştirme şubesine gittiğinizde URL'de bulabilirsiniz, örneğin: `https://connection.us-east4.gcp.keboola.com/admin/projects/PROJECT_ID/branch/BRANCH_ID/dashboard`. Şube kimliği, `X-Branch-Id: <branchId>` başlığı kullanılarak her istekten eklenmeli, aksi takdirde MCP Server varsayılan olarak prodüksiyonsal şubeyi kullanır. Bunu, AI istemcisi veya sunucu bağlantısını işleyen ortam tarafından yönetilmelidir.
  
  ### Araç Yetkilendirmesi ve Erişim Kontrolü
  
  HTTP tabanlı aktarımlar (Akışlı HTTP) kullanırken, HTTP başlıklarını kullanarak istemciler için hangi araçların kullanılabileceğini kontrol edebilirsiniz. Bu, AI ajanı yeteneklerini kısıtlamak veya uyumluluk ilkelerini uygulamak için kullanışlıdır.
  
  #### Yetkilendirme Başlıkları
  
  | Başlık | Açıklama | Örnek |
  |--------|----------|--------|
  | `X-Allowed-Tools` | İzin verilen araçların virgülle ayrılmış listesi | `get_configs,get_buckets,query_data` |
  | `X-Disallowed-Tools` | Hariç tutulacak araçların virgülle ayrılmış listesi | `create_config,run_job` |
  | `X-Read-Only-Mode` | Yalnızca salt okunur araçlarla sınırlandırın | `true`, `1`, veya `yes` |
  
  #### Filtre Davranışı
  
  Filtreler sırasına göre uygulanır: izin verilen → salt okunur kesişim → yasak hariç tutma. Boş başlıklar = kısıtlama yok.
  ...
  Ayrıntılı belgeler için [developers.keboola.com/integrate/mcp/#tool-authorization-and-access-control](https://developers.keboola.com/integrate/mcp/#tool-authorization-and-access-control) adresine bakın.
  
  ---
  
  ## Yerel MCP Sunucusu Kurulumu (Özel veya Geliştirme Yolu)
  
  Tam kontrol ve kolay geliştirme için MCP sunucusunu kendi makinenizde çalıştırın. Araçları özelleştirmek, yerel olarak hata ayıklamak veya hızlı yineleme yapmak istediğinizde bunu seçin. Depoyu klonlayacak, sunucu aktarımına bağlı olarak Keboola kimlik bilgilerini ortam değişkenleri veya başlıklar aracılığıyla ayarlayacak, bağımlılıkları yükleyecek ve sunucuyu başlatacaksınız. Bu yaklaşım maksimum esneklik (özel araçlar, yerel günlüğe kaydetme, çevrimdışı yineleme) sunar, ancak manuel kurulum gerektirir ve güncellemeleri ile sırları siz yönetirsiniz.
  
  Sunucu, sunucuyu başlatırken `--transport <transport>` argümanı sağlayarak seçilebilen birden fazla **aktarım** seçeneğini destekler:
  - `stdio` - `--transport` belirtilmediğinde varsayılan. Standart giriş/çıkış, tipik olarak tek bir istemci ile yerel dağıtım için kullanılır.
  - `streamable-http` - Sunucuyu çift yönlü akış kanalı ile HTTP üzerinden uzaktan çalıştırır, istemci ve sunucunun sürekli olarak mesaj alışverişi yapmasına izin verir. <url>/mcp aracılığıyla bağlanın (örn. http://localhost:8000/mcp).
  - `http-compat` - `streamable-http` için bir takma ad, geriye dönük uyumluluk için tutulur.
  
  İstemci–sunucu iletişimi için, Keboola kimlik bilgileri, Keboola Bölgenizde projeniz ile çalışmayı etkinleştirmek için sağlanmalıdır. Aşağıdakiler gereklidir: `KBC_STORAGE_TOKEN`, `KBC_STORAGE_API_URL`, `KBC_WORKSPACE_SCHEMA` ve isteğe bağlı olarak `KBC_BRANCH_ID`. Bunu iki şekilde sağlayabilirsiniz:
  - Kişisel kullanım için (esas olarak stdio aktarımı ile): sunucuyu başlatmadan önce ortam değişkenlerini ayarlayın. Tüm istekler bu önceden tanımlanmış kimlik bilgilerini yeniden kullanacaktır.
  - Çok kullanıcılı kullanım için: değişkenleri istek başlıklarına ekleyin, böylece her istek, kendisiyle sağlanan kimlik bilgilerini kullanır.
  ...
  Depolama API belirteçlerini oluşturma ve yönetme talimatları için [resmi Keboola belgelerine](https://help.keboola.com/management/project/tokens/) bakın.
  
  **Not**: MCP sunucusunun sınırlı erişime sahip olmasını istiyorsanız, özel depolama belirteci kullanın; MCP'nin projenizde her şeye erişmesini istiyorsanız, ana belirteci kullanın.
  
  ### KBC_WORKSPACE_SCHEMA
  
  Bu, Keboola'da çalışma alanınızı tanımlar ve SQL sorguları için kullanılır. Ancak, bu **yalnızca Ana Belirteç yerine özel bir depolama belirteci kullanıyorsanız gereklidir**:
  
  - [Ana Belirteci](https://help.keboola.com/management/project/tokens/#master-tokens) kullanıyorsanız: Çalışma alanı arka planda otomatik olarak oluşturulur
  - [Özel depolama belirteci](https://help.keboola.com/management/project/tokens/#limited-tokens) kullanıyorsanız: KBC_WORKSPACE_SCHEMA'nız almak için bu [Keboola rehberini](https://help.keboola.com/tutorial/manipulate/workspace/) izleyin
  
  **Not**: Çalışma alanını el ile oluştururken, Tüm Proje verilerine salt okunur erişim ver seçeneğini işaretleyin
  
  **Not**: KBC_WORKSPACE_SCHEMA, BigQuery çalışma alanlarında Dataset Name olarak adlandırılır; Bağlan'a tıklayın ve Dataset Name'i kopyalayın
  
  ### KBC_STORAGE_API_URL (Keboola Bölgesi)
  
  Keboola Bölgesi API URL'niz dağıtım bölgenize bağlıdır. Keboola projenizde oturum açtığınızda tarayıcıdaki URL'ye bakarak bölgenizi belirleyebilirsiniz:
  
  | Bölge | API URL |
  |-------|---------|
  | AWS Kuzey Amerika | `https://connection.keboola.com` |
  | AWS Avrupa | `https://connection.eu-central-1.keboola.com` |
  | Google Cloud AB | `https://connection.europe-west3.gcp.keboola.com` |
  | Google Cloud ABD | `https://connection.us-east4.gcp.keboola.com` |
  | Azure AB | `https://connection.north-europe.azure.keboola.com` |
  
  ### KBC_BRANCH_ID (İsteğe Bağlı)
  
  Belirli bir [Keboola geliştirme şubesinde](https://help.keboola.com/components/branches/) işlem yapmak için, `KBC_BRANCH_ID` parametresini kullanarak şube kimliğini ayarlayın. MCP sunucusu, işlevselliğini belirtilen şubeye kapsamlar, tüm değişikliklerin yalıtılmış kalmasını ve prodüksiyonsal şubeyi etkilememesini sağlar.
  
  - Sağlanmazsa, sunucu varsayılan olarak prodüksiyonsal şubeyi kullanır.
  - Geliştirme işi için, `KBC_BRANCH_ID`'yi şubenizin sayısal kimliğine ayarlayın (örn. `123456`). Geliştirme şube kimliğini, kullanıcı arayüzünde geliştirme şubesine gittiğinizde URL'de bulabilirsiniz, örneğin: `https://connection.us-east4.gcp.keboola.com/admin/projects/PROJECT_ID/branch/BRANCH_ID/dashboard`.
  - Uzak aktarımlarda, her istek ile `X-Branch-Id: <branchId>` veya `KBC_BRANCH_ID: <branchId>` HTTP başlığı ile geçersiz kılabilirsiniz.
  ...
  **Not**: `uv` yüklü olduğundan emin olun. MCP istemcisi bunu, Keboola MCP Server'ı otomatik olarak indirmek ve çalıştırmak için kullanacaktır.
  **uv Yükleme**:
  
  *macOS/Linux*:
  
  ```bash
  #eğer homebrew makinenizde yüklü değilse kullanın:
  # /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"
  
  # Homebrew kullanarak yükleyin
  brew install uv
  ```
  ...
  *Windows*:
  ...
  ```powershell
  # Yükleyici komut dosyasını kullanarak
  powershell -ExecutionPolicy ByPass -c "irm https://astral.sh/uv/install.ps1 | iex"
  
  # Veya pip kullanarak
  pip install uv
  
  # Veya winget kullanarak
  winget install --id=astral-sh.uv -e
  ```
  
  Daha fazla yükleme seçeneği için [resmi uv belgelerine](https://docs.astral.sh/uv/getting-started/installation/) bakın.
  
  
  ### Keboola MCP Server'ı Çalıştırma
  
  İhtiyaçlarınıza bağlı olarak Keboola MCP Server'ı kullanmanın dört yolu vardır:
  
  ### Seçenek A: Entegre Mod (Önerilen)
  
  Bu modda, Claude veya Cursor, MCP sunucusunu sizin için otomatik olarak başlatır. **Terminalinizde hiçbir komut çalıştırmanız gerekmez**.
  ...
  3. Claude masaüstünü değişikliklerin etkili olması için yeniden başlatın
  ...
  2. "+ Yeni genel MCP Sunucusu Ekle" seçeneğine tıklayın
  3. Şu ayarlarla yapılandırın:
  ...
  **Not**: MCP sunucuları için kısa, açıklayıcı adlar kullanın. Tam araç adı sunucu adını içerdiğinden ve ~60 karakterin altında kalması gerektiğinden, daha uzun adlar Cursor'da filtrelenebilir ve Ajanı tarafından görüntülenmez.
  
  
  #### Windows WSL için Cursor Yapılandırması
  
  MCP sunucusunu Windows Subsystem for Linux'tan Cursor AI ile çalıştırırken şu yapılandırmayı kullanın:
  ...
  MCP sunucusu kodu üzerinde çalışan geliştiriciler için:
  ...
  Sunucuyu test veya hata ayıklama için terminalde el ile çalıştırabilirsiniz:
  ...
  > **Not**: Bu mod, esas olarak hata ayıklama veya test içindir. Claude veya Cursor ile normal kullanım için,
  > sunucuyu el ile çalıştırmanız gerekmez.
  
  > **Not**: Sunucu, Akışlı HTTP aktarımını kullanacak ve `/mcp`'deki gelen bağlantılar için `localhost:8000`'i dinleyecektir.
  > İnşa ettiği yeri değiştirmek için `--port` ve `--host` parametrelerini kullanabilirsiniz.
  ...
  > **Not**: Sunucu, Akışlı HTTP aktarımını kullanacak ve `/mcp`'deki gelen bağlantılar için `localhost:8000`'i dinleyecektir.
  > Konteyner portunu başka bir yere eşlemek için `-p` değerini değiştirebilirsiniz.
  
  ### Sunucuyu Kendim Başlatmam Gerekiyor mu?
  
  | Senaryo | El ile Çalıştırılması Gerekiyor mu? | Bu Kurulumu Kullanın |
  |---------|--------------------------------------|----------------------|
  | Claude/Cursor Kullanma | Hayır | Uygulamada MCP'yi yapılandırın |
  | Yerel MCP Geliştirme | Hayır (Claude başlatır) | Python yoluna işaret edin |
  | CLI'yı El ile Test Etme | Evet | Terminalde çalıştırmak için kullanın |
  | Docker Kullanma | Evet | Docker konteynerini çalıştırın |
  
  ## MCP Server Kullanımı
  
  MCP istemciniz (Claude/Cursor) yapılandırılıp çalışmaya başladıktan sonra, Keboola verilerinizi sorgulamaya başlayabilirsiniz:
  
  ### Kurulumunuzu Doğrulayın
  
  Her şeyin çalışıp çalışmadığını doğrulamak için basit bir sorgu ile başlayabilirsiniz:
  
  ```text
  Keboola projemde hangi kavanozlar ve tablolar var?
  ```
  
  ### Yapabileceğiniz Şeylere Örnekler
  
  **Veri İnceleme:**
  
  - "Müşteri bilgileri içeren tablolar nelerdir?"
  - "Gelire göre ilk 10 müşteriyi bulmak için bir sorgu çalıştırın"
  
  **Veri Analizi:**
  
  - "Son çeyrek için bölgeye göre satış verilerini analiz edin"
  - "Müşteri yaşı ve satın alma sıklığı arasındaki korelasyonları bulun"
  
  **Veri Boru Hatları:**
  
  - "Müşteri ve sipariş tablolarını birleştiren bir SQL dönüşümü oluşturun"
  - "Salesforce bileşenim için veri çıkarma işini başlatın"
