...
description_tr: "Kubernetes ve OpenShift desteği sunan güçlü bir MCP sunucusu. **Herhangi bir** Kubernetes kaynağı için CRUD işlemleri sağlamanın yanı sıra, kümenizle etkileşim kurmak için özelleştirilmiş araçlar sunmaktadır."
...
body_tr: |-
  ...
  [✨ Özellikler](#özellikler) | [🚀 Başlarken](#başlarken) | [🎥 Demolar](#demolar) | [⚙️ Yapılandırma](#yapılandırma) | [🛠️ Araçlar](#araçlar-ve-işlevsellik) | [💬 Topluluk](#topluluk) | [🧑‍💻 Geliştirme](#geliştirme)

  https://github.com/user-attachments/assets/be2b67b3-fc1c-4d11-ae46-93deba8ed98e

  ## ✨ Özellikler <a id="özellikler"></a>

  **Kubernetes** ve **OpenShift** desteğine sahip güçlü ve esnek bir Kubernetes [Model Context Protocol (MCP)](https://blog.marcnuri.com/model-context-protocol-mcp-introduction) sunucusu uygulaması.

  - **✅ Yapılandırma**:
    - Kubernetes yapılandırmasındaki değişiklikleri otomatik olarak algılayın ve MCP sunucusunu güncelleyin.
    - Geçerli [Kubernetes `.kube/config`](https://blog.marcnuri.com/where-is-my-default-kubeconfig-file) veya cluster içi yapılandırmayı **görüntüleyin** ve yönetin.
  ...
  Sistemde yüklü olması gereken **DÖŞ HARICI bağımlılık veya araç YOKTUR**.
  Native ikili dosyaları kullanıyorsanız, sisteminizde Node veya Python yüklü olması gerekmez.
  ...
  [Goose CLI](https://blog.marcnuri.com/goose-on-machine-ai-agent-cli-introduction) yapay zeka (AI) ajanları ile başlamak için en kolay (ve en ucuz) yoldur.
  ...
  ### Basit bir oyun _Vibe Coding_ ve OpenShift'e dağıtma

  Bu demoda, VS Code kullanarak basit bir oyunu _Vibe Coding_ yapma sürecini ve bunu OpenShift'e dağıtmak için [Podman MCP server](https://github.com/manusa/podman-mcp-server) ve Kubernetes MCP server'ını nasıl kullanacağınızı göstereceğim.
  ...
  ## ⚙️ Yapılandırma <a id="yapılandırma"></a>

  Kubernetes MCP server'ı komut satırı (CLI) argümanları kullanılarak yapılandırılabilir.

  CLI yürütülebilir dosyasını `npx`, `uvx` kullanarak veya [son yayın ikili dosyasını](https://github.com/containers/kubernetes-mcp-server/releases/latest) indirerek çalıştırabilirsiniz.
  ...
  | `--toolsets`              | Etkinleştirilecek araç setlerinin virgülle ayrılmış listesi. Daha fazla bilgi için [🛠️ Araçlar ve İşlevsellik](#araçlar-ve-işlevsellik) bölümünü kontrol edin.                                                                                                                                                |
  ...
  **[Yapılandırma Referansı](docs/configuration.md)**'na bakın.
  ...
  Sunucu, MCP günlüğe kaydetme yeteneğini destekler ve istemcilerin yapılandırılmış günlük mesajları aracılığıyla hata ayıklama bilgisi almasını sağlar.
  ...
  **[MCP Günlüğe Kaydetme Rehberi](docs/logging.md)**'ne bakın.
  ...
  Kubernetes MCP server'ı `--toolsets` komut satırı bayrağı veya `toolsets` yapılandırma seçeneği aracılığıyla belirli araç ve işlevsellik gruplarını (araçlar, kaynaklar, promptlar vb.) etkinleştirmeyi veya devre dışı bırakmayı destekler.
  Bu, AI araçlarınız için hangi Kubernetes işlevselliğinin kullanılabilir olduğunu kontrol etmenizi sağlar.
  Yalnızca ihtiyaç duyduğunuz araç setlerini etkinleştirmek, context boyutunu azaltmaya ve LLM'nin araç seçimi doğruluğunu iyileştirmeye yardımcı olabilir.
  ...
  | kiali    | Kiali'yi yönetmek için en yaygın araçlar, daha fazla ayrıntı için [Kiali dokümantasyonuna](https://github.com/containers/kubernetes-mcp-server/blob/main/docs/KIALI.md) bakın.            |         |
  | kubevirt | KubeVirt sanal makine yönetimi araçları, daha fazla ayrıntı için [KubeVirt dokümantasyonuna](https://github.com/containers/kubernetes-mcp-server/blob/main/docs/kubevirt.md) bakın. |         |
  ...
  - **namespaces_list** - Geçerli cluster'daki tüm Kubernetes namespace'lerini listeleyin
    - `fieldSelector` (`string`) - Namespace'leri alan değerlerine göre filtrelemek için isteğe bağlı Kubernetes field selector'ı (örn. 'metadata.name=default', 'status.phase=Active'). Desteklenen alanlar: metadata.name, status.phase. Bkz. https://kubernetes.io/docs/concepts/overview/working-with-objects/field-selectors/
  ...
  - **nodes_log** - Kubernetes node'undan günlükleri alın (kubelet, kube-proxy veya diğer sistem günlükleri). Bu, Kubernetes API proxy'si aracılığıyla kubelet'e erişir
    ...
    - `query` (`string`) **(gerekli)** - query, günlüklerin döndürüleceği servis(ler) veya dosyaları belirtir (gerekli). Örnek: kubelet günlüklerini almak için "kubelet", node'dan belirli bir günlük dosyasını almak için "/<log-file-name>" (örn., "/var/log/kubelet.log" veya "/var/log/kube-proxy.log")
